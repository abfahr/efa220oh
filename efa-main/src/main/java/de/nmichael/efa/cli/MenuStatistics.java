...
public class MenuStatistics extends MenuData {
  ...
  private boolean setFilter(StatisticsRecord sr, UUID id, String name) {
    try {
      if (sr.getFilterPromptPerson()) {
        if (name != null) {
          PersonRecord p = Daten.project.getPersons(false).getPerson(name,
              System.currentTimeMillis());
          if (p == null) { // try again with "_" replaced by " "
            p = Daten.project.getPersons(false).getPerson(EfaUtil.replace(name, "_", " ", true),
                System.currentTimeMillis());
          }
          if (p != null) {
            id = p.getId();
      ...
      if (sr.getFilterPromptBoat()) {
        if (name != null) {
          ...
          if (b == null) { // try again with "_" replaced by " "
            b = Daten.project.getBoats(false).getBoat(EfaUtil.replace(name, "_", " ", true),
                System.currentTimeMillis());
          }
          if (b != null) {
            id = b.getId();
      ...
      if (sr.getFilterPromptGroup()) {
        if (name != null) {
          GroupRecord g = Daten.project.getGroups(false).findGroupRecord(name,
              System.currentTimeMillis());
          if (g == null) { // try again with "_" replaced by " "
            g = Daten.project.getGroups(false).findGroupRecord(
                EfaUtil.replace(name, "_", " ", true), System.currentTimeMillis());
          }
          if (g != null) {
            id = g.getId();
          }
