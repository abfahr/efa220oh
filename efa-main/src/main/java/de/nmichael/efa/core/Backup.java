    ...
    try {
      ...
      if (isProjectDataAccess(meta.getType())) {
        ...
        if (currentProjectName == null ||
          ...
          logMsg(
              Logger.ERROR,
              Logger.MSG_BACKUP_RESTOREERROR,
              LogString.fileRestoreFailed(
                  meta.getNameAndType(),
                  meta.getDescription(),
                  International
                      .getMessage(
  ...
  public int runBackup(BackupTask backupTask) {
    ...
    try {
      ...
      if (backupProject) {
        ...
        cnt = backupStorageObjects(dataAccesses, zipOut, Daten.efaSubdirDATA + Daten.fileSep
            + currentProjectName);
        successful += cnt;
    ...
    try {
      ...
      if (errors == 0) {
        ...
        logMsg(Logger.INFO, Logger.MSG_BACKUP_RESTOREFINISHEDWITHERRORS,
            LogString.operationFinishedWithErrors(International.getString("Wiederherstellung"),
                errors));
      }
    } catch (Exception e) {
      lastErrorMsg = LogString.operationFailed(International.getString("Wiederherstellung"),
          e.toString());
      logMsg(Logger.ERROR, Logger.MSG_BACKUP_RESTOREFAILED,
          lastErrorMsg);
      Logger.logdebug(e);
  ...
  private void openOrCreateProject(String newProjectName) {
    try {
      ...
      if (Daten.project != null) {
        logMsg(
            Logger.INFO,
            Logger.MSG_BACKUP_REOPENINGFILES,
            LogString.fileClosing(Daten.project.getProjectName(),
      ...
      if (Project.openProjectSilent(newProjectName, false)) {
        logMsg(Logger.INFO, Logger.MSG_EVT_PROJECTOPENED,
