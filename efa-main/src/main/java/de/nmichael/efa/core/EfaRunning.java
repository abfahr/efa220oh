...
public class EfaRunning {
  ...
  public boolean run() {
    ...
    try {
      BufferedWriter f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
          Daten.efaBaseConfig.efaUserDirectory + Daten.EFA_RUNNING), Daten.ENCODING_UTF));
      f.write(Integer.toString(port) + " (" + EfaUtil.getCurrentTimeStamp() + ")\n");
      f.close();
    } catch (Exception e) {
      Logger.log(
          Logger.ERROR,
          Logger.MSG_ERR_EFARUNNING_FAILED,
          LogString.fileCreationFailed(Daten.efaBaseConfig.efaUserDirectory + Daten.EFA_RUNNING,
          ...
          if (!myID.equals(s)) {
            Logger.log(Logger.ERROR, Logger.MSG_ERR_DATALOCK_CONFLICT,
                International.getMessage(
                    "efa hat eine fremde Datenänderung von {program} festgestellt. " +
  ...
  /*
   * // nur zu Testzwecken public static void main(String[] args) {
   * Daten.efaBaseConfig.efaUserDirectory = "./"; EfaRunning r = new EfaRunning(); if
   * (args[0].equals("SERVER")) { r.run(); try { Thread.sleep(30000); } catch(Exception e) {}
   * r.closeServer(); } else { r.isRunning(args[0]); } }
   */
}
