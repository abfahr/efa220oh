...
public class EfaSec {
  ...
  private String read() {
    try {
      BufferedReader fsec = new BufferedReader(new InputStreamReader(new FileInputStream(filename),
          Daten.ENCODING_UTF));
  ...
  public boolean isSecFileWritable() {
    try {
      BufferedReader fsecr = new BufferedReader(new InputStreamReader(
          new FileInputStream(filename), Daten.ENCODING_UTF));
      String efaSecSHA = fsecr.readLine();
      fsecr.close();
      BufferedWriter fsecw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(
          filename), Daten.ENCODING_UTF));
  ...
  public boolean writeSecFile(String sha, boolean dontDelete) {
    try {
      BufferedWriter fsec = new BufferedWriter(new OutputStreamWriter(
  ...
  public static boolean createNewSecFile(String secFile, String jarFile) {
    ...
    try {
      ...
      Logger.log(
          Logger.INFO,
          Logger.MSG_CORE_EFASECCREATED,
          LogString.fileNewCreated(efaSec.getFilename(),
              International.getString("Sicherheitsdatei")));
      return true;
    } catch (Exception e) {
      Logger.log(
          Logger.ERROR,
          Logger.MSG_CORE_EFASECFAILEDCREATE,
          LogString.fileCreationFailed(efaSec.getFilename(),
              International.getString("Sicherheitsdatei"), e.toString()));
      return false;
    }
  }
