...
public class OnlineUpdate {
  ...
  public synchronized static boolean runOnlineUpdate(JDialog parent, String eouFile) {
    ...
    if (!canWrite) {
      if (parent != null) {
        Dialog.error(LogString.directoryNoWritePermission(Daten.efaMainDirectory,
            International.getString("Verzeichnis"))
            +
            "\n\n"
            +
            International.getMessage(
                "Bitte wiederhole das Online-Update als {osname}-Administrator.", Daten.osName));
      } else {
        lastError = LogString.directoryNoWritePermission(Daten.efaMainDirectory,
            International.getString("Verzeichnis"));
      }
      return false;
    }
...
class ExecuteAfterDownloadImpl implements ExecuteAfterDownload {
  ...
  public void success() {
    ...
    if (parent != null) {
      ...
      .infoDialog(
          ...
          + (Daten.applID != Daten.APPL_DRV ? "\n"
              +
              International
              .getString("Es werden jetzt alle Daten gesichert und anschließend die neue Version installiert.")
              : ""));
    }
    ...
    if (parent != null) {
      Dialog.infoDialog(
          International.getString("Version aktualisiert"),
          LogString.operationSuccessfullyCompleted(International
              .getString("Installation des Updates"))
              + "\n" +
