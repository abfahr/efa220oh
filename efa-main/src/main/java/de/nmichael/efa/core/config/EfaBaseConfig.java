...
public class EfaBaseConfig {
  ...
  public EfaBaseConfig(String dir) {
    ...
    if (Logger.isTraceOn(Logger.TT_CORE, 9) || Logger.isDebugLoggingActivatedByCommandLine()) {
      Logger.log(Logger.DEBUG, Logger.MSG_CORE_STARTUPINITIALIZATION, "EfaBaseConfig(): filename="
          + filename);
    }
    reset();
  }
  ...
  public boolean trySetUserDir(String dir, boolean createDir) {
    ...
    if (Logger.isTraceOn(Logger.TT_CORE, 9) || Logger.isDebugLoggingActivatedByCommandLine()) {
      Logger.log(Logger.DEBUG, Logger.MSG_CORE_STARTUPINITIALIZATION,
          "trySetUserDir(): efa.dir.user=" + efaUserDirectory);
    }
    if (efaCanWrite(dir, createDir)) {
      efaUserDirectory = dir;
      if (Logger.isTraceOn(Logger.TT_CORE, 9) || Logger.isDebugLoggingActivatedByCommandLine()) {
        Logger.log(Logger.DEBUG, Logger.MSG_CORE_STARTUPINITIALIZATION,
            "trySetUserDir(): can write, setting efa.dir.user=" + efaUserDirectory);
      }
      return true;
    }
    if (Logger.isTraceOn(Logger.TT_CORE, 9) || Logger.isDebugLoggingActivatedByCommandLine()) {
      Logger.log(Logger.DEBUG, Logger.MSG_CORE_STARTUPINITIALIZATION,
          "trySetUserDir(): cannot write: " + dir);
    }
    return false;
  }
