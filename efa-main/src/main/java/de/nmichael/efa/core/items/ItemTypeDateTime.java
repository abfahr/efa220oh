...
public class ItemTypeDateTime extends ItemTypeLabelTextfield {
  ...
  public ItemTypeDateTime(String name, DataTypeDate dateValue, DataTypeTime timeValue, int type,
    ...
    this.referenceDate = (dateValue != null && dateValue.isSet() ? new DataTypeDate(dateValue)
    : DataTypeDate.today());
    this.referenceTime = (timeValue != null && timeValue.isSet() ? new DataTypeTime(timeValue)
    : DataTypeTime.now());
    this.type = type;
  ...
  public IItemType copyOf() {
    return new ItemTypeDateTime(name, new DataTypeDate(dateValue), new DataTypeTime(timeValue),
  ...
  protected void field_focusLost(FocusEvent e) {
    ...
    if (!isSet() && isNotNullSet()) {
      parseValue((referenceDate.isSet() ? referenceDate.toString() : DataTypeDate.today()
          .toString()) + " " +
  ...
  public boolean isValidInput() {
    if (mustBeBefore != null
        ...
        (
            ...
            &&
            !(dateValue.equals(mustBeBefore.dateValue) && timeValue
                .isBefore(mustBeBefore.timeValue)) &&
                !(dateValue.equals(mustBeBefore.dateValue) && timeValue.equals(mustBeBefore.timeValue) && mustBeCanBeEqual)
            )) {
      lastInvalidError = International.getMessage("Der Zeitpunkt muß vor {timestamp} liegen.",
          mustBeBefore.toString());
      return false;
    }
    if (mustBeAfter != null
        && isSet()
      ...
      lastInvalidError = International.getMessage("Der Zeitpunkt muß nach {timestamp} liegen.",
          mustBeAfter.toString());
      return false;
    }
    if (isNotNullSet()) {
      if (!isSet()) {
        lastInvalidError = International.getMessage("Das Feld '{field}' darf nicht leer sein.",
            getDescription());
        return false;
      }
    }
