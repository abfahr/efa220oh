...
public class ItemTypeFile extends ItemTypeString {
  ...
  public IItemType copyOf() {
    return new ItemTypeFile(name, value, fileItem, fileTypes, fileExtensions, fileOpenSave,
        fileOrDir, type, category, description);
  }
    ...
    if (fileOpenSave == MODE_OPEN && filename.length() > 0) {
      ...
      if (fileOrDir == TYPE_DIR && !(new File(filename)).isDirectory()) {
        Dialog.error(International.getMessage(
            "{directorydescription} '{directoryname}' existiert nicht",
