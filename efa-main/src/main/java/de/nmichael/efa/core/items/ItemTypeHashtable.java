...
public class ItemTypeHashtable<E> extends ItemType {
  ...
  private void addToHash(Hashtable<String, E> hash, String key, String val) {
    ...
    if (!matchingTypeFound) {
      ...
      Logger.log(
          Logger.ERROR,
          Logger.MSG_CORE_UNSUPPORTEDDATATYPE,
          "ConfigTypesHashtable: unsupported value type for key " + key + ": "
              + c.getCanonicalName());
    }
  }
  ...
  public String toString() {
    ...
    for (String key2 : keys) {
      E value = hash.get(key2);
      if (key2.equals(DUMMY)) {
        continue;
      }
      try {
        String key = Base64.encodeBytes(key2.getBytes(Daten.ENCODING_UTF));
        ...
        Logger.log(
            Logger.ERROR,
            Logger.MSG_CORE_DATATYPEINVALIDVALUE,
            "ConfigTypesHashtable: cannot create string for value '" + key2 + "': "
                + e.toString());
      }
    }
    return s;
  ...
  public int displayOnGui(Window dlg, JPanel panel, int x, int y) {
    ...
    panel.add(titlelabel, new GridBagConstraints(x, y, 2, 1, 0.0, 0.0,
        GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(padYbefore, padXbefore,
            (keys.length == 0 ? padYafter : 0), 0), 0, 0));
    if (allowedAdd) {
      ...
      panel.add(addButton, new GridBagConstraints(x + 2, y, 2, 1, 0.0, 0.0,
          GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(padYbefore, 0,
              (keys.length == 0 ? padYafter : 0), padXafter), 0, 0));
    }
    ...
    for (int i = 0; i < keys.length; i++) {
      ...
      panel.add(label, new GridBagConstraints(x, y + i + 1, 1, 1, 0.0, 0.0,
          GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, padXbefore,
              (i + 1 == keys.length ? padYafter : 0), 0), 0, 0));
  ...
  public void setVisible(boolean visible) {
    ...
    for (JTextField element : textfield) {
