...
public class ItemTypeItemList extends ItemType {
  ...
  public IItemType copyOf() {
    ItemTypeItemList copy = new ItemTypeItemList(name, (Vector<IItemType[]>) items.clone(),
        itemFactory, type, category, description);
  ...
  public int displayOnGui(Window dlg, JPanel panel, int x, int y) {
    ...
    panel.add(titlelabel,
        new GridBagConstraints(x, y, 2, 1, 0.0, 0.0,
            GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(padYbefore, padXbefore,
                (items.size() == 0 ? padYafter : 0), 0), 0, 0));
    panel.add(addButton, new GridBagConstraints(x + xForAddDelButtons, y, 2, 1, 0.0, 0.0,
        GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(padYbefore, 2,
            (items.size() == 0 ? padYafter : 0), padXafter), 0, 0));
    myY++;
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      if (label != null) {
        panel.add(label, new GridBagConstraints(x, y + myY, 2, 1, 0.0, 0.0,
            GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(padYbetween,
                padXbefore, 0, 0), 0, 0));
      }
      panel
      .add(delButton, new GridBagConstraints(x + xForAddDelButtons, y + myY, 1, 1, 0.0, 0.0,
          GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(padYbetween, 2, 0, 0),
          0, 0));
