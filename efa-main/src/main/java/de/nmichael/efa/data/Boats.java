...
public class Boats extends StorageObject {
  ...
  public Boats(int storageType,
    ...
    super(storageType, storageLocation, storageUsername, storagePassword, storageObjectName,
        DATATYPE, International.getString("Boote"));
  ...
  public BoatRecord getBoat(String boatName, long validAt) {
    try {
      DataKey[] keys = data().getByFields(
          ...
          staticBoatRecord.getQualifiedNameValues(boatName), validAt);
      if (keys == null || keys.length < 1) {
        return null;
      }
      for (DataKey key : keys) {
        BoatRecord r = (BoatRecord) data().get(key);
  ...
  public BoatRecord getBoat(String boatName, long validFrom, long validUntil,
    ...
    try {
      DataKey[] keys = data().getByFields(
          staticBoatRecord.getQualifiedNameFields(),
          staticBoatRecord.getQualifiedNameValues(boatName));
      if (keys == null || keys.length < 1) {
        return null;
      }
  ...
  // the idea of this function is to provide any boat record for boatId, if it doesn't matter which
  // version we get
  public BoatRecord getAnyBoatRecord(UUID boatId) {
    try {
      DataKey k = BoatRecord.getKey(boatId, -1);
      // first try to find the currently valid record (this is usually a fast operation, especially
      // for remote access)
  ...
  public void preModifyRecordCallback(DataRecord record, boolean add, boolean update,
    ...
    if (delete) {
      assertNotReferenced(record, getProject().getBoatDamages(false),
          new String[] { BoatDamageRecord.BOATID });
      assertNotReferenced(record, getProject().getBoatReservations(false),
          new String[] { BoatReservationRecord.BOATID });
      assertNotReferenced(record, getProject().getBoatStatus(false),
          new String[] { BoatStatusRecord.BOATID });
      ...
      for (int i = 0; logbooks != null && i < logbooks.length; i++) {
        assertNotReferenced(record, getProject().getLogbook(logbooks[i], false),
            new String[] { LogbookRecord.BOATID });
      }
    }
  }
  ...
  class MergeBoatsProgressTask extends ProgressTask {
    ...
    public void run() {
      ...
      try {
        ...
        for (UUID mergeID : mergeIDs) {
          DataRecord[] mergeBoat = boats.data().getValidAny(BoatRecord.getKey(mergeID, 0));
