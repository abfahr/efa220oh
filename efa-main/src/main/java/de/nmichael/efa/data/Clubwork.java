...
public class Clubwork extends StorageObject {
  ...
  public void doCarryOver(JDialog parent) {
    ...
    String message = International
        .getString("Möchtest du den Übertrag für das letzte Vereinsarbeitsbuch erstellen und in das aktuell offene "
  ...
  public void doCarryOver(int thisOrNext, JDialog parent) {
    ...
    for (String name2 : names) {
      ...
      Clubwork clubwork = Daten.project.getClubwork(name2, false);
      if (thisOrNext == 0) {
    ...
    if (thisOrNext == 0) {
      ...
      if (sourceOrTarget == null) {
        int res = Dialog.yesNoDialog("Hinweis", International
            .getString("Kein kommendes Vereinsarbeitsbuch gefunden. Soll eines erstellt "
                + "werden?"));
        if (res == Dialog.YES) {
  ...
  public void doCarryOver(Clubwork from, Clubwork to) {
    ...
    if (pr == null) {
      Dialog.error(International.getMessage("Kein Vereinsarbeitsbuch '{name}' gefunden.",
          from.getName()));
      return;
    }
      ...
      for (DataKey k = it.getFirst(); k != null; k = it.getNext()) {
        ...
        if (r.getFlag() == ClubworkRecord.Flags.CarryOver) {
          if (deleteOldCarryOver == false) {
            int res = Dialog.yesNoDialog("Hinweis", International
                .getString("Es existieren Überträge im Ziel-Vereinsarbeitsbuch. Diese "
                    + "löschen?"));
            if (res == Dialog.YES) {
      ...
      if (entries.size() > 0) {
        ...
        for (Map.Entry<UUID, Double> entry : entries) {
          ...
          if (personId != null && persons != null) {
            PersonRecord[] personRecords = persons.getPersons(personId, from.getStartDate()
                .getTimestamp(null),
            ...
            if (personRecords != null && personRecords.length > 0) {
              for (PersonRecord personRecord : personRecords) {
                ...
                month = personMemberMonthToFullYear(personRecord, month, from);
                // }
              }
            }
          }
        ...
        if (successSaved > 0) {
          Dialog.infoDialog(International.getMessage("{thing} erfolgreich berechnet.",
              International.getString("Übertrag")));
          efaBoathouseFrame.openClubwork(to.getName());
        } else {
          Dialog.error(International.getMessage("{thing} konnte nicht berechnet werden!",
              International.getString("Übertrag")));
        }
      } else {
        Dialog.error(International.getMessage("Keine Einträge im aktuellen {book} gefunden!",
            International.getString("Vereinsarbeitsbuch")));
      }

      // Save Yearly Credit
      /*
       * Vector<PersonRecord> persons = Daten.project.getPersons(false).getAllPersons(-1, false,
       * false); Iterator<PersonRecord> personItr = persons.iterator(); while (personItr.hasNext())
       * { PersonRecord person = personItr.next(); ClubworkRecord record =
       * clubwork.createClubworkRecord(UUID.randomUUID()); record.setPersonId(person.getId());
       * record.setWorkDate(DataTypeDate.today());
       * record.setDescription(International.getString("Gutschrift (jährlich)"));
       * record.setHours(person.getYearlyClubworkCredit()); try { clubwork.data().add(record); }
       * catch (Exception eignore) { Logger.logdebug(eignore); } }
       */
    } catch (Exception e) {
      String message = International
          .getString("efa hat soeben versucht den Übertrag für die Vereinsarbeit zu berechnen.")
          + "\n"
  ...
  private static Integer personMemberMonthToFullYear(PersonRecord person, int month,
      Clubwork fromClubwork) {
    long fromLong = person.getValidFrom();
    long toLong = person.getInvalidFrom();

    if (fromLong > 0) {
      DataTypeDate from = new DataTypeDate(fromLong);
      if (fromClubwork.getStartDate().isBeforeOrEqual(from)
          && fromClubwork.getEndDate().isAfter(from)) {
        month -= from.getMonth();
      }
    }
    if (toLong > 0) {
