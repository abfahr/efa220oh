...
public class Fahrtenabzeichen extends StorageObject {
  ...
  public Fahrtenabzeichen(int storageType,
    ...
    super(storageType, storageLocation, storageUsername, storagePassword, storageObjectName,
        DATATYPE, International.onlyFor("Fahrtenabzeichen", "de"));
  ...
  public static boolean isNameInFahrtenabzeichenEqualPersonRecord(String firstName,
      String lastName, PersonRecord p) {
    ...
    try {
      ...
      while (k != null) {
        ...
        if (fa != null && fa.getPersonId() != null) {
          ...
          DataRecord[] records = getProject().getPersons(false).data()
              .getValidAny(PersonRecord.getKey(id, -1));
    ...
    if (qnr != null && wett != null) {
      if (qnr.length == 1) {
        ...
        switch (Dialog.auswahlDialog("Bestätigungsdateien herunterladen",
            "Es wurden Bestätigungsdateien der Jahre " + firstYear + " - " + lastYear
            + " gefunden.",
            "alle Bestätigungsdateien herunterladen", "nur neueste Bestätiungsdatei herunterladen",
            true)) {
              case 0:
  ...
  private boolean downloadFahrtenhefte(String qnr) {
    ...
    String request = EfaWettClient.makeScriptRequestString(EfaWettClient.VERBAND_DRV,
        EfaWettClient.ACTION_ABRUFEN, (qnr != null ? "qnr=" + qnr : null), null);
    if (request == null) {
    ...
    if (!DownloadThread.getFile(Dialog.frameCurrent(), request, localFile, true)) {
      Dialog.error("Die Bestätigungsdatei konnte nicht heruntergeladen werden!");
      return false;
    }
    try {
      BufferedReader f = new BufferedReader(new InputStreamReader(new FileInputStream(localFile),
          Daten.ENCODING_ISO));
      String s = f.readLine();
      ...
      Dialog.error("Die heruntergeladene Bestätigungsdatei konnte nicht geöffnet werden: "
          + e.toString());
      return false;
    }
  ...
  private boolean fahrtenhefteEinspielen(String localFile) {
    try {
      ...
      if (Daten.keyStore != null && sigfile.keyName != null && sigfile.keyDataBase64 != null) {
        if (Daten.keyStore.getCertificate(sigfile.keyName) == null) {
          ...
          try {
            ...
            Dialog.error("Fehler beim Importieren des Schlüssels " + sigfile.keyName + ": "
                + ee.toString());
          }
        }
      }
      ...
      for (int i = 0; i < fahrtenhefte.size(); i++) {
        DRVSignatur sig = fahrtenhefte.get(i);
      ...
      if (sigError == 0) {
        ...
        sigGueltigInfo = sigErrorUnknownKey
            + " Fahrtenhefte konnten nicht geprüft werden, da der Schlüssel unbekannt ist";
      ...
      meldungEingespielteFahrtenhefte += (meldungEingespielteFahrtenhefte.length() > 0 ? "\n" : "")
          + "Quittungsnummer " + sigfile.quittungsnr + ": " + fahrtenhefte.size()
          + " signierte Fahrtenhefte eingespielt (" + sigGueltigInfo + ").";
