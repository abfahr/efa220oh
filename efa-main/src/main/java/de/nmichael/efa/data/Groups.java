...
public class Groups extends StorageObject {
  ...
  public Groups(int storageType,
    ...
    super(storageType, storageLocation, storageUsername, storagePassword, storageObjectName,
        DATATYPE, International.getString("Gruppen"));
  ...
  private GroupRecord[] getGroupsForPerson(UUID personId, long validAt, long validFrom,
      long validUntil) {
  ...
  public GroupRecord findGroupRecord(String groupName, long validAt) {
    try {
      DataKey[] keys = data().getByFields(
          staticGroupRecord.getQualifiedNameFields(),
          staticGroupRecord.getQualifiedNameValues(groupName), validAt);
      if (keys == null || keys.length < 1) {
        return null;
      }
      for (DataKey key : keys) {
        GroupRecord r = (GroupRecord) data().get(key);
