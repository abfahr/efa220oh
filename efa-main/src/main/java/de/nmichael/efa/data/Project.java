...
public class Project extends StorageObject {
  ...
  public static boolean openProject(Project p, String projectName, boolean runAudit, boolean silent) {
    ...
    try {
      ...
      if (Logger.isTraceOn(Logger.TT_CORE, 3)) {
        Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_DATA, "Project Description " + projectName
            + " opened.");
      }
      p.convertInternal();
      Daten.project = p;
      ...
      if (!silent) {
        Logger.log(ee);
        Dialog.error(LogString.fileOpenFailed(projectName, International.getString("Projekt"),
            ee.toString()));
      }
      Daten.project = null;
      return false;
    ...
    try {
      ...
      if (Logger.isTraceOn(Logger.TT_CORE, 3)) {
        Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_DATA,
            "Deletion of empty Boathouse Records failed.");
      }
      Logger.logdebug(e);
    }
    ...
    try {
      if (isOpen()) {
        if (getNumberOfBoathouses() == 0) {
          ...
          ProjectRecord oldProjectRecord = getRecord(ProjectRecord.getDataKey(
              ProjectRecord.TYPE_PROJECT, null));
          ProjectRecord oldClubRecord = getRecord(ProjectRecord.getDataKey(ProjectRecord.TYPE_CLUB,
              null));
          ...
          if (oldProjectRecord != null && oldConfigRecord != null) {
            ProjectRecord boathouseRecord = createNewBoathouseRecord(International
                .getString("Bootshaus"));
            String s;
  ...
  public boolean deleteProject() {
    ...
    try {
      ...
      if (projectDir == null || (new File(projectDir)).exists()) {
        ...
        .error(International
            .getMessage(
                "Das Projekt konnte nicht vollständig gelöscht werden. Es befinden sich noch Daten in {directory}.",
                projectDir));
      }
    } catch (Exception e) {
      ...
      Logger.log(e);
      Dialog.error(LogString.fileDeletionFailed(projectName, International.getString("Projekt"),
          e.toString()));
      return false;
    } finally {
      Logger.setLogExceptions(true);
  ...
  public boolean deleteLogbookRecord(String logbookName) {
    try {
      getMyDataAccess(ProjectRecord.TYPE_LOGBOOK).delete(
          createProjectRecord(ProjectRecord.TYPE_LOGBOOK, logbookName).getKey());
    } catch (Exception e) {
      Logger.logdebug(e);
  ...
  public boolean deleteClubworkBook(String clubworkName) {
    try {
      getMyDataAccess(ProjectRecord.TYPE_CLUBWORK).delete(
          createProjectRecord(ProjectRecord.TYPE_CLUBWORK, clubworkName).getKey());
    } catch (Exception e) {
      Logger.logdebug(e);
      if (e instanceof EfaModifyException) {
        ((EfaModifyException) e).displayMessage();
  ...
  public void addRecord(ProjectRecord rec, final String type) throws EfaException {
    if (!rec.getType().equals(type)) {
      throw new EfaException(Logger.MSG_DATA_GENERICEXCEPTION,
          dataAccess.getUID() + ": Attempt to add a Record as a " + type
          + " Record which is not a " + type + " Record", Thread.currentThread()
          .getStackTrace());
    }
  ...
  private StorageObject getPersistence(Class c, String storageObjectName, String storageObjectType,
    ...
    return getPersistence(c, storageObjectName, storageObjectType, createNewIfDoesntExist,
        description, false);
  }

  private synchronized StorageObject getPersistence(Class c, String storageObjectName,
    ...
    try {
      ...
      // we only have to do this in the slow path (usually when a new persistence object is created
      ...
      if (!silent) {
        ...
        .log(
            ...
            LogString.fileOpenFailed((p != null ? p.toString() : "<?>"), description,
                e.toString()));
        if (getProjectStorageType() != IDataAccess.TYPE_EFA_REMOTE) {
          Logger.log(e);
        }
  ...
  public StorageObject getStorageObject(String storageObjectName, String storageObjectType,
    ...
    return this.getPersistence(null, storageObjectName, storageObjectType, createNewIfDoesntExist,
        "Remote Request", true);
  }
  ...
  public AutoIncrement getAutoIncrement(boolean createNewIfDoesntExist) {
    return (AutoIncrement) getPersistence(AutoIncrement.class, STORAGEOBJECT_AUTOINCREMENT,
  ...
  public SessionGroups getSessionGroups(boolean createNewIfDoesntExist) {
    return (SessionGroups) getPersistence(SessionGroups.class, STORAGEOBJECT_SESSIONGROUPS,
        SessionGroups.DATATYPE,
  ...
  public Fahrtenabzeichen getFahrtenabzeichen(boolean createNewIfDoesntExist) {
    return (Fahrtenabzeichen) getPersistence(Fahrtenabzeichen.class,
        STORAGEOBJECT_FAHRTENABZEICHEN, Fahrtenabzeichen.DATATYPE,
  ...
  public BoatStatus getBoatStatus(boolean createNewIfDoesntExist) {
    return (BoatStatus) getPersistence(BoatStatus.class, STORAGEOBJECT_BOATSTATUS,
        BoatStatus.DATATYPE,
  ...
  public BoatReservations getBoatReservations(boolean createNewIfDoesntExist) {
    return (BoatReservations) getPersistence(BoatReservations.class,
        STORAGEOBJECT_BOATRESERVATIONS, BoatReservations.DATATYPE,
  ...
  public BoatDamages getBoatDamages(boolean createNewIfDoesntExist) {
    return (BoatDamages) getPersistence(BoatDamages.class, STORAGEOBJECT_BOATDAMAGES,
        BoatDamages.DATATYPE,
  ...
  public Destinations getDestinations(boolean createNewIfDoesntExist) {
    return (Destinations) getPersistence(Destinations.class, STORAGEOBJECT_DESTINATIONS,
  ...
  public Statistics getStatistics(boolean createNewIfDoesntExist) {
    return (Statistics) getPersistence(Statistics.class, STORAGEOBJECT_STATISTICS,
        Statistics.DATATYPE,
  ...
  public void setProjectStorageLocation(String storageLocation) {
    if (getProjectStorageType() == IDataAccess.TYPE_FILE_XML) {
      // for file-based projects: storageLocation of content is always relative to this project
      // file!
  ...
  public String getProjectStorageLocation() {
    ...
    if (getProjectStorageType() == IDataAccess.TYPE_EFA_REMOTE
      ...
      String location = EfaOnlineClient.getRemoteAddress(getProjectEfaOnlineUsername(),
          getProjectEfaOnlinePassword());
      if (location != null) {
        return location;
      }
    }
      ...
      if (((ProjectRecord) record).getType().equals(ProjectRecord.TYPE_LOGBOOK)) {
        ...
        if (lName != null && lName.length() > 0 && r.getName().equals(lName)) {
          throw new EfaModifyException(
              ...
              International
              .getMessage(
                  "Der Datensatz kann nicht gelöscht werden, da er noch von {listtype} '{record}' genutzt wird.",
      ...
      if (((ProjectRecord) record).getType().equals(ProjectRecord.TYPE_BOATHOUSE)) {
        if (getNumberOfBoathouses(true) == 1) {
          throw new EfaModifyException(
              ...
              International
              .getMessage(
                  "Eintrag kann nicht gelöscht werden, da mindestens {count} Einträge verbleiben müssen.",
                  1),
                  Thread.currentThread().getStackTrace());
        }
        ...
        try {
          ...
          DataKey[] k = dest.data().getByFields(
              new String[] { DestinationRecord.ONLYINBOATHOUSEID },
          ...
          if (k != null && k.length > 0) {
            ...
            throw new EfaModifyException(
                ...
                .getMessage(
                    "Der Datensatz kann nicht gelöscht werden, da er noch von {listtype} '{record}' genutzt wird.",
                    International.getString("Ziel"), (dr != null ? dr.getQualifiedName()
                        : "<unknown>")),
                        Thread.currentThread().getStackTrace());
          }
          ...
          if (k != null && k.length > 0) {
            ...
            throw new EfaModifyException(
                ...
                .getMessage(
                    "Der Datensatz kann nicht gelöscht werden, da er noch von {listtype} '{record}' genutzt wird.",
                    International.getString("Bootsstatus"), (br != null ? br.getQualifiedName()
                        : "<unknown>")),
                        Thread.currentThread().getStackTrace());
          }
