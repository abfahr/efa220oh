...
public class ProjectRecord extends DataRecord {
  ...
  public static final String CURRENTLOGBOOKEFABASE = "CurrentLogbookEfaBase"; // previous
  // ProjectRecord
  public static final String CURRENTLOGBOOKEFABOATHOUSE = "CurrentLogbookEfaBoathouse"; // previous
  // ProjectRecord
  ...
  public static final String CURRENTCLUBWORKEFABASE = "CurrentClubworkEfaBase"; // previous
  // ProjectRecord
  public static final String CURRENTCLUBWORKEFABOATHOUSE = "CurrentClubworkEfaBoathouse"; // previous
  // ProjectRecord
  ...
  public double getDefaultMonthlyClubworkTargetHours() {
    return this.getDefaultClubworkTargetHours()
        / this.getStartDate().getMonthsDifference(this.getEndDate());
  }
  ...
  public Vector<IItemType> getGuiItems(AdminRecord admin, int subtype, String usecategory,
      boolean newProject) {
    IItemType item;
    ...
    if (getType().equals(TYPE_PROJECT)) {
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 1) {
        ...
        ((ItemTypeString) item)
        .setAllowedCharacters("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_");
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 3) {
        ...
        if (getStorageType() == IDataAccess.TYPE_EFA_REMOTE) {
          ...
          v.add(item = new ItemTypeString(ProjectRecord.EFAONLINEUSERNAME, getEfaOnlineUsername(),
              ...
              Daten.EFA_ONLINE + " - "
                  + International.getString("Benutzername")));
          v.add(item = new ItemTypePassword(ProjectRecord.EFAONLINEPASSWORD,
              getEfaOnlinePassword(), true,
              IItemType.TYPE_PUBLIC, category,
              Daten.EFA_ONLINE + " - "
                  + International.getString("Paßwort")));
        }
    ...
    if (getType().equals(TYPE_CLUB)) {
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 2 || subtype == GUIITEMS_SUBTYPE_EFAWETT) {
        ...
        v.add(item = new ItemTypeString(ProjectRecord.ASSOCIATIONGLOBALNAME,
            getGlobalAssociationName(),
            IItemType.TYPE_PUBLIC, category,
            International.getString("Dachverband") + " - "
                + International.getString("Name")));
        v.add(item = new ItemTypeString(ProjectRecord.ASSOCIATIONGLOBALMEMBERNO,
            getGlobalAssociationMemberNo(),
            IItemType.TYPE_PUBLIC, category,
            International.getString("Dachverband") + " - "
                + International.getString("Mitgliedsnummer")));
        v.add(item = new ItemTypeString(ProjectRecord.ASSOCIATIONGLOBALLOGIN,
            getGlobalAssociationLogin(),
            IItemType.TYPE_PUBLIC, category,
            International.getString("Dachverband") + " - "
                + International.getString("Benutzername")
                + (Daten.efaConfig.getValueUseFunctionalityRowingGermany() ? " (efaWett)" : "")));
        v.add(item = new ItemTypeString(ProjectRecord.ASSOCIATIONREGIONALNAME,
            getRegionalAssociationName(),
            IItemType.TYPE_PUBLIC, category,
            International.getString("Regionalverband") + " - "
                + International.getString("Name")));
        v.add(item = new ItemTypeString(ProjectRecord.ASSOCIATIONREGIONALMEMBERNO,
            getRegionalAssociationMemberNo(),
            IItemType.TYPE_PUBLIC, category,
            International.getString("Regionalverband") + " - "
                + International.getString("Mitgliedsnummer")));
        v.add(item = new ItemTypeString(ProjectRecord.ASSOCIATIONREGIONALLOGIN,
            getRegionalAssociationLogin(),
            IItemType.TYPE_PUBLIC, category,
            International.getString("Regionalverband") + " - "
                + International.getString("Benutzername")
                + (Daten.efaConfig.getValueUseFunctionalityRowingGermany() ? " (efaWett)" : "")));
        if (Daten.efaConfig.getValueUseFunctionalityRowingGermany()) {
          ...
          v.add(item = new ItemTypeBoolean(ProjectRecord.MEMBEROFADH, getMemberOfADH(),
              ...
              International.onlyFor(
                  "Mitglied im Allgemeinen Deutschen Hochschulsportverband (ADH)", "de")));
        }
      }
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 2 || subtype == GUIITEMS_SUBTYPE_KANUEFB) {
        if (Daten.efaConfig.getValueUseFunctionalityCanoeingGermany()
            || subtype == GUIITEMS_SUBTYPE_KANUEFB) {
          if (usecategory == null) {
            category = "%02C%" + International.getString("Verbände");
          }
          ...
          v.add(item = new ItemTypeLong(ProjectRecord.KANUEFBLASTSYNC, getKanuEfbLastSync(), 0,
              Long.MAX_VALUE,
    ...
    if (getType().equals(TYPE_BOATHOUSE)) {
      if (usecategory == null) {
        category = BaseTabbedDialog.makeCategory("%03%" + International.getString("Bootshaus"),
            getName());
      }
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 1) {
        ...
        if (!newProject) {
          ...
          v.add(item = new ItemTypeString(ProjectRecord.AUTONEWLOGBOOKNAME,
              getAutoNewLogbookName(),
              IItemType.TYPE_EXPERT, category,
              International.getString("Fahrtenbuchwechsel") + " - "
        ...
        if (((Project) getPersistence()).getNumberOfBoathouses() > 1) {
          v.add(item = new ItemTypeString(ProjectRecord.BOATHOUSE_IDENTIFIER,
              getBoathouseIdentifier(),
              IItemType.TYPE_PUBLIC, category,
    ...
    if (getType().equals(TYPE_LOGBOOK)) {
      ...
      if (usecategory == null) {
        category = BaseTabbedDialog.makeCategory("%04%" + International.getString("Fahrtenbuch"),
            getName());
      }
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 2) {
        // @todo (P4) allow to change time range of logbook (and make sure in ProjectEditDialog that
        // all sessions fit into that range)
    ...
    if (getType().equals(TYPE_CLUBWORK)) {
      ...
      if (subtype == GUIITEMS_SUBTYPE_ALL || subtype == 3) {
        v.add(item = new ItemTypeDouble(ProjectRecord.DEFAULTCLUBWORKTARGETHOURS,
            getDefaultClubworkTargetHours(), 0, ItemTypeDouble.MAX,
            IItemType.TYPE_PUBLIC, category,
            International.getString("Sollstunden für Vereinsarbeit")));

        v.add(item = new ItemTypeDouble(ProjectRecord.TRANSFERABLECLUBWORKHOURS,
            getTransferableClubworkHours(), 0, ItemTypeDouble.MAX,
            IItemType.TYPE_PUBLIC, category,
            International.getString("Übertragbare Vereinsarbeitsstunden pro Zeitraum")));

        v.add(item = new ItemTypeDouble(ProjectRecord.FINEFORTOOLITTLECLUBWORK,
            getFineForTooLittleClubwork(), 0, ItemTypeDouble.MAX,
            IItemType.TYPE_PUBLIC, category,
