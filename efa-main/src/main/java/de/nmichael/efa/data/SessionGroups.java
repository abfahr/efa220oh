...
public class SessionGroups extends StorageObject {
  ...
  public SessionGroups(int storageType,
    ...
    super(storageType, storageLocation, storageUsername, storagePassword, storageObjectName,
        DATATYPE, International.getString("Fahrtgruppen"));
  ...
  public void preModifyRecordCallback(DataRecord record, boolean add, boolean update, boolean delete)
      ...
      if (startDate.isSet() && endDate.isSet() && activeDays != IDataAccess.UNDEFINED_INT) {
        ...
        if (activeDays < 1 || activeDays > days) {
          throw new EfaModifyException(Logger.MSG_DATA_MODIFYEXCEPTION,
              International.getMessage("Das Feld '{field}' hat einen ungültigen Wert.",
                  SessionGroupRecord.ACTIVEDAYS),
                  Thread.currentThread().getStackTrace());
        }
      }

      // check whether all referencing logbook records fit into this range
      Vector<LogbookRecord> logbookRecords = ((SessionGroupRecord) record)
          .getAllReferencingLogbookRecords();
    ...
    if (delete) {
      assertNotReferenced(record,
          getProject().getLogbook(((SessionGroupRecord) record).getLogbook(), false),
