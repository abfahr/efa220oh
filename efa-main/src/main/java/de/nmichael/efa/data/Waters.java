...
public class Waters extends StorageObject {
  ...
  public Waters(int storageType,
    ...
    super(storageType, storageLocation, storageUsername, storagePassword, storageObjectName,
        DATATYPE, International.getString("Gewässer"));
  ...
  public WatersRecord findWatersByName(String name) {
    try {
      DataKey[] keys = data()
          .getByFields(new String[] { WatersRecord.NAME }, new String[] { name });
  ...
  public void preModifyRecordCallback(DataRecord record, boolean add, boolean update, boolean delete)
    ...
    if (delete) {
      assertNotReferenced(record, getProject().getDestinations(false),
          new String[] { DestinationRecord.WATERSIDLIST });
    }
  }
