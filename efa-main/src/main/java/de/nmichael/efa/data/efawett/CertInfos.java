...
public class CertInfos {
  ...
  private static String getFingerprint(byte[] certBytes, String algorithm) throws Exception {
    ...
    for (int i = 0; i < fp.length; i++) {
      s += EfaUtil.hexByte(fp[i] & 0xff) + (i + 1 < fp.length ? ":" : "");
    }
    return s;
  }
  ...
  public static String getValidityYears(X509Certificate cert) {
    ...
    if (tmj.monat <= 0) {
      ...
      return Integer.toString(tmj.monat);
    } else {
      return tmj.monat + " - " + tmj.jahr;
    }
  }
  ...
  public static boolean sigKeyValidForYear(X509Certificate cert, int jahr) {
    ...
    if (tmj.jahr == 0) {
      ...
      if (jahr < tmj.monat || jahr > tmj.jahr) {
        return false;
      }
    }
    if (isRevokedForYear(cert, jahr)) {
      return false;
    }
    return true;
  }

  // prüft, ob der Schlüssel, der durch das Zertifikat cert angegeben ist, zum Signierzeitpunkt
  // gesperrt ist
  public static boolean isRevokedForDate(X509Certificate cert, Date date) {
    return false;
  }

  // prüft, ob der Schlüssel, der durch das Zertifikat cert angegeben ist, für das Jahr this.jahr
  // gesperrt ist
  ...
  public static boolean sigKeyValidOnDate(X509Certificate cert, Date sigDate) {
    ...
    if (sigDate.before(cert.getNotBefore()) || sigDate.after(cert.getNotAfter())) {
      return false;
    }
    if (isRevokedForDate(cert, sigDate)) {
      return false;
    }
    return true;
  }
  ...
  public static String getValiditySign(X509Certificate cert) {
    return EfaUtil.date2String(cert.getNotBefore(), false) + " bis "
        + EfaUtil.date2String(cert.getNotAfter(), false);
  }
  ...
  public static String getAliasName(X509Certificate cert) {
    ...
    if (pos > 0) {
      alias = alias.substring(pos + 2, alias.length());
    }
