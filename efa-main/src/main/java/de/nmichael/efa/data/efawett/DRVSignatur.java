...
public class DRVSignatur {
  ...
  public DRVSignatur(String fahrtenheft) {
    ...
    try {
      ...
      // Signatur ermitteln
      if (v.size() > 0) {
        this._signatur = EfaUtil.replace((String) v.get(v.size() - 1), " ", "", true); // _signatur
        // ist immer
        // das letzte
        // Element
      }
  ...
  public DRVSignatur(String teilnNr,
    ...
    if (anzAbzeichenAB > anzAbzeichen) {
      throw new Exception(
          "Anzahl der Abzeichen für Jugend-Gruppen A/B ist größer als Anzahl der Abzeichen insgesamt");
    }
    if (gesKmAB > gesKm) {
      throw new Exception(
          "Anzahl der Kilometer für Jugend-Gruppen A/B ist größer als Anzahl der Kilometer insgesamt");
    }
    if (jahr < 1900) {
          ...
          if (this.keyNr < 0) {
            ...
            if (cert == null) {
              ...
              if (!CertInfos.sigKeyValidForYear(cert, this.jahr)) {
                ...
                this.signatureError = "Schlüssel " + getKeyName() + " für Fahrtenhefte des Jahres "
                    + this.jahr + " nicht gültig";
              } else {
                ...
                if (!CertInfos.sigKeyValidOnDate(cert, getSignatureDate())) {
                  ...
                  this.signatureError = "Schlüssel " + getKeyName()
                      + " zum Signierzeitpunkt nicht gültig (" + sdate + ")";
                } else {
  ...
  private String construct_meldung(boolean forDisplay) {
    switch (this.version) {
      case 2: // Version 2
        if (forDisplay) {
          return teilnNr + "; " + vorname + "; " + nachname + "; " + jahrgang + "; " + anzAbzeichen
              + "; " + gesKm + "; " + anzAbzeichenAB + "; " + gesKmAB + "; " + jahr + "; "
              + sigDatum;
        }
        return teilnNr + ";" + vorname + ";" + nachname + ";" + jahrgang + ";" + anzAbzeichen + ";"
        + gesKm + ";" + anzAbzeichenAB + ";" + gesKmAB + ";" + jahr + ";" + sigDatum;
      case 3: // Version 3
        if (forDisplay) {
          return teilnNr + "; " + vorname + "; " + nachname + "; " + jahrgang + "; " + anzAbzeichen
              + "; " + gesKm + "; " + anzAbzeichenAB + "; " + gesKmAB + "; " + jahr + "; "
        ...
        return teilnNr + ";" + vorname + ";" + nachname + ";" + jahrgang + ";" + anzAbzeichen + ";"
        + gesKm + ";" + anzAbzeichenAB + ";" + gesKmAB + ";" + jahr + ";" + letztkm + ";"
        + sigDatum;
      default: // Unbekannte Version
        return null;
    }
  ...
  public static void main(String[] args) {
    if (args.length != 15) {
      System.out
      .println("usage: DRVSignatur <keystore-file> <password> <key-alias> <teilnnr> <vorname> <nachname> <jahrgang> <abzeichen> <km> <abzeichenAB> <kmAB> <jahr> <letztkm> <sigDatum=0> <version>");
      System.exit(1);
    }
    try {
