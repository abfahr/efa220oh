...
public class EfaWett {
  ...
  public boolean writeFile() throws IOException {
    ...
    BufferedWriter f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(datei),
        Daten.ENCODING_ISO));
    kennung = EfaWett.EFAWETT;
    if (Logger.isTraceOn(Logger.TT_STATISTICS)) {
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_EFAWETT,
          "EfaWett.writeFile(): Start writing Header and Common Data ...");
    }
    f.write(kennung + "\n");
    ...
    if (aktive_M_ab19 != null || aktive_M_bis18 != null || aktive_W_ab19 != null
        || aktive_W_bis18 != null) {
    ...
    if (Logger.isTraceOn(Logger.TT_STATISTICS)) {
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_EFAWETT,
          "EfaWett.writeFile(): Done writing Header and Common Data ...");
    }

    if (Logger.isTraceOn(Logger.TT_STATISTICS)) {
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_EFAWETT,
          "EfaWett.writeFile(): Start writing Meldungen ...");
    }
    EfaWettMeldung m = meldung;
    int c = 0;
    while (m != null) {
      f.write("\n[MELDUNG#" + (++c) + "]\n");
      if (Logger.isTraceOn(Logger.TT_STATISTICS)) {
        Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_EFAWETT, "EfaWett.writeFile(): Writing Meldung "
            + c + " == " + m.vorname + " " + m.nachname);
      }
    ...
    if (Logger.isTraceOn(Logger.TT_STATISTICS)) {
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_EFAWETT,
          "EfaWett.writeFile(): Done writing Meldungen ...");
    }
    f.close();
    if (Logger.isTraceOn(Logger.TT_STATISTICS)) {
      File ff = new File(datei);
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_EFAWETT,
          "EfaWett.writeFile(): Filesize ff.length() == " + ff.length());
  ...
  public boolean readFile() throws IOException {
    ...
    String s = f.readLine();
    if (s == null
        || (!s.startsWith(EFAWETT) && !s.startsWith(EFAWETT151) && !s.startsWith(EFAWETT150)
            && !s.startsWith(EFAWETT100) && !s.startsWith(EFAWETT091))) {
      f.close();
      return false;
    }
    kennung = s;
    ...
    String block = "";
    boolean newblock = false; // true, wenn der Block im aktuellen Schleifendurchlauf erstmals
    // gefunden wurde
