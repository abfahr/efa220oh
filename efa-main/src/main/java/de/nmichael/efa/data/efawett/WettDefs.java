...
public class WettDefs extends DatenListe {
      ...
      while ((s = freadLine()) != null) {
        ...
        if (block.equals("EFW")) {
          if (s.startsWith(EFW_LETZTE_AKTUALISIERUNG + "=")) {
            efw_letzte_aktualisierung = s.substring(EFW_LETZTE_AKTUALISIERUNG.length() + 1,
                s.length());
          }
          ...
          if (s.startsWith(EFW_DRV_FA_MELD_ERW + "=")) {
            efw_drv_fa_meld_erw = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_MELD_ERW.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_MELD_JUG + "=")) {
            efw_drv_fa_meld_jug = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_MELD_JUG.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_NADEL_ERW_SILBER + "=")) {
            efw_drv_fa_nadel_erw_silber = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_NADEL_ERW_SILBER.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_NADEL_ERW_GOLD + "=")) {
            efw_drv_fa_nadel_erw_gold = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_NADEL_ERW_GOLD.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_NADEL_JUG_SILBER + "=")) {
            efw_drv_fa_nadel_jug_silber = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_NADEL_JUG_SILBER.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_NADEL_JUG_GOLD + "=")) {
            efw_drv_fa_nadel_jug_gold = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_NADEL_JUG_GOLD.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_STOFF_ERW + "=")) {
            efw_drv_fa_stoff_erw = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_STOFF_ERW.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_DRV_FA_STOFF_JUG + "=")) {
            efw_drv_fa_stoff_jug = EfaUtil.string2int(
                s.substring(EFW_DRV_FA_STOFF_JUG.length() + 1, s.length()), 0);
          }
          ...
          if (s.startsWith(EFW_LRVBLN_SOMM_MELD_ERW + "=")) {
            efw_lrvbln_somm_meld_erw = EfaUtil.string2int(
                s.substring(EFW_LRVBLN_SOMM_MELD_ERW.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_LRVBLN_SOMM_MELD_JUG + "=")) {
            efw_lrvbln_somm_meld_jug = EfaUtil.string2int(
                s.substring(EFW_LRVBLN_SOMM_MELD_JUG.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_LRVBLN_WINT_MELD_ERW + "=")) {
            efw_lrvbln_wint_meld_erw = EfaUtil.string2int(
                s.substring(EFW_LRVBLN_WINT_MELD_ERW.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_LRVBLN_WINT_MELD_JUG + "=")) {
            efw_lrvbln_wint_meld_jug = EfaUtil.string2int(
                s.substring(EFW_LRVBLN_WINT_MELD_JUG.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_LRVBLN_BLWI_MELD_ERW + "=")) {
            efw_lrvbln_blwi_meld_erw = EfaUtil.string2int(
                s.substring(EFW_LRVBLN_BLWI_MELD_ERW.length() + 1, s.length()), 0);
          }
          if (s.startsWith(EFW_LRVBLN_BLWI_MELD_JUG + "=")) {
            efw_lrvbln_blwi_meld_jug = EfaUtil.string2int(
                s.substring(EFW_LRVBLN_BLWI_MELD_JUG.length() + 1, s.length()), 0);
          }
        ...
        if (block.equals("WETTDEF")) {
          ...
          if (s.startsWith("GRUPPEN[")) {
            ...
            if (index >= 0 && field != null && field.length() > 0 && value != null
                && value.length() > 0) {
              if (gruppen == null) {
                gruppen = new Vector();
              }
  ...
  public String[] getAllWettDefKeys() {
    ...
    for (int i = 0; i < ANZWETT; i++) {
      ...
      if (w != null) {
        wett[i] = w.key; // Namen der Wettbewerbe für höchstmögliches Jahr (falls sich Namen
        // ändern)!
      } else {
        wett[i] = "";
      }
  ...
  public String[] getAllWettDefNames() {
    ...
    for (int i = 0; i < ANZWETT; i++) {
      WettDef w = getWettDef(i, 9999);
      if (w != null) {
        wett[i] = w.name; // Namen der Wettbewerbe für höchstmögliches Jahr (falls sich Namen
        // ändern)!
      } else {
  ...
  public boolean erfuelltGruppe(int wettnr, int wettJahr, int gruppe, int jahrgang, int geschlecht,
    ...
    switch (wettnr) {
      ...
      case LRVBRB_FAHRTENWETT:
        ...
        break;
      case LRVMVP_WANDERRUDERWETT: // zusatz:= Gig-Km; zusatz2:= Gig-Fahrten; zusatz3:=
        // 20Km-Fahrten; zusatz4:= 30Km-Fahrten
        if (zusatz < wett.gruppen[gruppe].zusatz || zusatz2 < wett.gruppen[gruppe].zusatz2
  ...
  public static String getDRVAbzeichen(boolean erwachsen, int abzeichen, int abzeichenAB,
      int wettJahr) {
    ...
    if (erwachsen) {
      ...
      // Ab 2008 erhalten auch Jugendliche ausschließlich Fahrtenabzeichen in Gold für *Erwachsene*
      // (s. mail vom DRV April 2008)!!
      if ((abzeichenGesamt + 1) >= 5 && (abzeichenGesamt + 1) % 5 == 0) {
        return EfaWettMeldung.getAbzeichenGold(abzeichenGesamt + 1, true); // Erw-Gold auch für
        // Jugendliche!
      }
