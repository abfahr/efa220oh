    ...
    try {
      ...
      while (d != null) {
        ...
        if (boatID != null && imported.get(boatID) == null) {
          // normally the BoatStatusRecord should already have been created when the boat was
          // created...
          ...
          try {
            ...
            if (d.get(BootStatus.LFDNR).length() > 0) {
              ...
              // However, we don't know which the current logbook in efa1 is (unless we would read
              // the efa1 config file as well).
              // So we leave this empty. If logbook is null, efa boathouse will at runtime assume
              // the then opened logbook as the one.
            }
            ...
            logError(International
                .getMessage("Import von Eintrag fehlgeschlagen: {entry} ({error})", rs.toString(),
                    e.toString()));
            Logger.logdebug(e);
          }
          ...
          for (int i = 0; reservierungen != null && i < reservierungen.size(); i++) {
            ...
            try {
              ...
              logError(International.getMessage(
                  "Import von Eintrag fehlgeschlagen: {entry} ({error})", rr.toString(),
                  e.toString()));
              Logger.logdebug(e);
            }
          }
          ...
          if (d.get(BootStatus.BOOTSSCHAEDEN).length() > 0) {
            ...
            try {
              ...
              logError(International.getMessage(
                  "Import von Eintrag fehlgeschlagen: {entry} ({error})", rd.toString(),
                  e.toString()));
              Logger.logdebug(e);
            }
          }
          ...
          if (boatID == null) {
            logWarning(International.getMessage(
                "{type_of_entry} {entry} nicht in {list} gefunden.",
