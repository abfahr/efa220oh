...
public class ImportDestinations extends ImportBase {
  ...
  private boolean isChanged(DestinationRecord r, DatenFelder d) {
    ...
    if (!isIdentical(Long.toString(r.getDistance().getValueInMeters()),
        Integer.toString(EfaUtil.zehntelString2Int(d.get(Ziele.KM)) * 100))) {
      return true;
    }
    if (!isIdentical((r.getDestinationAreas() == null ? "" : r.getDestinationAreas().toString()),
        getZielbereiche(d))) {
      return true;
    }
    if (!isIdentical(Boolean.toString(r.getStartIsBoathouse()),
        Boolean.toString(d.get(Ziele.STEGZIEL).equals("+")))) {
      return true;
    }
    // Well, we skip the comparison for Waters. Actually, we should check this, but I currently just
    // don't feel like implementing this ;-)
    return false;
  }
    ...
    try {
      ...
      while (d != null) {
        ...
        if (r == null || isChanged(r, d)) {
          ...
          r.setDistance(DataTypeDistance.parseDistance(d.get(Ziele.KM)
          ...
          if (d.get(Ziele.STEGZIEL).equals("+")) {
            ...
            r.setRoundtrip(true); // we only assume destinations where
            // "Start und Ziel ist Bootshaus" to be roundtrips (we don't have
            // this info in efa1, so we just guess)
          }
      ...
      // mark all destinations that have *not* been imported with this run, but still have a valid
      // version, as deleted
      ...
      while (key != null) {
        ...
        if (dr != null && importedDestinations.get(dr.getId()) == null) {
          try {
            ...
            logError(International.getMessage(
                "Gültigkeit ändern von Eintrag fehlgeschlagen: {entry} ({error})", dr.toString(),
                e.toString()));
            Logger.logdebug(e);
          }
        }
      ...
      logError(International.getMessage("Import von {list} aus {file} ist fehlgeschlagen.",
          getDescription(), ziele.getFileName()));
      logError(e.toString());
      Logger.logdebug(e);
      return false;
    }
