...
public class ImportTask extends ProgressTask {
  ...
  private String newestLogbookName; // name of the logbook to be opened when this dialog is
  // completed
  ...
  public void run() {
    ...
    int errorCnt = 0;
    int successCnt = 0;
    String logfile = Daten.efaLogDirectory + Daten.fileSep + "import_"
        + EfaUtil.getCurrentTimeStampYYYYMMDD_HHMMSS() + ".log";
    setLogfile(logfile);
    ...
    if (errorCnt > 0) {
      msg += "\n"
          + International.getMessage(
              "Der Import von {count} Dateien wurde wegen Fehlern abgebrochen.", errorCnt);
    }
    msg += "\n"
        + International.getMessage("Es traten {count} Warnungen und {count} Fehler auf.",
