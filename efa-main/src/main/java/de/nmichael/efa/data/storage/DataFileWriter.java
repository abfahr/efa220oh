...
public class DataFileWriter extends Thread {
  ...
  public DataFileWriter(DataFile dataFile) {
    ...
    try {
      DataFileWriter.SAVE_INTERVAL = Daten.efaConfig.getValueDataFileSaveInterval() * 1000;
  ...
  public void run() {
    if (Logger.isTraceOn(Logger.TT_FILEIO)) {
      Logger.log(Logger.DEBUG, Logger.MSG_FILE_WRITETHREAD_RUNNING, "DataFileWriter["
          + dataFile.filename + "] running.");
    }
    while (dataFile.isStorageObjectOpen()) {
      try {
        if (writedata && System.currentTimeMillis() - lastSave > SAVE_INTERVAL) {
          if (Logger.isTraceOn(Logger.TT_FILEIO)) {
            Logger.log(Logger.DEBUG, Logger.MSG_FILE_WRITETHREAD_SAVING, "DataFileWriter["
                + dataFile.filename + "] found new data to be saved.");
          }
          try {
    ...
    if (Logger.isTraceOn(Logger.TT_FILEIO)) {
      Logger.log(Logger.DEBUG, Logger.MSG_FILE_WRITETHREAD_EXIT, "DataFileWriter["
          + dataFile.filename + "] exited.");
    }
  }
  ...
  synchronized public void save(boolean synchronous, boolean dataChanged) {
    if ((synchronous || (dataChanged && !writedata)) && Logger.isTraceOn(Logger.TT_FILEIO)) {
      Logger.log(Logger.DEBUG, Logger.MSG_FILE_WRITETHREAD_SAVING,
          "DataFileWriter[" + dataFile.filename + "] new " + (dataChanged ? "save" : "flush")
          + " request queued" + (synchronous ? " (sync)" : "") + ".");
    }
    if (synchronous) {
      lastSave = 0;
    }
