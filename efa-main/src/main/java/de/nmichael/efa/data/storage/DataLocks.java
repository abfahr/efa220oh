...
public class DataLocks {
  ...
  /**
   * Lock Implementation There are two types of Locks: - Global Exclusive Locks - Local Exclusive
   * Locks Global Exclusive Locks lock the entire Storage Object for exclusive (write) access. When
   * a thread holds a Global Exclusive Lock, no other Thread will be able to acquire either a Global
   * or Local Exclusive Lock. However, read access is possible at all times. Local Exclusive Locks
   * lock a specific record identified by its key in the Storage Object for exclusive (write)
   * access. When a thread holds a Local Exclusive Lock, no other Thread will be able to acquire a
   * Local Exclusive Lock for the same record. Additionally, no other Tread will be able to aquire a
   * Global Exclusive Lock. However, read access is possible at all times, to both other records as
   * also the locked record itself. Usecase Lock Required Notes
   * ====================================================================================== Add
   * Record Local Exclusive Lock on Record will be granted if there is no Global Exclusive Lock, and
   * if no other thread attempts to add a Record with identical key information at the same time.
   * Modify Record Local Exclusive Lock on Record will be granted if there is no Global Exclusive
   * Lock, and if no other thread holds a Local Exclusive Lock on this record. Delete Record Local
   * Exclusive Lock on Record will be granted if there is no Global Exclusive Lock, and if no other
   * thread holds a Local Exclusive Lock on this record. Read Record no Lock required reads will be
   * possible at any time, but there is no level of read consistency supported.
   */
  ...
  public int clearTimeouts() {
    ...
    synchronized (locks) {
      for (DataKey dataKey : locks.keySet()) {
        DataLock lock = locks.get(dataKey);
