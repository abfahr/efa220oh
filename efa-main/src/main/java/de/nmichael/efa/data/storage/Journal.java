...
public class Journal {
    ...
    if (jnr != fwnr || fw == null) {
      ...
      try {
        if (isOpenNewJournal(scn)) {
          // open with overwrite
          fw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(journalName, false),
              Daten.ENCODING_UTF));
          ...
          // open with append
          fw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(journalName, true),
              Daten.ENCODING_UTF));
        }
        fwnr = jnr;
        fwname = journalName;
        ...
        Logger.log(
            Logger.ERROR,
            Logger.MSG_DATA_JOURNALOPENFAILED,
            LogString.fileCreationFailed(journalName, International.getString("Journal"),
                e.toString()));
        fw = null;
        fwnr = -1;
        fwname = null;
  ...
  private boolean openForRead(long jgrp) {
    ...
    try {
      ...
      fr = new BufferedReader(new InputStreamReader(new FileInputStream(journalName),
          Daten.ENCODING_UTF));
      String s;
    ...
    try {
      ...
      if (s == null || s.length() == 0) {
        Logger.log(Logger.ERROR, Logger.MSG_DATA_JOURNALWRITEFAILED,
            LogString.fileWritingFailed(fwname, International.getString("Journal"),
                "empty log string"));
        return false;
      }
      f.write(s + "\n");
  ...
  public void deleteAllJournals() throws EfaException {
    for (int i = 0; i < numberOfJournals; i++) {
      ...
      try {
        ...
        if (f.isFile()) {
          if (!f.delete()) {
            throw new Exception(LogString.fileDeletionFailed(filename,
                International.getString("Journal")));
          }
        }
      } catch (Exception e) {
        throw new EfaException(Logger.MSG_DATA_DELETEFAILED, LogString.fileDeletionFailed(filename,
            International.getString("Journal"), e.toString()), Thread.currentThread()
            .getStackTrace());
      }
    }
  }
