...
public class RemoteEfaClient extends DataAccess {
  ...
  public RemoteEfaClient(String location, String username, String password, String name,
  ...
  public String getUID() {
    return "remote:" + getStorageUsername() + "@" + getStorageLocation() + "/"
        + getStorageObjectName() + "." + getStorageObjectType();
  }
  ...
  private Vector<RemoteEfaMessage> sendRequest(Vector<RemoteEfaMessage> requests) throws Exception {
    ...
    if (this.sessionId == null
        && lastLoginFailed > 0
        &&
        System.currentTimeMillis() - lastLoginFailed < Daten.efaConfig
        .getValueDataRemoteLoginFailureRetryTime() * 1000) {
      loggedIn = false;
      return null;
    }
    ...
    if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 5)) {
      Logger.log(Logger.DEBUG, Logger.MSG_REFA_DEBUGCOMMUNICATION,
          "Sending Request [" + url.toString() + "]: " + request.toString());
    }
    if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 2)) {
      for (int i = 0; i < requests.size(); i++) {
        ...
        Logger.log(
            Logger.DEBUG,
            Logger.MSG_REFA_TRACECOMMUNICATION,
            "Snd: " +
    ...
    for (int i = 0; i < responses.size(); i++) {
      ...
      if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 2)) {
        Logger.log(Logger.DEBUG, Logger.MSG_REFA_TRACECOMMUNICATION, "Rcv: "
            + (req != null ? req.getStorageObjectName() + "." + req.getStorageObjectType() + ":"
  ...
  private Vector<RemoteEfaMessage> getResponse(URLConnection connection,
      RemoteEfaMessage.EfaMessageInputStream eis) {
      ...
      try {
        ...
        if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 9)) {
          ...
          for (String header : m.keySet()) {
            Logger.log(Logger.DEBUG, Logger.MSG_REFA_DEBUGCOMMUNICATION, "    " + header + "="
    ...
    try {
      ...
      if (responseHandler.isDocumentComplete()) {
        ...
        if (responses != null && responses.size() > 0 && responses.get(0) != null) {
          if (this.sessionId != null) {
            ...
            // we don't have a session id, so this might be the response for a login: get the
            // session id
      ...
      Logger.log(Logger.ERROR, Logger.MSG_REFA_INVALIDRESPONSE,
          "Get Response failed: " + e.toString());
      Logger.logdebug(e);
      AccessStatistics.updateStatistics(this, "All",
          AccessStatistics.COUNTER_RESPERR, 1);
      return null;
      ...
      if (response != null && response.getResultCode() == RemoteEfaMessage.RESULT_OK) {
        ...
        if (records != null) {
          for (DataRecord record : records) {
            if (record != null) {
              cache.updateCache(record, scn, totalRecordCount);
            }
          }
        } else {
  ...
  private String getErrorLogstring(String requestName, String msg, int code) {
    return getStorageObjectName()
        + "."
        + getStorageObjectType()
        + ": "
        +
        International.getMessage("efaRemote-Anfrage {request} fehlgeschlagen: {reason}",
            requestName, msg + " (Code " + code + ")");
  }
  ...
  protected int runSimpleRequest(RemoteEfaMessage request) {
    try {
      ...
      if (responses == null || responses.size() == 0 ||
        ...
        Logger.log(Logger.ERROR, Logger.MSG_REFA_UNEXPECTEDRESPONSE,
            getErrorLogstring(request, "empty response", -1));
        return -1;
      }
      if (responses.size() > 1) {
  ...
  public boolean existsStorageObject() throws EfaException {
    return runSimpleRequest(RemoteEfaMessage.createRequestData(1, getStorageObjectType(),
  ...
  public void openStorageObject() throws EfaException {
    if (runSimpleRequest(RemoteEfaMessage.createRequestData(1, getStorageObjectType(),
        getStorageObjectName(),
  ...
  public boolean isStorageObjectOpen() {
    if (lastIsOpen
        && System.currentTimeMillis() - lastIsOpenTs < Daten.efaConfig
        .getValueDataRemoteIsOpenExpiryTime() * 1000) {
      return true;
    }
    lastIsOpen = runSimpleRequest(RemoteEfaMessage.createRequestData(1, getStorageObjectType(),
        getStorageObjectName(),
  ...
  public long acquireGlobalLock() throws EfaException {
    RemoteEfaMessage response = runDataRequest(RemoteEfaMessage.createRequestData(1,
        getStorageObjectType(), getStorageObjectName(),
  ...
  public DataKey[] getAllKeys() throws EfaException {
    ...
    request.addField(RemoteEfaMessage.FIELD_PREFETCH, Boolean.toString(true)); // prefetch all
    // records as well
    RemoteEfaMessage response = runDataRequest(request);
    if (response == null || response.getResultCode() != RemoteEfaMessage.RESULT_OK) {
      throw new EfaException(Logger.MSG_REFA_REQUESTFAILED,
  ...
  public DataKey[] getByFields(String[] fieldNames, Object[] values, long validAt)
    ...
    for (int i = 0; i < fieldNames.length; i++) {
      request.addFieldArrayElement(i, fieldNames[i],
  ...
  public DataRecord getValidNearest(DataKey key, long earliestValidAt, long latestValidAt,
    ...
    /*
     * RemoteEfaMessage request = RemoteEfaMessage.createRequestData(1, getStorageObjectType(),
     * getStorageObjectName(), RemoteEfaMessage.OPERATION_GETVALIDNEAREST); request.addKey(key);
     ...
     * RemoteEfaMessage response = runDataRequest(request); if (response == null ||
     * response.getResultCode() != RemoteEfaMessage.RESULT_OK) { throw new
     * EfaException(Logger.MSG_REFA_REQUESTFAILED,
     * getErrorLogstring(RemoteEfaMessage.OPERATION_GETVALIDNEAREST, (response != null ?
     * response.getResultText() : "unknown"), (response != null ? response.getResultCode() : -1)),
     * Thread.currentThread().getStackTrace()); } return response.getRecord(0);
     */
  }
