...
public class RemoteEfaServer {
  ...
  class MyHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
      ...
      if (requestMethod.equalsIgnoreCase("GET")) {
        ...
        responseBody.write(new String("<h1 align=\"center\">" + Daten.EFA_LONGNAME + "</h1>")
        .getBytes());
        responseBody.write(new String("<h2 align=\"center\">efaRemote Server running on port "
            + serverPort).getBytes());
        responseBody.close();
      }
      if (requestMethod.equalsIgnoreCase("POST")) {
        ...
        try {
          ...
          responses.add(RemoteEfaMessage.createResponseResult(0, RemoteEfaMessage.ERROR_UNKNOWN,
              e.getMessage()));
          Logger.log(e);
        }
        StringBuilder response = new StringBuilder();
        response.append("<?xml version='1.0' encoding='" + Daten.ENCODING_UTF + "' ?><"
            + RemoteEfaParser.XML_EFA + ">");
        ...
        if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 5)) {
          Logger.log(Logger.DEBUG, Logger.MSG_REFA_DEBUGCOMMUNICATION, "Sending Response ["
              + exchange.getRemoteAddress().toString() + "]: " + response.toString());
        }
  ...
  private Vector<RemoteEfaMessage> getRequests(RemoteEfaMessage.EfaMessageInputStream eis,
    ...
    if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 5)) {
      try {
        ...
        if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 9)) {
          ...
          String s;
          while ((s = buf.readLine()) != null) {
            Logger.log(Logger.DEBUG, Logger.MSG_REFA_DEBUGCOMMUNICATION, "Got Request ["
                + peerAddress.toString() + "]:" + s);
          }
        }
        in.reset();
      } catch (Exception e) {
  ...
  private Vector<RemoteEfaMessage> handleRequests(Vector<RemoteEfaMessage> requests,
      InetSocketAddress peerAddress) {
    if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 2)) {
    ...
    try {
      for (int i = 0; i < requests.size(); i++) {
        ...
        if (request == null) {
          responses.add(RemoteEfaMessage.createResponseResult(0,
              RemoteEfaMessage.ERROR_INVALIDREQUEST, "Invalid Request: <null>"));
          break;
        }
        ...
        if (msgId < 1) {
          responses.add(RemoteEfaMessage.createResponseResult(0,
              RemoteEfaMessage.ERROR_INVALIDREQUEST, "Invalid Request ID: " + msgId));
          break;
        }
        if (operation == null) {
          responses.add(RemoteEfaMessage.createResponseResult(msgId,
              RemoteEfaMessage.ERROR_INVALIDREQUEST, "Invalid Operation: <null>"));
          break;
        }
        ...
        if (admin == null) {
          responses.add(RemoteEfaMessage.createResponseResult(msgId,
              RemoteEfaMessage.ERROR_INVALIDSESSIONID, "Invalid SessionID"));
          break;
        }
        ...
        if (storageObjectType == null || storageObjectType.length() == 0 ||
          ...
          responses.add(RemoteEfaMessage.createResponseResult(msgId,
              RemoteEfaMessage.ERROR_NOSTORAGEOBJECT, "No StorageObject specified"));
          break;
        }
        ...
        if (p == null && !RemoteCommand.DATATYPE.equals(request.getStorageObjectType())) {
          responses.add(RemoteEfaMessage.createResponseResult(msgId,
              RemoteEfaMessage.ERROR_UNKNOWNSTORAGEOBJECT,
        ...
        while (true) { // not a loop
          ...
          // unknown request
          responses.add(RemoteEfaMessage.createResponseResult(msgId,
              RemoteEfaMessage.ERROR_INVALIDREQUEST, "Unsupported Request: " + request.toString()));
          break;
        }
        ...
        if (responses.size() > 0 && p != null) {
          ...
          if (lastResponse != null && lastResponse.getResultCode() == RemoteEfaMessage.RESULT_OK) {
            ...
            lastResponse.addField(RemoteEfaMessage.FIELD_TOTALRECORDCOUNT,
                Long.toString(p.dataAccess.getNumberOfRecords()));
          }
        }
  ...
  private RemoteEfaMessage requestLogin(RemoteEfaMessage request, InetSocketAddress peerAddress) {
    ...
    if (Daten.applPID.equals(pid)) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_SELFLOGIN, "Self Login Attempt");
    }
    if (Logger.isTraceOn(Logger.TT_REMOTEEFA, 1)) {
      Logger.log(Logger.DEBUG, Logger.MSG_REFA_SERVERLOG,
          "efaRemote Login Attempt from " + peerAddress + ": Username=" + username
          + " Password=***");
    }
    if (Daten.admins != null && Daten.admins.isOpen()) {
      ...
      if (admin == null) {
        Logger.log(
            Logger.WARNING,
            Logger.MSG_REFA_SERVERLOG,
            International.getMessage("efaRemote Login von {ipaddress} fehlgeschlagen",
                peerAddress.toString())
                +
                ": " + International.getString("Admin") + "=" + username);
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.ERROR_INVALIDLOGIN, "Invalid Login");
      } else {
        if (!admin.isAllowedRemoteAccess()) {
          Logger.log(
              Logger.WARNING,
              Logger.MSG_REFA_SERVERLOG,
              International.getMessage("efaRemote Login von {ipaddress} mit ungenügenden Rechten",
                  peerAddress.toString())
                  + ": " + International.getString("Admin") + "=" + username);
          return RemoteEfaMessage.createResponseResult(request.getMsgId(),
              RemoteEfaMessage.ERROR_NOPERMISSION, "No Permission");
        }
        ...
        if (logType.equals(Logger.INFO) || Logger.isTraceOn(Logger.TT_REMOTEEFA, 2)) {
          Logger.log(
              Logger.INFO,
              Logger.MSG_REFA_SERVERLOG,
              International.getMessage("efaRemote Login von {ipaddress} erfolgreich",
                  peerAddress.toString())
                  + ": " + International.getString("Admin") + "=" + username);
        }
      ...
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNABLETOCOMPLY, "Unable to comply");
    }
  }

  // =========================== Storage Object Methods ===========================

  private RemoteEfaMessage requestExistsStorageObject(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      if (p.dataAccess.existsStorageObject()) {
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_OK, null);
      } else {
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_FALSE, "StorageObject does not exists");
      }
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNKNOWN, e.toString());
    }
  }

  private RemoteEfaMessage requestOpenStorageObject(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      p.dataAccess.openStorageObject();
      return RemoteEfaMessage.createResponseResult(request.getMsgId(), RemoteEfaMessage.RESULT_OK,
          null);
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNKNOWN, e.toString());
  ...
  private RemoteEfaMessage requestCreateStorageObject(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestIsStorageObjectOpen(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      if (p.dataAccess.isStorageObjectOpen()) {
        ...
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_FALSE, "StorageObject is not open");
      }
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNKNOWN, e.toString());
  ...
  private RemoteEfaMessage requestCloseStorageObject(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
  ...
  private RemoteEfaMessage requestDeleteStorageObject(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    return RemoteEfaMessage.createResponseResult(request.getMsgId(),
        RemoteEfaMessage.ERROR_NOTYETSUPPORTED, "Operation not supported");
  }

  // =========================== Lock Methods ===========================

  private RemoteEfaMessage requestAcquireGlobalLock(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      long lockId = p.dataAccess.acquireGlobalLock();
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
      response.addField(RemoteEfaMessage.FIELD_LOCKID, Long.toString(lockId));
      return response;
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_FALSE, e.getMessage());
    }
  }

  private RemoteEfaMessage requestAcquireLocalLock(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      DataKey k = request.getKey(0);
      if (k == null) {
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.ERROR_INVALIDREQUEST, "No DataKey given");
      }
  ...
  private RemoteEfaMessage requestReleaseGlobalLock(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      long lockId = request.getLockId();
      if (lockId < 0) {
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.ERROR_INVALIDREQUEST, "No Lock given");
      }
      if (p.dataAccess.releaseGlobalLock(lockId)) {
        ...
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_FALSE, "Global Lock not released");
      }
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNKNOWN, e.getMessage());
    }
  }

  private RemoteEfaMessage requestReleaseLocalLock(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      long lockId = request.getLockId();
      if (lockId < 0) {
      ...
      if (p.dataAccess.releaseLocalLock(lockId)) {
        ...
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_FALSE, "Local Lock not released");
      }
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNKNOWN, e.getMessage());
  ...
  private RemoteEfaMessage requestGetNumberOfRecords(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
      response.addField(RemoteEfaMessage.FIELD_LONGVALUE,
          Long.toString(p.dataAccess.getNumberOfRecords()));
      return response;
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
  ...
  private RemoteEfaMessage requestGetSCN(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
  ...
  private RemoteEfaMessage requestAddValidAt(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      DataKey k = p.data().addValidAt(request.getRecord(0), request.getTimestamp(),
          request.getLockId());
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
      if (k != null) {
  ...
  private RemoteEfaMessage requestAddAll(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetAllKeys(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetByFields(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      DataKey[] keys = p.data().getByFields(request.getFieldArrayNames(),
          request.getFieldArrayValues(), request.getTimestamp());
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
      for (int i = 0; keys != null && i < keys.length; i++) {
        response.addKey(keys[i]);
  ...
  private RemoteEfaMessage requestGetValidAny(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetValidAt(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetValidLatest(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetValidNearest(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      DataRecord r = p.data().getValidNearest(request.getKey(0), request.getValidFrom(),
          request.getInvalidFrom(), request.getTimestamp());
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
      if (r != null) {
        response.addRecord(r);
  ...
  private RemoteEfaMessage requestIsValidAny(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      boolean validAny = p.data().isValidAny(request.getKey(0));
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          (validAny ? RemoteEfaMessage.RESULT_OK : RemoteEfaMessage.RESULT_FALSE), null);
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.ERROR_UNKNOWN, e.toString());
    }
  ...
  private RemoteEfaMessage requestUpdate(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestChangeValidity(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      p.data().changeValidity(request.getRecord(0), request.getValidFrom(),
          request.getInvalidFrom(), request.getLockId());
      return RemoteEfaMessage.createResponseResult(request.getMsgId(), RemoteEfaMessage.RESULT_OK,
          null);
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
  ...
  private RemoteEfaMessage requestDelete(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestDeleteVersionized(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestDeleteVersionizedAll(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestCountRecords(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
      long recs = p.data()
          .countRecords(request.getFieldArrayNames(), request.getFieldArrayValues());
      RemoteEfaMessage response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
          RemoteEfaMessage.RESULT_OK, null);
  ...
  private RemoteEfaMessage requestTruncateAllData(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetFirst(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestGetLast(RemoteEfaMessage request, AdminRecord admin,
      StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestSetInOpeningStorageObject(RemoteEfaMessage request,
      AdminRecord admin, StorageObject p) {
    try {
  ...
  private RemoteEfaMessage requestCmdExitEfa(RemoteEfaMessage request, AdminRecord admin) {
    try {
      if (!admin.isAllowedExitEfa()) {
        return RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.ERROR_NOPERMISSION, "No Permission to update efa");
      }
      final boolean restart = request.getBoolean();
      Logger.log(Logger.INFO, Logger.MSG_EVT_REMOTEEFAEXIT,
          International.getString("Remote-Kommando") + ": " +
      ...
      if (EfaBoathouseFrame.efaBoathouseFrame != null) {
        EfaBoathouseFrame.efaBoathouseFrame.cancelRunInThreadWithDelay(
            EfaBoathouseFrame.EFA_EXIT_REASON_USER, _admin, restart);
        response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_OK, null);
      } else {
        response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_FALSE, null);
      }
      return response;
    } catch (Exception e) {
      return RemoteEfaMessage.createResponseResult(request.getMsgId(),
  ...
  private RemoteEfaMessage requestCmdOnlineUpdate(RemoteEfaMessage request, AdminRecord admin) {
    try {
      ...
      Logger.log(Logger.INFO, Logger.MSG_EVT_REMOTEONLINEUPDATE,
          International.getString("Remote-Kommando") + ": " +
      ...
      if (EfaBoathouseFrame.efaBoathouseFrame != null) {
        if (OnlineUpdate.runOnlineUpdate(null, Daten.ONLINEUPDATE_INFO)) {
          EfaBoathouseFrame.efaBoathouseFrame.cancelRunInThreadWithDelay(
              EfaBoathouseFrame.EFA_EXIT_REASON_ONLINEUPDATE, _admin, true);
          response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
              RemoteEfaMessage.RESULT_OK, null);
        } else {
          response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
              RemoteEfaMessage.RESULT_FALSE, OnlineUpdate.getLastError());
        }
      } else {
        response = RemoteEfaMessage.createResponseResult(request.getMsgId(),
            RemoteEfaMessage.RESULT_FALSE, null);
