...
public abstract class StorageObject {
  ...
  public StorageObject(int storageType,
    ...
    dataAccess = DataAccess.createDataAccess(this, storageType, storageLocation, storageUsername,
    ...
    for (int i = 0; keys != null && i < keys.length; i++) {
      if (!myKey.equals(keys[i])) {
        throw new EfaModifyException(Logger.MSG_DATA_MODIFYEXCEPTION,
            International.getString("Es gibt bereits einen gleichnamigen Eintrag.")
            + " "
            + International.getMessage("Die Felder '{fields}' müssen eindeutig sein.",
                EfaUtil.arr2KommaList(fields)),
                Thread.currentThread().getStackTrace());
      }
    }
  }
   ...
   */
  protected void assertNotReferenced(DataRecord record, StorageObject referencingPersistence,
      String[] referencingFields) throws EfaModifyException {
  ...
  protected void assertNotReferenced(DataRecord record, StorageObject referencingPersistence,
    ...
    assertNotReferenced(record, referencingPersistence, referencingFields,
        keyMustMatchInAllReferencingFields, null, null);
  }

  protected void assertNotReferenced(DataRecord record, StorageObject referencingPersistence,
      String[] referencingFields,
      boolean keyMustMatchInAllReferencingFields,
      String[] referencingFieldFilerNames, String[] referencingFieldFilerValues)
          throws EfaModifyException {
    if (this.data().inOpeningStorageObject()) {
      return;
    }
    ...
    if (this.data().getMetaData().isVersionized()) {
      return; // @todo (P8) there are lots of operations which involve deleting of versions, which
      // trigger this check unnecessarily
    }
    ...
    try {
      ...
      if (keyFieldCount > 1
          && record.metaData.isVersionized()
          && record.metaData.getFieldName(keyFields[keyFieldCount - 1])
          .equals(DataRecord.VALIDFROM)) {
        keyFieldCount--;
      }
      ...
      for (int i = 0; i < referencingFieldIdx.length; i++) {
        referencingFieldIdx[i] = referencingPersistence.data().getMetaData()
            .getFieldIndex(referencingFields[i]);
      }

      // get the field indexes of the referencing records's filter fields ("where clause")
      int[] referencingFieldFilterIdx = (referencingFieldFilerNames != null ? new int[referencingFieldFilerNames.length]
          : null);
      for (int i = 0; referencingFieldFilterIdx != null && i < referencingFieldFilterIdx.length; i++) {
        referencingFieldFilterIdx[i] = referencingPersistence.data().getMetaData()
            .getFieldIndex(referencingFieldFilerNames[i]);
      }
