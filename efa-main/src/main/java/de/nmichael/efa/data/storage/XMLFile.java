...
import de.nmichael.efa.util.SaxErrorHandler;
...
public class XMLFile extends DataFile {
  ...
  public synchronized void readFromInputStream(InputStream in) throws EfaException {
    try {
      ...
      throw new EfaException(Logger.MSG_DATA_OPENFAILED, LogString.fileOpenFailed(filename,
  ...
  protected synchronized void readFile(BufferedReader fr) throws EfaException {
    ...
    try {
      ...
      inOpeningStorageObject = true; // don't update LastModified Timestamps, don't increment SCN,
      // don't check assertions!
      parser.parse(new InputSource(fr));
      if (xmlFileReader.getDocumentReadError() != null) {
        throw new EfaException(Logger.MSG_DATA_INVALIDHEADER, xmlFileReader.getDocumentReadError(),
            Thread.currentThread().getStackTrace());
      }
    } catch (Exception e) {
      Logger.logdebug(e);
...
class XmlFileInfo {
  ...
  public XmlFileInfo(IDataAccess dataAccess, OutputStream out) {
    ...
    this.out = out;
    // BufferedWriter is more efficient for files; only used if out is an instance of
    // FileOutputStream
    if (out != null && out instanceof FileOutputStream) {
      try {
        fout = new BufferedWriter(new OutputStreamWriter(out, DataFile.ENCODING));
        fout2 = getMirrorFile();
      } catch (Exception e) {
        fout = null;
      }
  ...
  private BufferedWriter getMirrorFile() {
    try {
      ...
      if (mirrorDir != null && mirrorDir.length() > 0 && dataAccess instanceof XMLFile &&
        ...
        return new BufferedWriter(new OutputStreamWriter(new FileOutputStream(mirrorFile),
            DataFile.ENCODING));
      }
    } catch (Exception e) {
  ...
  public void write(String s) throws IOException {
    if (fout != null) {
      ...
      out.write(s.getBytes(DataFile.ENCODING));
    }
  }
