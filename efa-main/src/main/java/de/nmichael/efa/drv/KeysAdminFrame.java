...
public class KeysAdminFrame extends JDialog implements ActionListener {
  ...
  void newButton_actionPerformed(ActionEvent e) {
    String _nr = Dialog.inputDialog("Schlüsselnummer",
        "Bitte gib eine Nummer für den neu zu erstellenden Schlüssel ein:");
    if (_nr == null) {
      return;
    }
    ...
    String _validity = Dialog.inputDialog("Gültigkeitsdauer",
        "Bitte gib an, für welche Wettbewerbsjahre der Schlüssel gelten soll (JJJJ-JJJJ):");
    if (_validity == null) {
      return;
    }
    ...
    int tage = EfaUtil.getDateDiff(EfaUtil.getCurrentTimeStampDD_MM_YYYY(), "31.12."
        + (tmj.monat + 1)) - 1;
    ...
    try {
      ...
      if (!ca.signRequest(Daten.efaTmpDirectory + "certreq.csr", Daten.efaTmpDirectory
          + "certreq.pem", tage)) {
        ...
        Dialog.infoDialog("Schlüssel wurde gelöscht",
            "Der erstellte Schlüssel wurde erfolgreich gelöscht.");
        return;
      }
      ...
      Logger.log(Logger.INFO, "Neuer Schlüssel " + alias + " (und privater Schlüssel " + alias
          + "_priv) erstellt.");
  ...
  void exportCertButton_actionPerformed(ActionEvent e) {
    ...
    if (alias.endsWith("_priv")) {
      Dialog
      .error("Zertifikate werden nur für öffentliche Schlüssel ausgestellt!\nBitte wähle einen Schlüssel mit Namen drvXX.");
      return;
    }
    ...
    try {
      ...
      String request = Main.drvConfig.makeScriptRequestString(DRVConfig.ACTION_UPLCERT, "cert="
          + alias + ".cert64", "data=" + data, null, null);
  ...
  void setDefaultButton_actionPerformed(ActionEvent e) {
    ...
    if (!alias.endsWith("_priv")) {
      Dialog
      .error("Nur private Schlüssel können als Standardschlüssel markiert werden!\nBitte wähle einen privaten Schlüssel aus.");
      return;
    }
