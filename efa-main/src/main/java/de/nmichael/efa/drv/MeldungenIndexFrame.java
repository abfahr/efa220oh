...
public class MeldungenIndexFrame extends JDialog implements ActionListener {
  ...
  private void jbInit() throws Exception {
    ...
    try {
      ...
      eastPanel.add(addButton, new GridBagConstraints(0, 4, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
      eastPanel.add(importButton, new GridBagConstraints(0, 5, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
      eastPanel.add(exportButton, new GridBagConstraints(0, 6, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
      eastPanel.add(jLabel2, new GridBagConstraints(0, 7, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
          GridBagConstraints.NONE, new Insets(50, 0, 0, 0), 0, 0));
      eastPanel.add(anzBestaetigte, new GridBagConstraints(1, 7, 1, 1, 0.0, 0.0,
          GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(50, 0, 0, 0), 0, 0));
      eastPanel.add(uploadButton, new GridBagConstraints(0, 8, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
      eastPanel.add(checkFahrtenheftButton, new GridBagConstraints(0, 9, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(50, 0, 0, 0), 0, 0));
      eastPanel.add(meldestatistikButton, new GridBagConstraints(0, 10, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(50, 0, 0, 0), 0, 0));
      eastPanel.add(printOverviewButton, new GridBagConstraints(0, 11, 2, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
  ...
  void showMeldungen() {
    ...
    // zählen der Meldungen, für die eine Bestätigungsdatei vorliegt, die noch nicht hochgeladen
    // wurde
    int countBestaetigung = 0;

    // Alle Meldungen einlesen und relevante Meldungen merken
    for (DatenFelder d = Main.drvConfig.meldungenIndex.getCompleteFirst(); d != null; d = Main.drvConfig.meldungenIndex
        .getCompleteNext()) {
    ...
    for (int i = 0; i < m.size(); i++) {
      ...
      int stat = EfaUtil.string2int(((DatenFelder) m.get(i)).get(MeldungenIndex.STATUS),
          MeldungenIndex.ST_UNBEKANNT);
      if (stat < 0 || stat >= MeldungenIndex.ST_NAMES.length) {
        stat = MeldungenIndex.ST_UNBEKANNT;
      }
      tableData[i][j++] = MeldungenIndex.ST_NAMES[stat];
      if (this.MELDTYP == MeldungenIndexFrame.MELD_FAHRTENABZEICHEN) {
        int fh = EfaUtil.string2int(((DatenFelder) m.get(i)).get(MeldungenIndex.FAHRTENHEFTE),
            MeldungenIndex.FH_UNBEKANNT);
        switch (fh) {
      ...
      String best = "";
      if (((DatenFelder) m.get(i)).get(MeldungenIndex.STATUS).equals(
          Integer.toString(MeldungenIndex.ST_BEARBEITET))) {
    ...
    String[] tableHeaderFA = { "Quittungsnr.", "Verein", "Mitgliedsnr.", "Datum", "Status",
        "Fahrtenhefte", "Bestätigt" };
    String[] tableHeaderWS = { "Quittungsnr.", "Verein", "Mitgliedsnr.", "Datum", "Status",
    "Bestätigt" };
    ...
    if (width < this.centerPanel.getSize().width - 20 || width > this.getSize().width) { // beim
      // ersten
      // Aufruf
      // steht
      // Tabellenbreite
  ...
  void downloadButton_actionPerformed(ActionEvent e) {
    if (Main.drvConfig.efw_script == null || Main.drvConfig.efw_script.length() == 0) {
      Dialog
      .error("Kein EFW-Script konfiguriert. Bitte vervollständige zunächst die Konfiguration!");
      return;
    }
    if (Main.drvConfig.efw_user == null || Main.drvConfig.efw_user.length() == 0) {
      Dialog
      .error("Kein EFW-Nutzer konfiguriert. Bitte vervollständige zunächst die Konfiguration!");
      return;
    }
    if (Main.drvConfig.efw_password == null || Main.drvConfig.efw_password.length() == 0) {
      Dialog
      .error("Kein EFW-Paßwort konfiguriert. Bitte vervollständige zunächst die Konfiguration!");
      return;
    }
    ...
    String url = Main.drvConfig.makeScriptRequestString(DRVConfig.ACTION_LIST, null, null, null,
        null);
    ...
    try {
      f = new BufferedReader(new InputStreamReader(new FileInputStream(listFile),
          Daten.ENCODING_ISO));
      String s;
      while ((s = f.readLine()) != null) {
        ...
        if (v.size() != 6) {
          ...
          Logger.log(
              Logger.ERROR,
              "Fehler beim Lesen der Meldungen-Indexdatei: Datei hat ungültiges Format ("
                  + v.size() + " Felder).");
          Logger.log(Logger.INFO, "ENDE Neue Meldungen aus dem Internet abrufen");
          f.close();
          return;
        ...
        if (dtmp == null
            || dtmp.get(MeldungenIndex.STATUS)
            .equals(Integer.toString(MeldungenIndex.ST_GELOESCHT))) {
          url = Main.drvConfig.makeScriptRequestString(DRVConfig.ACTION_GET, "item=" + qnr,
              "verein=" + (String) v.get(5), null, null);
          String localFile = Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep + qnr
              + ".efw";
          Logger.log(Logger.INFO, "Download der neuen Meldung " + qnr + " ...");
          if (!DownloadThread.getFile(this, url, localFile, true)
              || !EfaUtil.canOpenFile(localFile)) {
            ...
            try {
              ...
              if (!efw.readFile()) {
                ...
                Logger.log(Logger.ERROR, "Heruntergeladene Meldung " + qnr
              ...
              errorLog += "Heruntergeladene Meldung " + qnr + " kann nicht gelesen werden.";
              Logger.log(Logger.ERROR, "Heruntergeladene Meldung " + qnr
                  + " kann nicht gelesen werden: " + ee.toString());
              meldungOk = false;
            }
            if (meldungOk) {
              ...
              if (papierFahrtenhefteErforderlich && !elektronischeFahrtenhefteVorhanden) {
                ...
                d.set(MeldungenIndex.FAHRTENHEFTE,
                    Integer.toString(MeldungenIndex.FH_PAPIER_UND_ELEKTRONISCH));
              } else if (!papierFahrtenhefteErforderlich && !elektronischeFahrtenhefteVorhanden) {
  ...
  void exportButton_actionPerformed(ActionEvent e) {
    ...
    EfaWett efw = new EfaWett(Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep + qnr
        + ".efw");
    ...
    try {
      if (efw.readFile()) {
        ...
        f.close();
        Dialog
        .infoDialog("Meldung exportiert", "Die Meldung wurde erfolgreich exportiert:\n" + csv);
      }
    } catch (Exception ee) {
  ...
  void importButton_actionPerformed(ActionEvent e) {
    ...
    try {
      if (efw.readFile()) {
        efw.setFileName(Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep + qnr
            + ".efw");
        if (efw.writeFile()) {
          Logger
          .log(Logger.INFO, "Neue Meldungen mit Quittungsnummer " + qnr + " wird importiert.");
          DatenFelder d = new DatenFelder(MeldungenIndex._ANZFELDER);
          d.set(MeldungenIndex.QNR, qnr);
  ...
  void editButton_actionPerformed(ActionEvent e) {
    ...
    switch (EfaUtil.string2int(d.get(MeldungenIndex.STATUS), -1)) {
      case MeldungenIndex.ST_GELOESCHT:
        Dialog
        .error("Die gewählte Meldung wurde bereits gelöscht und kann nicht mehr bearbeitet werden.");
        return;
      case MeldungenIndex.ST_ZURUECKGEWIESEN:
        if (Dialog
            .yesNoCancelDialog(
                "Meldung bereits zurückgewiesen",
          ...
          if ((new File(Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep + qnr
              + ".efwsig")).exists()) {
            d.set(MeldungenIndex.STATUS, Integer.toString(MeldungenIndex.ST_BEARBEITET));
            d.set(MeldungenIndex.BESTAETIGUNGSDATEI, Daten.efaDataDirectory
                + Main.drvConfig.aktJahr + Daten.fileSep + qnr + ".efwsig");
            Logger.log(Logger.INFO, "Zurückweisen von Meldung " + qnr
                + " rückgängig gemacht. Meldung hat jetzt den Status 'bearbeitet'!");
            Dialog
            .infoDialog("Zurückweisen rückgängig gemacht",
                "Die Meldung hat jetzt den Status 'bearbeitet' und sollte erneut bestätigt werden!");
          } else {
            d.set(MeldungenIndex.STATUS, Integer.toString(MeldungenIndex.ST_UNBEARBEITET));
            Logger.log(Logger.INFO, "Zurückweisen von Meldung " + qnr
                + " rückgängig gemacht. Meldung hat jetzt den Status 'unbearbeitet'!");
  ...
  void deleteButton_actionPerformed(ActionEvent e) {
    ...
    if (Dialog.yesNoDialog("Meldung löschen", "Möchtest Du die Meldung mit Quittungsnummer " + qnr
        + " wirklich löschen?") != Dialog.YES) {
      return;
    }
  ...
  void uploadButton_actionPerformed(ActionEvent e) {
    ...
    for (DatenFelder d = Main.drvConfig.meldungenIndex.getCompleteFirst(); d != null; d = Main.drvConfig.meldungenIndex
      ...
      if (d.get(MeldungenIndex.STATUS).equals(Integer.toString(MeldungenIndex.ST_BEARBEITET))
        ...
        if (MELDTYP == MELD_WANDERRUDERSTATISTIK) {
          bestaetigungen.add(EfaUtil.getPathOfFile(Main.drvConfig.meldungenIndex.getFileName())
              + Daten.fileSep + d.get(MeldungenIndex.QNR) + ".efw");
        }
      }
    }
      ...
      try {
        ...
        if (MELDTYP == MELD_FAHRTENABZEICHEN) {
          ...
          if (!esfh.readFile()) {
            ...
            Logger.log(Logger.ERROR, "Bestätigungsdatei " + filename
                + " kann nicht gelesen werden.");
            continue;
          }
        ...
        if (MELDTYP == MELD_FAHRTENABZEICHEN) {
          BufferedReader f = new BufferedReader(new InputStreamReader(
              new FileInputStream(filename), Daten.ENCODING_ISO));
          String z;
        ...
        String request = Main.drvConfig.makeScriptRequestString(DRVConfig.ACTION_UPLOAD, "verein="
            + verein, "qnr=" + qnr, "wettid=" + wettid, "data=" + data);
        int pos = request.indexOf("?");
        if (pos < 0) {
          Logger.log(Logger.ERROR, "efaWett-Anfrage für Bestätigungsdatei " + filename
              + " kann nicht erstellt werden.");
          errors += "efaWett-Anfrage für Bestätigungsdatei " + filename
              + " kann nicht erstellt werden.\n";
          continue;
        }
        ...
        if (ok) {
          DatenFelder d = Main.drvConfig.meldungenIndex.getExactComplete(qnr);
          if (d == null) {
            Logger.log(Logger.ERROR, "Konnte Status für Meldung " + qnr
                + " nicht aktualisieren: Meldung nicht gefunden!");
            errors += "Konnte Status für Meldung " + qnr
                + " nicht aktualisieren: Meldung nicht gefunden!\n";
          } else {
          ...
          Logger.log(Logger.INFO, "Meldung " + qnr + " von Verein " + verein
              + " wurde erfolgreich bestätigt!ÿ");
          cok++;
        }
      } catch (Exception ee) {
        Logger
        .log(Logger.ERROR, "Fehler beim Bestätigen von Meldung " + qnr + ": " + ee.toString());
    ...
    Dialog.infoDialog("Meldungen bestätigt", "Es wurden " + cok
        + " Meldungen erfolgreich bestätigt.");
  ...
  void rejectButton_actionPerformed(ActionEvent e) {
    ...
    if (EfaUtil.string2int(d.get(MeldungenIndex.STATUS), -1) != MeldungenIndex.ST_UNBEARBEITET) {
      if (EfaUtil.string2int(d.get(MeldungenIndex.STATUS), -1) == MeldungenIndex.ST_ZURUECKGEWIESEN) {
        Dialog
        .error("Die Meldung wurde bereits zurückgewiesen. Sie kann nicht nochmals zurückgewiesen werden.");
        return;
      }
    ...
    String verein = null;
    EfaWett ew = new EfaWett(Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep + qnr
        + ".efw");
    try {
      if (ew.readFile()) {
    ...
    if (Dialog.yesNoDialog("Meldung zurückweisen", "Möchtest Du die Meldung " + qnr
        + " des Vereins " + verein + " wirklich zurückweisen?") != Dialog.YES) {
      return;
    }
    String grund;
    do {
    ...
    String url = Main.drvConfig.makeScriptRequestString(DRVConfig.ACTION_REJECT,
        "verein=" + verein, "qnr=" + qnr, "grund=" + EfaUtil.replace(grund, " ", "+", true), null);
    ...
    if (!DownloadThread.getFile(this, url, localFile, true) || !EfaUtil.canOpenFile(localFile)) {
      Logger.log(Logger.ERROR, "Zurückweisen der Meldung " + qnr + " von Verein " + verein
          + " fehlgeschlagen: Kann efaWett nicht erreichen");
    ...
    Logger.log(Logger.INFO, "Meldung " + qnr + " von Verein " + verein + " zurückgewiesen. Grund: "
        + grund);
  ...
  void createMeldestatistikFA() {
    try {
      ...
      String stat_complete = Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep
          + "meldestatistik_komplett.csv";
      BufferedWriter f;
      f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(stat_complete),
          Daten.ENCODING_ISO));
      ...
      for (DatenFelder d = Main.drvConfig.meldestatistik.getCompleteFirst(); d != null; d = Main.drvConfig.meldestatistik
          .getCompleteNext()) {
        f.write(d.get(Meldestatistik.VEREIN) + ";" + d.get(Meldestatistik.VORNAME) + ";"
            + d.get(Meldestatistik.NACHNAME) + ";"
            + d.get(Meldestatistik.JAHRGANG) + ";" + d.get(Meldestatistik.GESCHLECHT) + ";"
            + d.get(Meldestatistik.KILOMETER) + ";"
            + d.get(Meldestatistik.GRUPPE) + ";" + d.get(Meldestatistik.ANZABZEICHEN) + ";"
      ...
      String stat_div = Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep
          + "meldestatistik_einzeln.csv";
      f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(stat_div),
          Daten.ENCODING_ISO));
      ...
      f.write("Statistik 3: Jahrgang " + (Main.drvConfig.aktJahr - 75)
          + " und älter (75 Jahre und älter)\n");
      ...
      for (DatenFelder d = Main.drvConfig.meldestatistik.getCompleteFirst(); d != null; d = Main.drvConfig.meldestatistik
        ...
        if (jahrgang <= Main.drvConfig.aktJahr - 75) {
          int key = jahrgang * 100000
              + (999999 - EfaUtil.string2int(d.get(Meldestatistik.KILOMETER), 0));
          sStat.add(
              key,
              null,
      ...
      for (DatenFelder d = Main.drvConfig.meldestatistik.getCompleteFirst(); d != null; d = Main.drvConfig.meldestatistik
        ...
        if (km >= 4000) {
          sStat.add(
              ...
              d.get(Meldestatistik.KILOMETER),
              d.get(Meldestatistik.NACHNAME) + ", " + d.get(Meldestatistik.VORNAME) + " ("
                  + d.get(Meldestatistik.JAHRGANG) + "), " + d.get(Meldestatistik.VEREIN),
                  null);
        }
      }
      sStat.sort(false);
      ...
      for (int _anzAbz = 0; _anzAbz <= 95; _anzAbz += 5) {
        for (DatenFelder d = Main.drvConfig.meldestatistik.getCompleteFirst(); d != null; d = Main.drvConfig.meldestatistik
          ...
          if ((_anzAbz == 0 && !erwachsen && anzAbz % 5 == 0) || // Jugend-Gold
            ...
            String name = d.get(Meldestatistik.NACHNAME) + ", " + d.get(Meldestatistik.VORNAME)
                + " (" + d.get(Meldestatistik.JAHRGANG) + "), " + d.get(Meldestatistik.VEREIN);
      ...
      if (mitglnr_hash.size() < Main.drvConfig.meldungenIndex.countElements()) {
        ...
        stat_info = "ACHTUNG: Die Statistik enthält nur die Daten von " + mitglnr_hash.size()
            + " bereits fertig\n"
            + "bearbeiteten Vereinen. " + differenz
            + " nicht bearbeitete Vereine wurden NICHT berücksichtigt.\n\n";
      }
      ...
      if (Dialog.yesNoDialog("Statistiken kopieren",
        ...
        String ziel = Dialog
            .inputDialog("Ziel", "Wohin sollen die Dateien kopiert werden?", "A:\\");
        if (ziel != null && ziel.trim().length() > 0) {
          ...
          if (!(new File(ziel)).isDirectory()) {
            ...
            boolean b1 = EfaUtil.copyFile(stat_complete,
                ziel + EfaUtil.getFilenameWithoutPath(stat_complete));
            boolean b2 = EfaUtil
                .copyFile(stat_div, ziel + EfaUtil.getFilenameWithoutPath(stat_div));
            if (b1 && b2) {
  ...
  void createMeldestatistikWS() {
    try {
      ...
      for (DatenFelder d = Main.drvConfig.meldungenIndex.getCompleteFirst(); d != null; d = Main.drvConfig.meldungenIndex
        ...
        if (qnr != null && qnr.length() > 0) {
          EfaWett efw = new EfaWett(Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep
              + qnr + ".efw");
          try {
            if (efw.readFile()) {
              for (EfaWettMeldung ewm = efw.meldung; ewm != null; ewm = ewm.next) {
                if (ewm.drvint_wirdGewertet) {
                  if (ewm.drvWS_Gewaesser != null && ewm.drvWS_Gewaesser.length() > 0) {
                    Vector<String> g = EfaUtil.split(
                        EfaUtil.replace(ewm.drvWS_Gewaesser, ";", ",", true), ',');
      ...
      String stat_waters = Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep
          + "gewaesser.csv";
      f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(stat_waters),
          Daten.ENCODING_ISO));
  ...
  void printOverviewButton_actionPerformed(ActionEvent e) {
    ...
    try {
      ...
      for (DatenFelder d = Main.drvConfig.meldungenIndex.getCompleteFirst(); d != null; d = Main.drvConfig.meldungenIndex
          .getCompleteNext()) {
        if (selection == 0
            && EfaUtil.string2int(d.get(MeldungenIndex.STATUS), MeldungenIndex.ST_UNBEKANNT) != MeldungenIndex.ST_BEARBEITET) {
          continue;
        }
        String verein = d.get(MeldungenIndex.VEREIN);
        String status = MeldungenIndex.ST_NAMES[EfaUtil.string2int(d.get(MeldungenIndex.STATUS),
            MeldungenIndex.ST_UNBEKANNT)];
        String feld3 = "";
        if (MELDTYP == MELD_WANDERRUDERSTATISTIK) {
          DatenFelder ms = Main.drvConfig.meldestatistik.getExactComplete(d
              .get(MeldungenIndex.MITGLNR));
          if (ms != null) {
            feld3 = ms.get(Meldestatistik.WS_GEWAESSER);
          }
        } else {
          int fh = EfaUtil.string2int(d.get(MeldungenIndex.FAHRTENHEFTE),
              MeldungenIndex.FH_UNBEKANNT);
          switch (fh) {
            case MeldungenIndex.FH_UNBEKANNT:
              feld3 = "";
      ...
      /*
       * SimpleFilePrinter.sizeJEditorPane(out); SimpleFilePrinter sfp = new SimpleFilePrinter(out);
       * if (sfp.setupPageFormat()) { if (sfp.setupJobOptions()) { sfp.printFile(); } }
       */
  ...
  void addButton_actionPerformed(ActionEvent e) {
    ...
    try {
      ...
      dlg.setModal(true);
      dlg.show();
      if (MeldungEditFrame.hasBeenSaved()) {
