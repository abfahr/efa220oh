...
public class BoatReservation implements Comparable {
  ...
  public long validInMinutes = 0; // internes Feld, wird nicht gespeichert, sondern nur von
  // getReservierung(...) verwendet
  ...
  public int compareTo(Object o) throws ClassCastException {
    ...
    if (!this.dateFrom.equals(b.dateFrom)) {
      for (String element : WEEKDAYKEYS) {
        if (this.dateFrom.equals(element)) {
          return -1;
        }
        if (b.dateFrom.equals(element)) {
          return 1;
        }
      }
    }
