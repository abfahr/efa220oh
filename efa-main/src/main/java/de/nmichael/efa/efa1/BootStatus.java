...
public class BootStatus extends DatenListe {
  ...
  public static final int STAT_VORUEBERGEHEND_VERSTECKEN = 4; // wird intern für Kombiboote
  // verwendet
  private static final String[] STATUSKEYS = { "HIDE", "AVAILABLE", "ONTHEWATER", "NOTAVAILABLE",
  "CURRENTLYHIDDEN" };
  ...
  public Vector getBoote(int status) {
    Vector v = new Vector();
    for (DatenFelder d = this.getCompleteFirst(); d != null; d = this
        .getCompleteNext()) {
      if (getStatusID(d.get(STATUS)) == status) {
        v.add(d.get(NAME));
      }
    }
    return v;
  ...
  public static Vector getReservierungen(DatenFelder boot, int version) {
    ...
    while (tok.hasMoreTokens()) {
      ...
      switch (pos) {
        case 0:
          r.setDateFrom(tok.nextToken());
          break;
        case 1:
          r.setTimeFrom(tok.nextToken());
          break;
        case 2:
          r.setDateTo(tok.nextToken());
          break;
        case 3:
          r.setTimeTo(tok.nextToken());
          break;
        case 4:
          r.setForName(tok.nextToken());
          break;
        case 5:
          r.setReason(tok.nextToken());
          break;
        case 6:
          r.setOneTimeReservation(tok.nextToken().equals("+"));
          break;
      }
      pos++;
      if ((version == 160 && pos > 5) || (version >= 170 && pos > 6)) {
        if (version == 170) {
          ...
          if (r.getDateFrom().equals("Montag")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[0]);
          }
          if (r.getDateFrom().equals("Dienstag")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[1]);
          }
          if (r.getDateFrom().equals("Mittwoch")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[2]);
          }
          if (r.getDateFrom().equals("Donnerstag")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[3]);
          }
          if (r.getDateFrom().equals("Freitag")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[4]);
          }
          if (r.getDateFrom().equals("Samstag")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[5]);
          }
          if (r.getDateFrom().equals("Sonntag")) {
            r.setDateFrom(BoatReservation.WEEKDAYKEYS[6]);
          }
          if (r.getDateTo().equals("Montag")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[0]);
          }
          if (r.getDateTo().equals("Dienstag")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[1]);
          }
          if (r.getDateTo().equals("Mittwoch")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[2]);
          }
          if (r.getDateTo().equals("Donnerstag")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[3]);
          }
          if (r.getDateTo().equals("Freitag")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[4]);
          }
          if (r.getDateTo().equals("Samstag")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[5]);
          }
          if (r.getDateTo().equals("Sonntag")) {
            r.setDateTo(BoatReservation.WEEKDAYKEYS[6]);
          }
        }
        v.add(r);
        pos = 0;
      ...
      if (!r.isOneTimeReservation()) {
        switch (weekday) {
          case Calendar.MONDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[0])) {
              continue;
            }
            break;
          case Calendar.TUESDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[1])) {
              continue;
            }
            break;
          case Calendar.WEDNESDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[2])) {
              continue;
            }
            break;
          case Calendar.THURSDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[3])) {
              continue;
            }
            break;
          case Calendar.FRIDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[4])) {
              continue;
            }
            break;
          case Calendar.SATURDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[5])) {
              continue;
            }
            break;
          case Calendar.SUNDAY:
            if (!r.getDateFrom().equals(BoatReservation.WEEKDAYKEYS[6])) {
              continue;
            }
            break;
        }
  ...
  public static boolean deleteObsoleteReservierungen(DatenFelder boot) {
    ...
    for (int i = 0; i < res.size(); i++) {
      ...
      if (!r.isOneTimeReservation())
      {
        continue; // zyklische Reservierungen werden nicht gelöscht
      }
  ...
  public static void setReservierungen(DatenFelder boot, Vector v) {
    ...
    for (int i = 0; i < v.size(); i++) {
      a[i] = (BoatReservation) v.get(i);
    }
    Arrays.sort(a);
    for (BoatReservation r : a) {
        ...
        if (s != null && s.trim().startsWith(KENNUNG170)) {
          ...
          try {
            while ((s = freadLine()) != null) {
              ...
              /*
               * @efa1 if (bemerk.equals("nicht anzeigen")) bemerk =
               * International.getString("nicht anzeigen"); if (bemerk.equals("verfügbar")) bemerk =
               * International.getString("verfügbar"); if (bemerk.equals("unterwegs")) bemerk =
               * International.getString("unterwegs"); if (bemerk.equals("nicht verfügbar")) bemerk
               * = International.getString("nicht verfügbar"); if
               * (bemerk.equals("vorübergehend verstecken")) bemerk =
               * International.getString("vorübergehend verstecken");
               */
  ...
  public static String getStatusName(int status) {
    if (STATUSDESCR != null && status >= 0 && status < STATUSDESCR.length) {
  ...
  public static String getStatusKey(int status) {
    if (status >= 0 && status < STATUSKEYS.length) {
