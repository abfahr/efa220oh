...
public class Boote extends DatenListe {
  ...
  public static String getDetailBezeichnung(DatenFelder boot) {
    if (boot == null || Daten.efaTypes == null) {
      return null;
    }
  ...
  public void validateValues(DatenFelder d) {
    String s;
    if ((s = d.get(NAME)).indexOf("[") >= 0) {
      d.set(NAME, EfaUtil.replace(s, "[", "", true));
    }
    if ((s = d.get(NAME)).indexOf("]") >= 0) {
      d.set(NAME, EfaUtil.replace(s, "]", "", true));
    }
    if ((s = d.get(VEREIN)).indexOf("[") >= 0) {
      d.set(VEREIN, EfaUtil.replace(s, "[", "", true));
    }
    if ((s = d.get(VEREIN)).indexOf("]") >= 0) {
      d.set(VEREIN, EfaUtil.replace(s, "]", "", true));
    }
  ...
  public static String makeGruppen(String gruppe1, String gruppe2, String gruppe3, String gruppe4,
      String gruppe5) {
    String g = "";
    if (gruppe1.trim().length() > 0) {
      g += (g.length() > 0 ? "; " : "")
          + EfaUtil.removeSepFromString(EfaUtil.removeSepFromString(gruppe1.trim(), ";"));
    }
    if (gruppe2.trim().length() > 0) {
      g += (g.length() > 0 ? "; " : "")
          + EfaUtil.removeSepFromString(EfaUtil.removeSepFromString(gruppe2.trim(), ";"));
    }
    if (gruppe3.trim().length() > 0) {
      g += (g.length() > 0 ? "; " : "")
          + EfaUtil.removeSepFromString(EfaUtil.removeSepFromString(gruppe3.trim(), ";"));
    }
    if (gruppe4.trim().length() > 0) {
      g += (g.length() > 0 ? "; " : "")
          + EfaUtil.removeSepFromString(EfaUtil.removeSepFromString(gruppe4.trim(), ";"));
    }
    if (gruppe5.trim().length() > 0) {
      g += (g.length() > 0 ? "; " : "")
          + EfaUtil.removeSepFromString(EfaUtil.removeSepFromString(gruppe5.trim(), ";"));
    }
    return g;
  }
        ...
        if (s != null && s.trim().startsWith(KENNUNG170)) {
          ...
          try {
            while ((s = freadLine()) != null) {
              ...
              if (d.get(ANZAHL).equals("andere")) {
                numseats = EfaTypes.TYPE_NUMSEATS_OTHER;
              }
              if (d.get(RIGGER).equals("Skull")) {
                if (d.get(ANZAHL).equals("1er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_1;
                }
                if (d.get(ANZAHL).equals("2er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_2X;
                }
                if (d.get(ANZAHL).equals("3er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_3;
                }
                if (d.get(ANZAHL).equals("4er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_4X;
                }
                if (d.get(ANZAHL).equals("5er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_5;
                }
                if (d.get(ANZAHL).equals("6er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_6X;
                }
                if (d.get(ANZAHL).equals("8er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_8X;
                }
              } else if (d.get(RIGGER).equals("Riemen")) {
                if (d.get(ANZAHL).equals("2er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_2;
                }
                if (d.get(ANZAHL).equals("4er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_4;
                }
                if (d.get(ANZAHL).equals("6er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_6;
                }
                if (d.get(ANZAHL).equals("8er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_8;
                }
              } else {
                if (d.get(ANZAHL).equals("1er")) {
                  numseats = EfaTypes.TYPE_NUMSEATS_1;
              ...
              if (art == null) {
                ...
                /*
                 * @efa1 Logger.log(Logger.ERROR, Logger.MSG_CSVFILE_ERRORCONVERTING, getFileName()
                 * + ": " +
              ...
              if (rig == null) {
                ...
                /*
                 * @efa12 Logger.log(Logger.ERROR, Logger.MSG_CSVFILE_ERRORCONVERTING, getFileName()
                 * + ": " +
                 * International.getMessage("Fehler beim Konvertieren von Eintrag '{key}'!",
                 * constructKey(d)) + " " + International.getMessage(
                 * "Unbekannte Eigenschaft '{original_property}' korrigiert zu '{new_property}'.",
