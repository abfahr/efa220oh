...
public class Efa1Backup {
  ...
  public boolean create(String fname, int typ, String version) {
    ...
    if (typ == SAVE && month) {
      ...
      String bakname = fbak + cal.get(Calendar.YEAR) + "-"
    ...
    if (typ == SAVE && day) {
      ...
      String bakname = fbak + cal.get(Calendar.YEAR) + "-"
          + (monat < 10 ? "0" + monat : "" + monat) + "-" + (tag < 10 ? "0" + tag : "" + tag);
      if (!new File(bakname).isFile()) {
        if (!backupFile(fname, bakname)) {
          LogString.logError_fileArchivingFailed(fname,
              International.getString("Datei")
  ...
  private boolean backupFile(String orgName, String bakName) {
    try {
      BufferedReader f = new BufferedReader(new InputStreamReader(new FileInputStream(orgName),
          Daten.ENCODING_UTF));
      BufferedWriter ff = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(bakName),
          Daten.ENCODING_UTF));
      ...
      ff = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(bakName, true),
          Daten.ENCODING_UTF));
