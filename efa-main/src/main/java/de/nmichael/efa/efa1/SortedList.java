...
public class SortedList {
  ...
  public boolean put(String k, Object v) {
    ...
    if (!numeric) {
      while (c.prev != null && e.key.compareTo(c.key) < 0) {
        c = c.prev;
      }
    } else {
      // Key als int-Wert betrachten
      while (c.prev != null && EfaUtil.compareIntString(e.key, c.key) < 0) {
        c = c.prev;
      }
    }

    if ((!numeric && e.key.compareTo(c.key) == 0)
        || (numeric && EfaUtil.compareIntString(e.key, c.key) == 0)) {
  ...
  public boolean delete(String k) {
    ...
    // Element suchen
    while (c.next != null && !k.equals(c.key)) {
      c = c.next;
    }
  ...
  public int countElements() {
    /*
     * Element c = head; int i = 0; while (c.next != null) { c = c.next; i++; } return i;
     */
    return hash.size();
  }
  ...
  public String search(Element e, String such, boolean vorwaerts, boolean nurAnfang) {
    ...
    while (e != null) {
      if (ignoreCase) {
        if ((e.key.toLowerCase().startsWith(such) && nurAnfang)
            || (e.key.toLowerCase().equals(such) && !nurAnfang)) {
          lastElement = e;
          return e.key;
        }
      } else {
  ...
  // wie startsWith(such), nur daß "such" nicht als letzter Suchbegriff nicht gespeichert wird und
  // somit
  ...
  public String nextStartsWith(String such) {
    if (lastElement == null || !lastString.equals(such)) {
      return startsWith(such);
    }
  ...
  public Object getCompletePrev() {
    ...
    if (lastElement == null)
    {
      lastElement = head.next; // v0.81: ... = head;
    }
