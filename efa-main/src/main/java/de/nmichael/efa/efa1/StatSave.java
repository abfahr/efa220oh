...
public class StatSave extends DatenListe {
  ...
  public static final int STYLESHEET = 34; // geändert in v1.0 (ehemals TXTFORMATIERT (neu in
  // v0.80))
  ...
  public void validateValues(DatenFelder d) {
    ...
    // Überprüfen, ob die in bezeichnungen.cfg konfigurierten Fahrtarten mit denen in den
    // gespeicherten Statistikeinstellungen übereinstimmen
      ...
      if (fahrtart != null && Daten.efaTypes.size(EfaTypes.CATEGORY_SESSION) != fahrtart.length()) {
        if (fahrtart.length() < Daten.efaTypes.size(EfaTypes.CATEGORY_SESSION)) {
          if (fahrtart.indexOf("-") < 0) {
            ...
            while (fahrtart.length() < Daten.efaTypes.size(EfaTypes.CATEGORY_SESSION)) {
              fahrtart += "+";
            }
