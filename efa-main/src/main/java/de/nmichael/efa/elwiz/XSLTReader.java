...
public class XSLTReader extends DefaultHandler {
    ...
    if (uri.equals("http://www.nmichael.de/elwiz")) {
      ...
      if (!localName.equals("option")) {
        ...
        if (localName.equals("optional")) {
          o.type = ElwizOption.O_OPTIONAL;
        }
        if (localName.equals("select")) {
          o.type = ElwizOption.O_SELECT;
        }
        if (localName.equals("value")) {
          o.type = ElwizOption.O_VALUE;
        }
        ...
        for (int i = 0; i < atts.getLength(); i++) {
          ...
          if (atts.getLocalName(i).equals("name")) {
            o.name = atts.getValue(i);
          }

          // Description
          if (atts.getLocalName(i).equals("descr")) {
            o.descr = atts.getValue(i);
          }
        ...
        for (int i = 0; i < atts.getLength(); i++) {
          if (atts.getLocalName(i).equals("descr")) {
            eso.descr = atts.getValue(i);
          }
          if (atts.getLocalName(i).equals("value")) {
            eso.value = atts.getValue(i);
          }
          if (atts.getLocalName(i).equals("selected")) {
            eso.selected = atts.getValue(i).equals("true");
          }
          if (atts.getLocalName(i).equals("type")) {
            eso.type = atts.getValue(i);
          }
        }
  ...
  public static Vector run(String filename) {
    ...
    if (parser == null) {
      parser = tryToSetup("org.apache.xerces.parsers.SAXParser");
    }
    if (parser == null)
    {
      parser = tryToSetup("javax.xml.parsers.SAXParser"); // Java 1.5
    }
    if (parser == null) {
      parser = tryToSetup("org.apache.crimson.parser.XMLReaderImpl");
    }
    if (parser == null) {
      Logger.log(Logger.ERROR, Logger.MSG_DEBUG_ELWIZ,
          "No XML-Parser found!");
    ...
    /*
     * for (int i=0; i<allOptions.size(); i++) { ElwizOption o = (ElwizOption)allOptions.get(i);
     * System.out.println(o.type+": "+o.name+"; "+o.descr); if (o.options != null) for (int j=0;
     * j<o.options.size(); j++) { ElwizSingleOption eso = (ElwizSingleOption)o.options.get(j);
