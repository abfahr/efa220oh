...
public class XSLTWriter extends DefaultHandler {
    ...
    try {
      if (uri.equals("http://www.nmichael.de/elwiz")) {
        ...
        if (!localName.equals("option")) { // Hauptelement
          ...
          int pos = -1;
          for (int i = 0; i < atts.getLength(); i++) {
            if (atts.getLocalName(i).equals("pos")) {
              pos = EfaUtil.string2int(atts.getValue(i), -1);
            }
          }
        ...
        if (skip)
        {
          return; // Element unterdrücken?
        }

        f.write("<" + qname);
        for (int i = 0; i < atts.getLength(); i++) {
          f.write(" " + atts.getLocalName(i) + "=\""
              + EfaUtil.replace(atts.getValue(i), "<", "&lt;", true) + "\"");
        }
        if (localName.equals("stylesheet")) {
          f.write(" xmlns:xsl=\"http://www.w3.org/1999/XSL/Transform\" xmlns:fo=\"http://www.w3.org/1999/XSL/Format\"");
        }
        f.write(">");
      }
    } catch (IOException e) {}
  ...
  public void endElement(String uri, String localName, String qname) {
    try {
      if (uri.equals("http://www.nmichael.de/elwiz")) { // elwiz-Element
        if (option != null && option.name.equals(localName)) {
          option = null;
        }
        skip = false;
  ...
  public void characters(char[] ch, int start, int length) {
    ...
    try {
      ...
      if (s.indexOf('\n') >= 0 || s.indexOf(10) >= 0 || s.indexOf(13) >= 0) {
        s = s.trim();
      }
      if (s.length() > 0) {
        f.write(s);
  ...
  public static boolean run(String filename, String fileto, Vector allOptions) {
    if (!fileto.toLowerCase().endsWith(".xsl")) {
      fileto += ".xsl";
    }
    ...
    if (parser == null) {
      parser = XSLTReader.tryToSetup("org.apache.xerces.parsers.SAXParser");
    }
    if (parser == null)
    {
      parser = XSLTReader.tryToSetup("javax.xml.parsers.SAXParser"); // Java 1.5
    }
    if (parser == null) {
      parser = XSLTReader.tryToSetup("org.apache.crimson.parser.XMLReaderImpl");
    }
