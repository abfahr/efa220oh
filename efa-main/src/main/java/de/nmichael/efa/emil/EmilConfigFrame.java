...
public class EmilConfigFrame extends JDialog implements ActionListener {
  ...
  private void jbInit() throws Exception {
    ...
    efwdirButton.setIcon(new ImageIcon(EmilConfigFrame.class
        .getResource("/de/nmichael/efa/img/prog_open.gif")));
    ...
    cvsdirButton.setIcon(new ImageIcon(EmilConfigFrame.class
        .getResource("/de/nmichael/efa/img/prog_open.gif")));
    ...
    stdcvsButton.setIcon(new ImageIcon(EmilConfigFrame.class
        .getResource("/de/nmichael/efa/img/prog_save.gif")));
    ...
    nurExportErfuellt
    .setText("beim Export nur Teilnehmer exportieren, die die Bedingungen erfüllt " +
        "haben");
  ...
  void stdcvsButton_actionPerformed(ActionEvent e) {
    ...
    String dat = Dialog.dateiDialog(this, "Standard CSV-Datei", "CSV-Dateien (*.csv)", "csv", dir,
        true);
    if (dat != null) {
      this.std_csv.setText(dat);
    }
  }
  ...
  void efwdirButton_actionPerformed(ActionEvent e) {
    ...
    String dat = Dialog.dateiDialog(this, "EFW-Verzeichnis", null, null, dir, null, null, false,
        true);
    if (dat != null) {
      if (!new File(dat).isDirectory()) {
        int pos = dat.lastIndexOf(Daten.fileSep);
        if (pos > 0) {
          dat = dat.substring(0, pos);
        }
      }
      if (!dat.endsWith(Daten.fileSep)) {
        dat += Daten.fileSep;
      }
  ...
  void cvsdirButton_actionPerformed(ActionEvent e) {
    ...
    String dat = Dialog.dateiDialog(this, "CSV-Verzeichnis", null, null, dir, null, null, false,
        true);
    if (dat != null) {
      if (!new File(dat).isDirectory()) {
        int pos = dat.lastIndexOf(Daten.fileSep);
