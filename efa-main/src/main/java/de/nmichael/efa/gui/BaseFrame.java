...
public abstract class BaseFrame extends JFrame implements ActionListener {
  ...
  public String addKeyAction(String key) {
    ...
    try {
      ...
      Logger.log(
          Logger.ERROR,
          Logger.MSG_GUI_ERRORACTIONHANDLER,
          "Error setting up ActionHandler for " + getClass().getCanonicalName() + ": "
              + e.toString()); // no need to translate
      return null;
    }
  }
  ...
  protected void iniDialogCommon(String title) throws Exception {
    ...
    if (Logger.isTraceOn(Logger.TT_HELP, 5)) {
      Logger.log(Logger.DEBUG, Logger.MSG_HELP_DEBUGHELPTOPICFRAMEOPENED, "Dialog Help Topic(s): "
          + helpTopic1 + (helpTopic2 != null ? " , " + helpTopic2 : ""));
    }
