...
public abstract class BaseTabbedDialog extends BaseDialog {
  ...
  public void setItems(Vector<IItemType> guiItems) {
    ...
    itemsPerCategory = new Hashtable<String, Vector<IItemType>>(); // categoryhierarchy -> config
    // items
  ...
  private int recursiveBuildGui(Hashtable<String, Hashtable> categories,
    ...
    int itmcnt = 0;
    int pos = (selectedPanel != null && selectedPanel.length() > 0 ? selectedPanel
        .indexOf(CATEGORY_SEPARATOR) : -1);
    ...
    for (int i = 0; i < allGuiItems.size(); i++) {
      ...
      if (item.isChanged()) {
        ...
        if (Logger.isTraceOn(Logger.TT_GUI)) {
          Logger.log(Logger.DEBUG, Logger.MSG_GUI_DEBUGGUI, this.getClass().getCanonicalName()
              + ".getValuesFromGui(): " + item.getName() + " has changed");
        }
      }
    }
    return changed;
