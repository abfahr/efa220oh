...
public class BrowserDialog extends BaseDialog {
  ...
  boolean locked = false; // wenn efa durch Anzeiges des Browsers gelocked ist und nur von Admin
  ...
  class LinkFollower implements HyperlinkListener {
    ...
    public void hyperlinkUpdate(HyperlinkEvent e) {
      if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
        if ((e.getURL().toString().toLowerCase().startsWith("http://") || e.getURL().toString()
        ...
        /*
         * // Der folgende Code ist ein Workaround für Java-Bug-ID 6222200 (EditorPane GET instead
         * of POST). // Zur Zeit funktioniert der Workaround allerdings noch nicht. try { if
         * (Daten.javaVersion.startsWith("1.5") || Daten.javaVersion.startsWith("1.6")) { if
         * (de.nmichael.efa.java15.Java15.editorPaneHandlePostEvent(out,e)) return; } }
        ...
        try {
          ...
          if (conn == null || conn.getContentType() == null) {
            html.setText(International.getString("FEHLER")
                + ": "
                + International.getMessage("Kann Adresse '{url}' nicht öffnen: {message}", e
                    .getURL().toString(),
  ...
  protected void iniDialog() throws Exception {
    ...
    northWestPanel.add(backButton, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
        GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    northWestPanel.add(nextButton, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0,
        GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    northWestPanel.add(reloadButton, new GridBagConstraints(2, 0, 1, 1, 0.0, 0.0,
        GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    if (Daten.applID != Daten.APPL_EFABH || Daten.applMode == Daten.APPL_MODE_ADMIN) {
      northWestPanel.add(printButton, new GridBagConstraints(4, 0, 1, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
      northWestPanel.add(pageButton, new GridBagConstraints(5, 0, 1, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
      northWestPanel.add(saveButton, new GridBagConstraints(3, 0, 1, 1, 0.0, 0.0,
          GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    }
    ...
    searchPanel.add(searchLabel, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0,
        GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
    searchPanel.add(search, new GridBagConstraints(1, 0, 1, 2, 0.0, 0.0, GridBagConstraints.CENTER,
    ...
    /*
     * // Der folgende Code ist ein Workaround für Java-Bug-ID 6222200 (EditorPane GET instead of
     * POST). // Zur Zeit funktioniert der Workaround allerdings noch nicht. try { if
     * (Daten.javaVersion.startsWith("1.5") || Daten.javaVersion.startsWith("1.6"))
     * de.nmichael.efa.java15.Java15.setEditorPaneAutoFormSubmissionFalse(out); }
  ...
  public boolean cancel() {
    ...
    if (locked) {
      AdminRecord admin = AdminLoginDialog.login(this,
          International.getString("Entsperren von efa"));
      if (admin == null) {
        return false;
      }
  ...
  void saveButton_actionPerformed(ActionEvent e) {
    ...
    if (!EfaUtil.copyFile(quelle, ziel)) {
      ...
      if (ziel.toLowerCase().endsWith(".html") || ziel.toLowerCase().endsWith(".htm")) {
        // Dennis will unbedingt, daß auch in HTML-Seiten eingebundene Bilder mit gespeichert
        // werden.
        ...
        // Es wird in der Quelldatei nach der Zeichenkette "src=" gesucht (unabh. davon, ob sie in
        // einem
        // <img>-Tag auftritt oder nicht) und kopiert diese Datei, sofern es eine "lokale" Datei
        // ist, d.h.
        ...
        try {
                ...
                if (img.length() > 0 && (img.charAt(0) == '"' || img.charAt(0) == '\'')) {
                  ...
                  if (endquote > 0) { // Ende der Anfüngrungsstriche gefunden?
                    ...
                    if (!img.toLowerCase().startsWith("http://")
                      ...
                      if (!new File(zieldir + img).exists()) { // kopiere nur Dateien, die im
                        // Zielverzeichnis noch nicht
                        // existieren
          ...
          Dialog.error(LogString.fileSavingFailed("...", International.getString("Bilder"),
              ee.toString()));
  ...
  void downloadUrl(URLConnection conn, String fname) {
    ...
    try {
      ...
      String localName = Daten.efaTmpDirectory
    ...
    try {
      ...
      if (pos >= 0) {
        ...
        if (highlightTag == null) {
          highlightTag = html.getHighlighter().addHighlight(pos, pos + s.length(),
  ...
  public static String openInternalBrowser(Window parent, String title, String url, int width,
      int height) {
  ...
  public static boolean openInternalBrowserForAction(Window parent, String title, String url,
      String closeButtonText, String closeButtonImage) {
    BrowserDialog dlg = createBrowserDialog(parent, title, url, 0, 0, 0, closeButtonText,
        closeButtonImage);
  ...
  public static void openExternalBrowser(Window parent, String url) {
    if (Daten.efaConfig.getValueBrowser().length() > 0 &&
      ...
      try {
        ...
        LogString.logWarning_cantExecCommand(Daten.efaConfig.getValueBrowser(),
            International.getString("für Browser"), ee.toString());
