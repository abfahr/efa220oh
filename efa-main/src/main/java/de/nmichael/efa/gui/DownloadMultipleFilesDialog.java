...
public class DownloadMultipleFilesDialog extends BaseDialog {
  ...
  public DownloadMultipleFilesDialog(JDialog parent, String downloadName,
      ...
      boolean exit) {
    super(parent, International.getStringWithMnemonic("Download"), International
        .getStringWithMnemonic("Download starten"));
  ...
  protected void iniDialog() throws Exception {
    ...
    output.append(International.getMessage(
        "Folgende Dateien werden jetzt installiert (Gesamtgröße: {size} Bytes):", size) + "\n");
    for (int c = 0; c < remoteFileNames.length; c++) {
      if (remoteFileNames[c] != null) {
  ...
  public boolean cancel() {
        ...
        if (f.isFile()) {
          if (f.length() != fileSizes[i]) {
            s += remoteFileNames[i]
                + ": "
                +
                International.getMessage("ungültige Dateigröße (erwartet war: {size})",
                    fileSizes[i]) + "\n";
            ok = false;
          }
        } else {
    ...
    if (ok) {
      Dialog.infoDialog(International.getMessage("{name} wurde erfolgreich installiert. ",
          downloadName)
          + "\n" +
    ...
    for (int c = 0; c < remoteFileNames.length; c++) {
      if (remoteFileNames[c] != null) {
        ...
        output.append(International
            .getMessage("Starte Download von {file} ...", remoteFileNames[c]) + "\n");
        output.doLayout();
...
class AfterDownloadImpl implements ExecuteAfterDownload {
  ...
  public void notify(boolean ok, String s) {
    ...
    if (--fileCount == 0) {
      if (errorCount == 0) {
        Dialog.infoDialog(LogString.operationSuccessfullyCompleted(International
            .getString("Download")));
      } else {
