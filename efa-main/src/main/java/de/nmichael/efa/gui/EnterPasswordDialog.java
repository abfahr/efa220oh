...
public class EnterPasswordDialog extends BaseDialog {
  ...
  public void closeButton_actionPerformed(ActionEvent e) {
      ...
      if (newPassword) {
        if (s.length() < MIN_PASSWORD_LENGTH) {
          Dialog.error(International.getMessage(
              "Das Paßwort muß mindestens {n} Zeichen lang sein!", MIN_PASSWORD_LENGTH));
        }
        if (s.indexOf(" ") >= 0) {
        ...
        if (merkmale < 3) {
          Dialog
          .error(International
              .getString("Das Paßwort muß mindestens Zeichen aus drei der insgesamt vier Zeichengruppen "
    ...
    if (newPassword) {
      ...
      if (!s1.equals(s2)) {
        Dialog
        .error(International.getString("Die beiden eingegebenen Paßwörter sind verschieden."));
