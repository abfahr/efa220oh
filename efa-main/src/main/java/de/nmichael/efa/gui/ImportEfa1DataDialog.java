...
import de.nmichael.efa.data.importefa1.ImportMetadata;
import de.nmichael.efa.data.importefa1.ImportTask;
...
public class ImportEfa1DataDialog extends StepwiseDialog {
        ...
        for (File f : files) {
          if (f.isFile() && f.getName().startsWith(".efa_")) {
            try {
              ...
              BufferedReader efa1 = new BufferedReader(new InputStreamReader(new FileInputStream(
                  f.getAbsolutePath()), Daten.ENCODING_ISO));
              String s;
  ...
  void reinitializeItems() {
    ...
    if (step == 0) {
      ...
      checkImportData(importData, dir, new Adressen("adressen.efd"), ImportMetadata.TYPE_ADRESSEN,
      ...
      checkImportData(importData, dir, new Gruppen("gruppen.efg"), ImportMetadata.TYPE_GRUPPEN,
          International.getString("Gruppen"));
      checkImportData(importData, dir, new Mannschaften("mannschaften.efm"),
          ImportMetadata.TYPE_MANNSCHAFTEN, International.getString("Mannschaften"));
  ...
  private void checkImportData(HashMap<String, ImportMetadata> importData, String dir,
      DatenListe datenListe, int type, String description) {
  ...
  private void recursiveAddLogbook(HashMap<String, ImportMetadata> importData, String fname) {
    ...
    if (EfaUtil.canOpenFile(fb.getFileName()) && fb.readFile()) {
      ImportMetadata meta = new ImportMetadata(ImportMetadata.TYPE_FAHRTENBUCH, fb,
      ...
      while (d != null) {
        ...
        if (d.get(Fahrtenbuch.DATUM).length() > 0
            && (meta.firstDate == null || EfaUtil.secondDateIsAfterFirst(d.get(Fahrtenbuch.DATUM),
                meta.firstDate.toString()))) {
          meta.firstDate = DataTypeDate.parseDate(d.get(Fahrtenbuch.DATUM));
        }
        if (d.get(Fahrtenbuch.DATUM).length() > 0
            && (meta.lastDate == null || EfaUtil.secondDateIsAfterFirst(meta.lastDate.toString(),
      ...
      Boote boote = new Boote(EfaUtil.makeFullPath(EfaUtil.getPathOfFile(fname),
          fb.getDaten().bootDatei));
      boote.dontEverWrite();
      if (EfaUtil.canOpenFile(boote.getFileName()) && boote.readFile()) {
        meta.numRecBoats = boote.countElements();
      }
      Mitglieder mitglieder = new Mitglieder(EfaUtil.makeFullPath(EfaUtil.getPathOfFile(fname),
          fb.getDaten().mitgliederDatei));
      ...
      Ziele ziele = new Ziele(EfaUtil.makeFullPath(EfaUtil.getPathOfFile(fname),
          fb.getDaten().zieleDatei));
      ziele.dontEverWrite();
      if (EfaUtil.canOpenFile(ziele.getFileName()) && ziele.readFile()) {
        meta.numRecDests = ziele.countElements();
      }
      StatSave stat = new StatSave(EfaUtil.makeFullPath(EfaUtil.getPathOfFile(fname),
          fb.getDaten().statistikDatei));
      stat.dontEverWrite();
  ...
  boolean finishButton_actionPerformed(ActionEvent e) {
    ...
    ProgressDialog progressDialog = new ProgressDialog(this,
        International.getString("Daten importieren"), importTask, false);
    importTask.start();
  ...
  public String getNewestLogbookName() {
    return (importTask != null && !importTask.isRunning() ? importTask.getNewestLogbookName()
        : null);
  }
