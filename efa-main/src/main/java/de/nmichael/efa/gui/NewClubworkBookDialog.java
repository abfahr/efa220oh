...
public class NewClubworkBookDialog extends StepwiseDialog {
    ...
    switch (step) {
      case 0:
        return International
            .getString("Ein Vereinsarbeitsbuch sollte üblicherweise alle Arbeiten eines Jahr enthalten. "
                +
                "Vereine mit mehreren Bootshäusern sollten pro Bootshaus ein eigenes Vereinsarbeitsbuch (in demselben Projekt) anlegen.");
      case 1:
        return International
            .getString("Bitte wähle den Zeitraum für dieses Vereinsarbeitsbuch aus. efa wird später nur Arbeit "
                +
  ...
  void initializeItems() {
    ...
    String year = Integer.toString(cal.get(Calendar.MONTH) + 1 <= 10 ?
        cal.get(Calendar.YEAR) : cal.get(Calendar.YEAR) + 1); // current year until October, year+1
    // else

    item = new ItemTypeString(CLUBWORKNAME, year, IItemType.TYPE_PUBLIC, "0",
        International.getString("Name des Vereinsarbeitsbuch"));
    ((ItemTypeString) item)
    ...
    items.add(item);
    item = new ItemTypeString(CLUBWORKDESCRIPTION, "", IItemType.TYPE_PUBLIC, "0",
        International.getString("Beschreibung"));
    items.add(item);

    // Items for Step 1
    item = new ItemTypeDate(DATEFROM, new DataTypeDate(1, 1, EfaUtil.string2int(year, 2010)),
        IItemType.TYPE_PUBLIC, "1", International.getString("Beginn des Zeitraums"));
    ((ItemTypeDate) item).setNotNull(true);
    items.add(item);
    item = new ItemTypeDate(DATETO, new DataTypeDate(31, 12, EfaUtil.string2int(year, 2010)),
        IItemType.TYPE_PUBLIC, "1", International.getString("Ende des Zeitraums"));
    ((ItemTypeDate) item).setNotNull(true);
    items.add(item);

    items.add(item = new ItemTypeDouble(ProjectRecord.DEFAULTCLUBWORKTARGETHOURS, 0, 0,
        ItemTypeDouble.MAX,
        IItemType.TYPE_PUBLIC, "1",
        International.getString("Sollstunden für Vereinsarbeit")));

    items.add(item = new ItemTypeDouble(ProjectRecord.TRANSFERABLECLUBWORKHOURS, 0, 0,
        ItemTypeDouble.MAX,
        IItemType.TYPE_PUBLIC, "1",
        International.getString("Übertragbare Vereinsarbeitsstunden pro Zeitraum")));

    items.add(item = new ItemTypeDouble(ProjectRecord.FINEFORTOOLITTLECLUBWORK, 0, 0,
        ItemTypeDouble.MAX,
        IItemType.TYPE_PUBLIC, "1",
  ...
  boolean finishButton_actionPerformed(ActionEvent e) {
    ...
    ItemTypeDate clubworkFromTo = (ItemTypeDate) getItemByName(DATETO);
    // ItemTypeBoolean clubworkswitch = (ItemTypeBoolean)getItemByName(AUTOMATICCLUBWORKSWITCH);
    // ItemTypeStringList clubworkSwitchBoathouse =
    // (ItemTypeStringList)getItemByName(CLUBWORKSWITCHBOATHOUSE);
    ...
    try {
      ...
      // Daten.project.getBoathouseName(EfaUtil.string2int(clubworkSwitchBoathouse.getValue(),
      // -1)));
      // if (bh != null) {
