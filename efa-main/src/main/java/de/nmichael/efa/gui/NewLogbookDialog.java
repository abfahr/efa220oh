...
public class NewLogbookDialog extends StepwiseDialog {
    ...
    switch (step) {
      case 0:
        return International
            .getString("Ein Fahrtenbuch sollte üblicherweise alle Fahrten eines Jahr enthalten. "
                +
                "Vereine mit mehreren Bootshäusern sollten pro Bootshaus ein eigenes Fahrtenbuch (in demselben Projekt) anlegen.");
      case 1:
        return International
            .getString("Bitte wähle den Zeitraum für Fahrten dieses Fahrtenbuches aus. efa wird später nur Fahrten "
                +
                "innerhalb dieses Zeitraums für dieses Fahrtenbuch zulassen.");
      case 2:
        return International
            .getString("Durch die Konfiguration eines Fahrtenbuchwechsels ist es möglich, ein neues Fahrtenbuch "
                +
  ...
  void initializeItems() {
    ...
    item = new ItemTypeString(LOGBOOKNAME, year, IItemType.TYPE_PUBLIC, "0",
    ...
    items.add(item);
    item = new ItemTypeString(LOGBOOKDESCRIPTION, "", IItemType.TYPE_PUBLIC, "0",
  ...
  boolean checkInput(int direction) {
    ...
    if (step == 1) {
      ...
      if (logFromDate != null && logswitch != null) {
        logswitch.setDescription(International.getMessage(
            "Fahrtenbuchwechsel automatisch zum {datum}",
      ...
      if (logswitch.getValue()) {
        ...
        if (switchok) {
          Dialog.infoDialog(International.getMessage(
              "Fahrtenbuchwechsel zum {datum} konfiguriert.",
              logFromDate.getDate().toString()));
        } else {
          Dialog
          .error(International
              .getString("Fahrtenbuchwechsel konnte nicht konfiguriert werden.")
              + "\n"
              +
