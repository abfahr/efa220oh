...
public class NewProjectDialog extends StepwiseDialog implements IItemListener {
    ...
    switch (step) {
      case 0:
        return International
            .getString("In efa2 werden alle Daten in Projekten zusammengefaßt. Üblicherweise solltest Du für einen Verein "
                +
      ...
      case 1:
        return International
            .getString("Bitte wähle, wo die Daten des Projekts gespeichert werden sollen") + ":\n" +
      ...
      case 2:
        return International
            .getString("Bitte gib an, wo die Daten gespeichert werden sollen und wie der Zugriff erfolgen soll");
      case 3:
        return International.getString("Bitte vervollständige die Angaben zu Deinem Verein.");
      case 4:
        return International
            .getString("Bitte gib an, in welchen Dachverbänden Dein Verein Mitglied ist, und (falls vorhanden) die Benutzernamen "
                +
    ...
    if (step == 1) {
      ...
      if (!item.getValue().equals(IDataAccess.TYPESTRING_FILE_XML) &&
        ...
        Dialog.error(International.getMessage(
            "Die ausgewählte Option '{option}' wird zur Zeit noch nicht unterstützt.",
            item.getValue()));
        item.requestFocus();
        return false;
      }
      ...
      itemsToBeDeleted.add(new ItemTypeBoolean(GUIITEM_CREATE_WATERS_LIST, true,
          IItemType.TYPE_PUBLIC, "3", ""));
      itemsToBeDeleted
      .add(new ItemTypeLabel(GUIITEM_NODATA_LABEL1, IItemType.TYPE_PUBLIC, "3", ""));
      itemsToBeDeleted
      .add(new ItemTypeLabel(GUIITEM_NODATA_LABEL2, IItemType.TYPE_PUBLIC, "4", ""));
  ...
  public void itemListenerAction(IItemType itemType, AWTEvent event) {
    if (itemType.getName().equals(ProjectRecord.EFAONLINECONNECT)
  ...
  boolean finishButton_actionPerformed(ActionEvent e) {
    ...
    // The storageType of the project's content (set through prj.setProjectStorageType(storageType))
    // may differ.
    ...
    try {
      ...
      // Project Properties
      prj.setProjectDescription(((ItemTypeString) getItemByName(ProjectRecord.DESCRIPTION))
          .getValue());
      ...
      if (getItemByName(ProjectRecord.STORAGELOCATION) != null) {
        prj.setProjectStorageLocation(((ItemTypeString) getItemByName(ProjectRecord.STORAGELOCATION))
            .getValue());
      }
      if (getItemByName(ProjectRecord.STORAGEUSERNAME) != null) {
        prj.setProjectStorageUsername(((ItemTypeString) getItemByName(ProjectRecord.STORAGEUSERNAME))
            .getValue());
      }
      if (getItemByName(ProjectRecord.STORAGEPASSWORD) != null) {
        prj.setProjectStoragePassword(((ItemTypeString) getItemByName(ProjectRecord.STORAGEPASSWORD))
            .getValue());
      }
      if (getItemByName(ProjectRecord.REMOTEPROJECTNAME) != null) {
        prj.setProjectRemoteProjectName(((ItemTypeString) getItemByName(ProjectRecord.REMOTEPROJECTNAME))
            .getValue());
      }
      if (getItemByName(ProjectRecord.EFAONLINECONNECT) != null) {
        prj.setProjectEfaOnlineConnect(((ItemTypeBoolean) getItemByName(ProjectRecord.EFAONLINECONNECT))
            .getValue());
      }
      if (getItemByName(ProjectRecord.EFAONLINEUSERNAME) != null) {
        prj.setProjectEfaOnlineUsername(((ItemTypeString) getItemByName(ProjectRecord.EFAONLINEUSERNAME))
            .getValue());
      }
      if (getItemByName(ProjectRecord.EFAONLINEPASSWORD) != null) {
        prj.setProjectEfaOnlinePassword(((ItemTypePassword) getItemByName(ProjectRecord.EFAONLINEPASSWORD))
            .getValue());
      }
      ...
      if (storageType == IDataAccess.TYPE_FILE_XML) {
        ...
        prj.setClubAddressStreet(((ItemTypeString) getItemByName(ProjectRecord.ADDRESSSTREET))
            .getValue());
        prj.setClubAddressCity(((ItemTypeString) getItemByName(ProjectRecord.ADDRESSCITY))
            .getValue());
        ...
        // Club Properties (2)
        prj.setClubGlobalAssociationName(((ItemTypeString) getItemByName(ProjectRecord.ASSOCIATIONGLOBALNAME))
            .getValue());
        prj.setClubGlobalAssociationMemberNo(((ItemTypeString) getItemByName(ProjectRecord.ASSOCIATIONGLOBALMEMBERNO))
            .getValue());
        prj.setClubGlobalAssociationLogin(((ItemTypeString) getItemByName(ProjectRecord.ASSOCIATIONGLOBALLOGIN))
            .getValue());
        prj.setClubRegionalAssociationName(((ItemTypeString) getItemByName(ProjectRecord.ASSOCIATIONREGIONALNAME))
            .getValue());
        prj.setClubRegionalAssociationMemberNo(((ItemTypeString) getItemByName(ProjectRecord.ASSOCIATIONREGIONALMEMBERNO))
            .getValue());
        prj.setClubRegionalAssociationLogin(((ItemTypeString) getItemByName(ProjectRecord.ASSOCIATIONREGIONALLOGIN))
            .getValue());
        if (getItemByName(ProjectRecord.MEMBEROFDRV) != null) {
          prj.setClubMemberOfDRV(((ItemTypeBoolean) getItemByName(ProjectRecord.MEMBEROFDRV))
              .getValue());
        }
        if (getItemByName(ProjectRecord.MEMBEROFSRV) != null) {
          prj.setClubMemberOfSRV(((ItemTypeBoolean) getItemByName(ProjectRecord.MEMBEROFSRV))
              .getValue());
        }
        if (getItemByName(ProjectRecord.MEMBEROFADH) != null) {
          prj.setClubMemberOfADH(((ItemTypeBoolean) getItemByName(ProjectRecord.MEMBEROFADH))
              .getValue());
        }
        if (getItemByName(ProjectRecord.KANUEFBUSERNAME) != null) {
          prj.setClubKanuEfbUsername(((ItemTypeString) getItemByName(ProjectRecord.KANUEFBUSERNAME))
              .getValue());
        }
        if (getItemByName(ProjectRecord.KANUEFBPASSWORD) != null) {
          prj.setClubKanuEfbPassword(((ItemTypeString) getItemByName(ProjectRecord.KANUEFBPASSWORD))
              .getValue());
        }
      }
        ...
        try {
          if (Waters.getResourceTemplate(International.getLanguageID()) != null) {
            ...
            if (createWatersList != null && createWatersList.getValue() &&
              ...
              .addAllWatersFromTemplate(International.getLanguageID());
            }
          }
