...
public class OpenProjectOrLogbookDialog extends BaseDialog implements IItemListener {
  ...
  void configureButton_actionPerformed(ActionEvent e) {
    ...
    if (type == Type.project) {
      ...
      ProjectEditDialog dlg = new ProjectEditDialog(this, prj, ProjectRecord.GUIITEMS_SUBTYPE_ALL,
          admin);
      dlg.showDialog();
    }
    ...
    if (type == Type.logbook) {
      ...
      ProjectEditDialog dlg = new ProjectEditDialog(this, Daten.project, logbook,
          ProjectRecord.GUIITEMS_SUBTYPE_ALL, admin);
      dlg.showDialog();
    }
    ...
    if (type == Type.clubwork) {
      ...
      ProjectEditDialog dlg = new ProjectEditDialog(this, Daten.project, clubwork,
          ProjectRecord.GUIITEMS_SUBTYPE_ALL, admin);
      dlg.showDialog();
    }
  }
  ...
  void deleteButton_actionPerformed(ActionEvent e) {
    ...
    if (type == Type.project) {
      ...
      message = International
          .getMessage("Möchtest Du das Projekt '{name}' wirklich löschen?", name)
          + "\n"
          +
          (prj.getProjectStorageType() == IDataAccess.TYPE_FILE_XML ?
              International
              .getString("Alle Daten des Projekts gehen damit unwiederbringlich verloren!")
              :
                International
                .getString("Es wird nur die Projektkonfiguration gelöscht. Die Daten selbst bleiben erhalten."));
    }
    if (type == Type.logbook) {
      message = International.getMessage("Möchtest Du das Fahrtenbuch '{name}' wirklich löschen?",
          name)
          + "\n"
          +
          International
          .getString("Alle Fahrten des Fahrtenbuchs gehen damit unwiederbringlich verloren!");
    }
    if (type == Type.clubwork) {
      message = International.getMessage(
          "Möchtest Du das Vereinsarbeitsbuch '{name}' wirklich löschen?", name)
          + "\n"
          + International
          .getString("Alle Einträge des Vereinsarbeitsbuch gehen damit unwiederbringlich verloren!");
    }
    if (message == null) {
      return;
    }
    ...
    if (type == Type.project) {
      try {
        if (prj.getProjectStorageType() == IDataAccess.TYPE_FILE_XML) {
          res = Dialog
              .yesNoDialog(
                  International.getString("Bist Du sicher?"),
                  International
                  .getString("Sämtliche Daten dieses Projekts (Fahrtenbücher, Mitglieder, Boote, Ziele etc.) werden unwiederbringlich gelöscht!")
                  + "\n" +
