...
public class ShowLogbookDialog extends BaseDialog implements IItemListener {
  ...
  public ShowLogbookDialog(Frame parent, Logbook logbook) {
    super(parent, International.getStringWithMnemonic("Fahrtenbuch"), International
  ...
  protected void iniDialog() throws Exception {
    if (Daten.efaConfig.getValueEfaDirekt_startMaximized()) {
      ...
      this.setPreferredSize(new Dimension((Dialog.screenSize.width * 98) / 100,
          (Dialog.screenSize.height * 95) / 100));
    }
    ...
    showOnlyNumber = new ItemTypeInteger("SHOWONLYNUMBER",
        (Daten.efaConfig.getValueEfaDirekt_anzFBAnzeigenFahrten() > 0 ? Daten.efaConfig
            .getValueEfaDirekt_anzFBAnzeigenFahrten() : 50), 1,
            (Daten.efaConfig.getValueEfaDirekt_maxFBAnzeigenFahrten() > 0 ? Daten.efaConfig
                .getValueEfaDirekt_maxFBAnzeigenFahrten() : 100),
                IItemType.TYPE_PUBLIC, "", International.getString("Anzahl der anzuzeigenden Fahrten"));
    showOnlyNumber.registerItemListener(this);

    showAlsoIncomplete = new ItemTypeBoolean("SHOWALSOINCOMPLETE",
        Daten.efaConfig.getValueEfaDirekt_FBAnzeigenAuchUnvollstaendige(),
        IItemType.TYPE_PUBLIC, "",
        International.getString("auch Fahrten von Booten anzeigen, die noch unterwegs sind"));
  ...
  public void updateTable(int max, boolean alsoIncomplete) {
    ...
    try {
      ...
      while (k != null) {
        ...
        fahrten[c][3] = new TableItem(r.getCoxAsName(), obmann == 0); // (obmann == 0 ? BOLD : "") +
        // d.get(Fahrtenbuch.STM);
        ...
        for (int j = 0, i = 1; i <= LogbookRecord.CREW_MAX; i++) {
          ...
          if (s != null && s.length() > 0) {
            mRowData[j++][0] = new TableItem(s, obmann == i); // (obmann == ii+1 ? BOLD : "") +
            // d.get(i);
          }
        }
    ...
    table.getColumn(International.getString("Steuermann")).setCellRenderer(
        new HighlightTableCellRenderer());
    table.getColumn(International.getString("Mannschaft")).setCellRenderer(
        new TableInTableRenderer());
    ...
    if (width < this.getSize().width - 20 || width > this.getSize().width) { // beim ersten Aufruf
      // steht Tabellenbreite
      // noch nicht (korrekt)
      // zur Verfügung, daher
      // dieser Plausi-Check
  ...
  void changeRequest(DataTypeIntString entryNo) {
    ...
    try {
      ...
      ItemTypeTextArea text = new ItemTypeTextArea("TEXT",
          EfaUtil.wrapString(
              International.getMessage("Korrekturwunsch für Fahrtenbuch-Eintrag {entry}",
      ...
      from.setAutoCompleteData(new AutoCompleteList(Daten.project.getPersons(false).data(), now,
          now));
      ...
      if (SimpleInputDialog.showInputDialog(this,
        ...
        Dialog.infoDialog(International
            .getString("Vielen Dank, dein Korrekturwunsch wurde übermittelt."));
      }
    } catch (Exception e) {
      Logger.logdebug(e);
    }
