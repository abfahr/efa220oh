...
public class FixLogbookDialog extends BaseDialog implements IItemListener {
      ...
      for (DataKey k = it.getFirst(); k != null; k = it.getNext()) {
        ...
        for (int i = 0; i <= LogbookRecord.CREW_MAX; i++) {
          ...
          if (name != null && name.length() > 0 && r.getCrewId(i) == null) {
            ...
            ci.addField(r, LogbookRecord.getCrewFieldNameId(i));
            changes.put(name, ci);
  ...
  private void fixSelectedItems() {
    ...
    ProgressDialog progressDialog = new ProgressDialog(this,
        International.getString("Einträge korrigieren"), task, false);
  ...
  class ChangeItem {
    ...
    public void setGuiFields(ItemTypeBoolean fixme, ItemTypeLabel infoEntry,
        ItemTypeLabel infoCount, IItemType item) {
      this.fixme = fixme;
