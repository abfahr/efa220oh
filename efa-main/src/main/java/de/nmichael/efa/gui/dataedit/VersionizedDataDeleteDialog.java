...
public class VersionizedDataDeleteDialog extends BaseDialog implements IItemListener {
  ...
  public VersionizedDataDeleteDialog(Frame parent, String name) {
    super(parent, International.getString("Daten löschen"), International
        .getStringWithMnemonic("Löschen"));
    this.name = name;
  }
  ...
  public void closeButton_actionPerformed(ActionEvent e) {
    ...
    if (deleteAll.getValue()) {
      if (Dialog.yesNoCancelDialog(
          International.getString("Wirklich löschen?"),
          International.getString("Wenn der Datensatz vollständig gelöscht wird, können auch " +
              ...
              International.getMessage(
                  "Möchtest Du den Datensatz '{record}' wirklich vollständig löschen?", name)) != Dialog.YES) {
        return;
      }
    }
    if (deleteAll.getValue()) {
