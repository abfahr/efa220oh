...
public class AutoCompletePopupWindow extends JWindow {
  ...
  private void showAtTextField(JTextField field) {
    if (showingAt == field) {
      // Unter Windows bewirkt toFront(), daß der ursprüngliche Frame den Fokus verliert, daher muß
      // unter Windows darauf verzichtet werden
    ...
    } catch (Exception ee) { // nur zur Sicherheit: Es gibt seltene Exceptions in efa, die keiner
      // Stelle im Code zugeordnet werden können und hierher kommen könnten
    }
  }
  ...
  public static void showAndSelect(JTextField field, AutoCompleteList list, String eintrag,
      AutoCompletePopupWindowCallback callback) {
    try {
      if (window == null) {
