...
public class TableSorter extends TableMap {
  ...
  public int compareRowsByColumn(int row1, int row2, int column) {
     ...
     * object to return many values. The Number subclasses in the JDK are immutable and so will not
     * be used in this way but other subclasses of Number might want to do this to save space and
  ...
  public void shuttlesort(int from[], int to[], int low, int high) {
     ...
     * subset are already ordered. If so, no further comparisons are needed; the sub-array can just
     * be copied. The array must be copied rather than assigned otherwise sister calls in the
     * recursion might get out of sinc. When the number of elements is three they are partitioned so
     * that the first set, [low, mid), has one element and and the second, [mid, high), has two. We
     * skip the optimisation when the number of elements is three or less as the first compare in
     * the normal merge will produce the same sequence of steps. This optimisation seems to be
     * worthwhile for partially ordered lists but some analysis is needed to find out how the
     * performance drops to Nlog(N) as the initial order diminishes - it may drop very quickly.
     */
