...
public class AlertWidget extends Widget implements IItemFactory {
  ...
  public boolean isShowForBoat(ItemTypeItemList list, int i, String bType, String bSeats) {
    try {
      return isInSelection(bType,
          ((ItemTypeMultiSelectList) list.getItem(i, PARAM_BOATTYPE)).getValues())
          &&
          isInSelection(bSeats,
              ((ItemTypeMultiSelectList) list.getItem(i, PARAM_BOATSEATS)).getValues());
    } catch (Exception e) {
      return false;
    }
  }
  ...
  /**
   * While we're loading EfaConfig, we don't have loaded EfaTypes yet, so we need to update the list
   * data for boat types and seats after loading EfaTypes.
   */
