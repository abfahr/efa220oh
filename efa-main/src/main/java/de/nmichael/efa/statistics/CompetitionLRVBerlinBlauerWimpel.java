...
public class CompetitionLRVBerlinBlauerWimpel extends Competition {
  ...
  public void calculate(StatisticsRecord sr, StatisticsData[] sd) {
    ...
    if (!checkWettZeitraum(sr.sCompYear, sr.sStartDate, sr.sEndDate,
        WettDefs.LRVBERLIN_BLAUERWIMPEL)) {
    ...
    for (int i = 0; i < sd.length && i < anzWertung; i++) {
      if (!sr.sIsOutputCompWithoutDetails) {
        if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett
          ...
          sr.pAdditionalTable1[i][2] = DataTypeDistance.getDistance(sd[i].distance)
    ...
    if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett) {
      ...
      sr.pAdditionalTable2[1][1] = DataTypeDistance.getDistance(totalDistanceInDefaultUnit)
          .getStringValueInKilometers(true, 0, 0);
      sr.pAdditionalTable2[2][0] = "Durchschnittskilometer pro Ruderer:";
      sr.pAdditionalTable2[2][1] = DataTypeDistance.getDistance(
          totalDistanceInDefaultUnit / anzWertung).getStringValueInKilometers(true, 0, 1);
    }
