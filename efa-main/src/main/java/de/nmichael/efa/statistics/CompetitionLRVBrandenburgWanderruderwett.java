...
public class CompetitionLRVBrandenburgWanderruderwett extends Competition {
  ...
  public void calculate(StatisticsRecord sr, StatisticsData[] sd) {
    ...
    if (!checkWettZeitraum(sr.sCompYear, sr.sStartDate, sr.sEndDate,
        WettDefs.LRVBRB_WANDERRUDERWETT)) {
    ...
    for (int g = 0; g < gruppen.length; g++) {
      ...
      sr.pCompGroupNames[g][2] = gruppen[g].km + " Kilometer"
          + "; davon " + gruppen[g].zusatz + " Gigboot-Kilometer und " + gruppen[g].zusatz2
          + " Wanderfahrt-Tage";
      ...
      for (int i = 0; i < sd.length; i++) {
        ...
        if (!sd[i].sYearOfBirth.equals("")
          ...
          String[][] wafa = new String[gruppen[g].zusatz2][6]; // 3 Einträge mit jeweils
          // LfdNr/Abfahrt/Ankunft/Ziel/Km/Bemerk
          int fahrtnr = 0;
          for (int nr = 0; nr < wafa.length; nr++) { // max. für 3 auszufüllende Felder Fahrten
            // suchen
            DRVFahrt drvel = null;
            if (fahrtnr < keys.length) {
              do {
                drvel = wanderfahrten.get(keys[fahrtnr]);
                // wenn ein Ruderer an einer Mehrtagesfahrt (als einzelne Etappen eingetragen) nur
                // einen Tag
              ...
              if (!sr.sIsOutputCompWithoutDetails && erfuellt) {
                ...
                if (sr.sIsOutputCompShort) {
                  if (sr.sIsOutputCompAdditionalWithRequirements) {
                    participant.sAdditional = DataTypeDistance.getDistanceFromMeters(
                        sd[i].compData.gigbootmeters).getStringValueInKilometers()
                        + "/" + gruppen[g].zusatz + " Gig-Km; "
                        + anzWafaTage + "/" + gruppen[g].zusatz2 + " WafaTage";
                  } else {
                    participant.sAdditional = DataTypeDistance.getDistanceFromMeters(
                        sd[i].compData.gigbootmeters).getStringValueInKilometers()
                        + " Gig-Km; " + anzWafaTage + " WafaTage";
                  }
                } else {
                  participant.sAdditional = "davon "
                      + DataTypeDistance.getDistanceFromMeters(sd[i].compData.gigbootmeters)
