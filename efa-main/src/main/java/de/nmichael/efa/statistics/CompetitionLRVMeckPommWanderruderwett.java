...
public class CompetitionLRVMeckPommWanderruderwett extends Competition {
  ...
  public void calculate(StatisticsRecord sr, StatisticsData[] sd) {
    ...
    if (!checkWettZeitraum(sr.sCompYear, sr.sStartDate, sr.sEndDate,
        WettDefs.LRVMVP_WANDERRUDERWETT)) {
    ...
    for (int g = 0; g < gruppen.length; g++) {
      ...
      sr.pCompGroupNames[g][2] = gruppen[g].km
          + " Kilometer"
          + (gruppen[g].zusatz > 0 ? "; davon " + gruppen[g].zusatz
              + " Gigboot-Kilometer in mind. " + gruppen[g].zusatz2 + " Fahrten" : // bis 2008
              " in mind. " + gruppen[g].zusatz2 + " Fahrten")
          + // ab 2009
          (gruppen[g].zusatz3 > 0 ? " mit " + gruppen[g].zusatz3 + " Fahrten von mind. 20 Km" : "")
          + (gruppen[g].zusatz3 > 0 && gruppen[g].zusatz4 > 0 ? " und"
              : (gruppen[g].zusatz4 > 0 ? " mit" : ""))
      ...
      for (int i = 0; i < sd.length; i++) {
        ...
        if (!sd[i].sYearOfBirth.equals("")
          ...
          Hashtable<String, DRVFahrt> wanderfahrten = CompetitionDRVFahrtenabzeichen
              .getWanderfahrten(sd[i]);
          ...
          int wafaAnzMTour = 0;
          int jumAnz = 0;
          if (sr.sCompYear >= LRVMVP_NEU) { // erst ab 2009 haben LRV und DRV den gleichen Zeitraum,
            // sonst ist dieser Abgleich nicht möglich!
            for (Object key : keys) {
              ...
              if (!sr.sIsOutputCompWithoutDetails && erfuellt) {
                participant.sAdditional = "davon "
                    + (sr.sCompYear < LRVMVP_NEU
                        ...
                        : "")
                    + sd[i].compData.gigboot20plus + " Fahrten >= 20 Km und "
                    + sd[i].compData.gigboot30plus + " Fahrten >= 30 Km";
                ...
                if (fahrtIDs != null) {
                  ...
                  for (int j = 0; j < fahrtIDs.length; j++) {
                    ...
                    participant.sDetailsArray[j][4] = r.getDistance().getStringValueInKilometers(
                        true, 0, 1);
                ...
                if (sr.sIsOutputCompShort) {
                  if (sr.sIsOutputCompAdditionalWithRequirements) {
                    ...
                    participant.sAdditional = (sr.sCompYear < LRVMVP_NEU ? DataTypeDistance
                        .getDistanceFromMeters(sd[i].compData.gigbootmeters)
                        .getStringValueInKilometers(false, 0, 0)
                        + " Gig-Km; " : "")
                        + sd[i].compData.gigbootanz + " Fahrten; "
                  ...
                  participant.sAdditional = "davon "
                      + (sr.sCompYear < LRVMVP_NEU
                          ...
                          : "")
                      + sd[i].compData.gigbootanz
                      + " Fahrten mit " + sd[i].compData.gigboot20plus + " Fahrten >= 20 Km und "
                      + sd[i].compData.gigboot30plus + " Fahrten >= 30 Km";
                }
          ...
          if (sd[i].sYearOfBirth.equals("")
              && Daten.wettDefs.erfuellt(WettDefs.LRVMVP_WANDERRUDERWETT, sr.sCompYear, 0,
