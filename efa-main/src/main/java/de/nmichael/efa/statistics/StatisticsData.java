  ...
  public void getAllDestinationAreas() {
    ...
    for (int i = 0; sessionHistory != null && i < sessionHistory.size(); i++) {
      ...
      if (r != null && r.zielfahrt != null) {
        ...
        // r.getDestinationAndVariantName() + " (" + r.getDistance().toString() + ") -> " +
        // r.zielfahrt.getBereiche() );
      }
    }
    destinationAreas.reduceToMinimun();
  }
  ...
  public void createStringOutputValues(StatisticsRecord sr, int absPos, String sPosition) {
    ...
    if (sr.sIsAggrDistance) {
      ...
      if (sr.sIgnoreNullValues && distance == 0) {
        ...
        this.sDistance = DataTypeDistance.getDistance(this.distance).getStringValueInDefaultUnit(
            sr.sDistanceWithUnit, 0, decimals);
      }
    }
    if (sr.sIsAggrRowDistance) {
      ...
      if (sr.sIgnoreNullValues && rowdistance == 0) {
        ...
        this.sRowDistance = DataTypeDistance.getDistance(this.rowdistance)
    ...
    if (sr.sIsAggrCoxDistance) {
      ...
      if (sr.sIgnoreNullValues && coxdistance == 0) {
        ...
        this.sCoxDistance = DataTypeDistance.getDistance(this.coxdistance)
            .getStringValueInDefaultUnit(sr.sDistanceWithUnit, 0, decimals);
      }
    }
    ...
    if (sr.sIsAggrAvgDistance) {
      if (this.count > 0) {
        ...
        this.sAvgDistance = DataTypeDistance.getDistance(this.avgDistance)
            .getStringValueInDefaultUnit(sr.sDistanceWithUnit, 1, 1);
      } else {
        this.avgDistance = 0;
        this.sAvgDistance = "";
      }
    ...
    if (sr.sIsAggrSpeed) {
      if (this.duration > 0) {
        ...
        this.sSpeed = DataTypeDistance.getDistance(this.speed).getStringValueInDefaultUnit(
            sr.sDistanceWithUnit, 1, 1)
            + (sr.sDistanceWithUnit ? "/h" : "");
      } else {
        this.speed = 0;
        this.sSpeed = "";
    ...
    if (sr.sIsAggrClubworkRelativeToTarget) {
      if (sr.sIgnoreNullValues && clubworkOverUnderCarryOver == 0) {
        ...
        this.sClubworkRelativeToTarget = "" + Math.round(this.clubworkRelativeToTarget * 100)
            / 100d;
      }
    }
    if (sr.sIsAggrClubworkOverUnderCarryOver) {
      if (this.isSummary || (sr.sIgnoreNullValues && clubworkOverUnderCarryOver == 0)) {
        ...
        this.sClubworkOverUnderCarryOver = "" + Math.round(this.clubworkOverUnderCarryOver * 100)
            / 100d;
      }
    }
