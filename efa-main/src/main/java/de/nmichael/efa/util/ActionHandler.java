...
// @i18n complete
/**
 * General helper class which convieniently allows user-interface handling. Components or keystrokes
 * are coupled with function calls; in this they are Java methods via reflection like C function
 * pointers.
 *
...
public class ActionHandler extends AbstractAction {
  ...
  /**
   * Constructor Last change: 25-04-2003
   *
   * @param methodContainer class containing the functions to call; in most cases <this>
   */
  ...
  /**
   * Assign swing components with function calls aka methods. The argument is an twodimensional
   * object array; the first index of the first dimension contain the JComponents, the second index
   * of the first dimension the name of the method as String. For example: The object array new
   * Object[][]{{new JButton("Test")},{"testRoutine"}} as argument assign the Button named "Test"
   * with an routine <public testRoutine(ActionEvent evt)> protected and private methods are
   * allowed, too. Last change: 25-04-2003
   *
   * @params aObj Object array[2][Number of entries]
   * @throws IllegalArgumentException - if the number of entries differ in the seperate dimensions -
   *           if so. try to reassign the same component
  ...
  public void addActionListenerTable(Object[][] aObj)
    ...
    for (int i = 0; i < iLength; i++) {
      ...
      if (!methods.containsKey(aObj[0][i])) {
        Method m = methodObject.getClass().getDeclaredMethod(routineName,
  ...
  /**
   * Assign swing components with swing actions The argument is an twodimensional object array like
   * the argument in addActionListener In this case the second index of the first dimension contain
   * swing actions. Last change: 25-04-2003
   *
   * @params aObj Object array[2][Number of entries]
   * @throws IllegalArgumentException - if the number of entries differ in the seperate dimensions -
   *           if so. try to reassign the same component
  ...
  /**
   * Assign keystrokes of a component with focus to function calls aka methods. the first index of
   * the first dimension contain the JComponents, the second index of the first dimension the name
   * of the method as String. For example: The object array new Object[][]{{new
   * JButton("Test")},{"testRoutine"}} as argument assign the Button named "Test" with an routine
   * <public testRoutine(ActionEvent evt)> protected and private methods are allowed, too. Last
   * change: 25-04-2003
   *
   * @params comp JComponent with focus.
   * @params inputcondition Change condition when the action is triggered; use e.g.
   *         JComponent.WHEN_IN_FOCUSED_WINDOW
   * @params keystrokes String array with names which must be accepted by KeyStroke.getKeyStroke()
   * @params methodNames String array with function names: same syntax as in addActionListener
   * @throws IllegalArgumentException - if the number of entries differ in the seperate dimensions -
   *           if so. try to reassign the same component - if inputcondition is not valid
   * @throws NoSuchMethodException -if the method is not found.
   */
  public void addKeyActions(JComponent comp, int inputcondition, String[] keystrokes,
      String[] methodNames)
          throws IllegalArgumentException, NoSuchMethodException {
    ...
    for (int i = 0; i < keystrokes.length; i++) {
      ...
      Method m = methodObject.getClass().getDeclaredMethod(methodNames[i],
          new Class[] { ActionEvent.class });
      methods.put(id, m);
    }// for
  }// addKeyActions
  ...
  public String addKeyAction(JComponent comp, int inputcondition, String keystroke,
      String methodName)
          throws IllegalArgumentException, NoSuchMethodException {
    if (!(inputcondition == JComponent.WHEN_IN_FOCUSED_WINDOW
        || inputcondition == JComponent.WHEN_FOCUSED
    ...
    comp.getActionMap().put(id, a);
    Method m = methodObject.getClass().getDeclaredMethod(methodName,
        new Class[] { ActionEvent.class });
    methods.put(id, m);
    return id;
  }
  ...
  /**
   * Action listener interface which invoke the stored matching method. Bugfix: 1.1 no return in
   * KEYSTROKE_ACTION if-branch resulting in throwing an IllegalArgumentException Last change:
   * 25-04-2003
   */
  @Override
  ...
  /**
   * Helper class for assigning keystrokes to the ActionHandler Last change: 25-04-2003
   */
