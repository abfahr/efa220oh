...
public class International {
  ...
  private static boolean LOG_MISSING_KEYS = false; // default for production: false (requires
    ...
    try {
      if (Daten.efaBaseConfig != null && Daten.efaBaseConfig.language != null &&
        ...
        if (Logger.isTraceOn(Logger.TT_INTERNATIONALIZATION)) {
          Logger.log(Logger.DEBUG, Logger.MSG_INTERNATIONAL_DEBUG,
              "Using configured language setting: " + Daten.efaBaseConfig.language);
        }
      ...
      if (Logger.isTraceOn(Logger.TT_INTERNATIONALIZATION)) {
        Logger.log(Logger.DEBUG, Logger.MSG_INTERNATIONAL_DEBUG,
            "Language is now: " + locale.getDisplayName());
      }

      bundle = ResourceBundle.getBundle(BUNDLE_NAME, locale);
      numberFormat = NumberFormat.getNumberInstance(locale);
      msgFormat = new MessageFormat("", locale);
      decimalSeparator = ((DecimalFormat) numberFormat).getDecimalFormatSymbols()
          .getDecimalSeparator();
      Daten.EFA_SHORTNAME = "efa";
      Daten.EFA_LONGNAME = Daten.EFA_SHORTNAME + " - "
          + International.getString("elektronisches Fahrtenbuch");
      ...
      Logger.log(Logger.ERROR, Logger.MSG_INTERNATIONAL_FAILEDSETUP,
          "Failed to set up internationalization: " + e.toString()); // no need for translation
  ...
  public static void initialize() {
    ...
    try {
      if (Daten.efaBaseConfig != null && Daten.efaBaseConfig.language == null) {
        if (Logger.isTraceOn(Logger.TT_INTERNATIONALIZATION)) {
          Logger.log(Logger.DEBUG, Logger.MSG_INTERNATIONAL_DEBUG,
              "No preferred Language configured!");
        }
        ...
        if (Daten.isGuiAppl() && bundles != null && bundles.length > 0) {
          ...
          listDisplay[0] = International.getString("Default"); // must be in English (in case user's
          ...
          if (SimpleInputDialog.showInputDialog((JFrame) null,
              International.getString("Select Language"), // must be in English (in case user's
              // language is not supported)
              languages)) {
  ...
  public static String[] getLanguageBundles() {
    if (Daten.efaProgramDirectory == null) {
      // not yet initialized
      // This can happen when we try to translate something before Daten.initialize() calls
      // iniLanguageSupport()
      return null;
    }
    if (Logger.isTraceOn(Logger.TT_INTERNATIONALIZATION)) {
  ...
  private static String getString(String s, boolean defaultIfNotFound, boolean includingMnemonics,
      boolean messageString, ResourceBundle useBundle) {
    if (bundle == null) {
      initialize();
    }
    try {
      ...
      if (defaultIfNotFound) {
        if (LOG_MISSING_KEYS) {
          Logger.log(Logger.WARNING, Logger.MSG_INTERNATIONAL_MISSINGKEY, "Missing Key: "
              + makeKey(s)); // no need for translation!
        }
        ...
        if (messageString) {
          ...
          while ((p1 = s.indexOf("{")) >= 0 && (p2 = s.indexOf("}")) >= 0) {
            ...
            if (p1 > 0 && s.charAt(p1 - 1) == '\'' && p2 + 1 < s.length()
                && s.charAt(p2 + 1) == '\'') {
              s = s.substring(0, p1) + "'@@[@@" + i++ + "@@]@@'" + s.substring(p2 + 1); // substitute
              // '{varname}'
              // by
              // ''{1}''
            } else {
  ...
  /**
   * Retrieves an internationalized string. This method strips any mnemonics from the translated
  ...
  /**
   * Retrieves an internationalized variant of a string. This method is intended for situations
   * where the same key may be translated in different ways depending on the context it is being
   * used in. For such situations, an additional variant discriminator may be specified to
   * distinguish (otheriwse same) keys in different contexts. If no translation for this key
   * including its variant discriminator is found, this method will attempt to return a translation
   * for the key without the variant discriminator. This method strips any mnemonics from the
   * translated string before returning it.
   *
   * @param s key to be retrieved
  ...
  /**
   * Retrieves an internationalized string. This method keeps all mnemonics in the translated string
   * that is returned.
   *
   * @param s key to be retrieved
  ...
  /**
   * Retrieves an internationalized variant of a string (see getString(String, String). This method
   * keeps all mnemonics in the translated string that is returned.
   *
   * @param s key to be retrieved
   * @param variant discriminator to specify the context of the key
   * @return the translation for "s___variant", if existing; translation for "s" otherwise --
   *         including mnemonics marked with "&".
   */
  ...
  private static String getMessage(String s, Object[] args) {
    ...
    try {
      ...
      if (LOG_MISSING_KEYS) {
        Logger.log(Logger.WARNING, Logger.MSG_INTERNATIONAL_INCORRECTKEY,
            "Incorrect Compound Key: " + s); // no need for translation!
      }
      if (STACKTRACE_MISSING_KEYS) {
        EfaErrorPrintStream.ignoreExceptions = true;
        e.printStackTrace();
      ...
      return (MARK_MISSING_KEYS ? "#" : "") + s + ": " + getArrayStrings(args)
          + (MARK_MISSING_KEYS ? "#" : "");
    }
  }

  /**
   * Retrieves an internationalized compound message. All arguments inside this message have to be
   * masked by "{arg}", where "arg" may be any text that is not further interpreted or used (other
   * than being part of the key). For each of the arguments masked by "{arg}", and argument arg1,
   * arg2, ...., argn has to be supplied.
   *
   * @param s key to be retrieved
  ...
  public static String getMessage(String s, int arg1, int arg2, int arg3, int arg4) {
    Object[] args = { "dummy", Integer.toString(arg1), Integer.toString(arg2),
        Integer.toString(arg3), Integer.toString(arg4) };
    return getMessage(s, args);
  }

  public static String getMessage(String s, int arg1, int arg2, int arg3, int arg4, int arg5) {
    Object[] args = { "dummy", Integer.toString(arg1), Integer.toString(arg2),
        Integer.toString(arg3), Integer.toString(arg4), Integer.toString(arg5) };
    return getMessage(s, args);
  }
