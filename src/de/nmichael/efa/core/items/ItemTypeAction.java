...
public class ItemTypeAction extends ItemTypeButton {
  ...
  private void resetTypesToDefault() {
    ...
    int count = 0;
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesBoat(), EfaTypes.CATEGORY_BOAT,
        true);
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesNumSeats(),
        EfaTypes.CATEGORY_NUMSEATS, true);
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesRigging(),
    ...
    Dialog.infoDialog(International.getMessage(
        "Es wurden {count} Typen neu generiert (sichtbar im Expertenmodus).", count));
  ...
  private void addAllDefaultSessionTypes() {
    ...
    // addAllDefaultSessionTypes() is only called from buttonHit(ActionEvent) if the configured
    // action is
    // ACTION_TYPES_RESETTODEFAULT.
    // This is (and must!) only be the case if dlg is a EfaConfigFrame!
    EfaConfigDialog efaConfigFrame = null;
  ...
  private void generateTypes(int selection) {
    ...
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesBoat(), EfaTypes.CATEGORY_BOAT,
        false);
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesNumSeats(),
        EfaTypes.CATEGORY_NUMSEATS, false);
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesRigging(),
        EfaTypes.CATEGORY_RIGGING, false);
    count += addNewTypes(Daten.efaTypes, efaConfigFrame.getTypesCoxing(), EfaTypes.CATEGORY_COXING,
        false);

    Dialog.infoDialog(International.getMessage(
        "Es wurden {count} Bootstypen neu generiert (sichtbar im Expertenmodus).", count));
    efaConfigFrame.updateGui(false);
  }

  private int addNewTypes(EfaTypes types, ItemTypeHashtable<String> cfgTypes, String cat,
      boolean overwrite) {
    int count = 0;
