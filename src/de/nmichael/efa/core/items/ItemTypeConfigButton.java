...
public class ItemTypeConfigButton extends ItemType {
  ...
  public IItemType copyOf() {
    return new ItemTypeConfigButton(name, text, bcolor, show, isChangeableText, isChangeableColor,
        isChangeableShow, type, category, description);
  }
  ...
  public int displayOnGui(Window dlg, JPanel panel, int x, int y) {
    ...
    panel.add(field, new GridBagConstraints(1, y, 1, 1, 0.0, 0.0,
        GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(padYbefore, 0, padYafter,
            (checkbox == null ? padXafter : 0)), 0, 0));
    if (checkbox != null) {
  ...
  private void buttonHit(ActionEvent e) {
    if (!isChangeableText && !isChangeableColor) {
      return;
    }
    if (!isChangeableColor) {
      chooseText();
