        ...
        if (r.getDate() != null && r.getDate().isSet()) {
          if (latest == null
              ...
              || (r.getDate().equals(latest.getDate())
                  ...
                  && (latest.getStartTime() == null || r.getStartTime().isAfter(
                      latest.getStartTime())))) {
            latest = r;
          }
        }
  ...
  public void preModifyRecordCallback(DataRecord record, boolean add, boolean update, boolean delete)
    ...
    if (add || update) {
      ...
      if (r.getBoatVariant() == IDataAccess.UNDEFINED_INT &&
          r.getBoatId() != null) {
        try {
          BoatRecord br = r.getPersistence().getProject().getBoats(false)
              .getBoat(r.getBoatId(), r.getValidAtTimestamp());
          if (br != null) {
    ...
    if (delete) {
      assertNotReferenced(record, getProject().getBoatStatus(false),
          new String[] { BoatStatusRecord.ENTRYNO }, true,
