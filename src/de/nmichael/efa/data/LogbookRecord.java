...
public class LogbookRecord extends DataRecord {
  ...
  public static void initialize() {
    ...
    f.add(ENTRYID);
    t.add(IDataAccess.DATA_INTSTRING);
    f.add(DATE);
    ...
    f.add(DESTINATIONID);
    t.add(IDataAccess.DATA_UUID);
    f.add(DESTINATIONNAME);
    t.add(IDataAccess.DATA_STRING);
    f.add(DESTINATIONVARIANTNAME);
    t.add(IDataAccess.DATA_STRING);
    f.add(WATERSIDLIST);
    t.add(IDataAccess.DATA_LIST_UUID);
    f.add(WATERSNAMELIST);
  ...
  /*
   * public String getDestinationAndVariantName(long validAt, boolean prefixedByWaters) { return
   * getDestinationAndVariantName(validAt, prefixedByWaters, true); }
   */
  ...
  public String[] getEquivalentFields(String fieldName) {
    ...
    for (int i = 0; i <= CREW_MAX; i++) {
      if (fieldName.equals(EXP_CREW + i) || fieldName.equals(getCrewFieldNameId(i))
          || fieldName.equals(getCrewFieldNameName(i))) {
        return new String[] { getCrewFieldNameId(i), getCrewFieldNameName(i) };
      }
    }
    if (fieldName.equals(EXP_DESTINATION) || fieldName.equals(DESTINATIONID)
        || fieldName.equals(DESTINATIONNAME)) {
      return new String[] { DESTINATIONID, DESTINATIONNAME };
    }
    if (fieldName.equals(EXP_WATERSLIST) || fieldName.equals(WATERSIDLIST)
        || fieldName.equals(WATERSNAMELIST)) {
  ...
  public String[] getFieldNamesForTextExport(boolean includingVirtual) {
    ...
    for (String f : allFields) {
      ...
      for (int i = 0; i <= CREW_MAX; i++) {
        if (f.equals(LogbookRecord.getCrewFieldNameId(i))) {
      ...
      for (int i = 0; i <= CREW_MAX; i++) {
        if (f.equals(LogbookRecord.getCrewFieldNameName(i))) {
          found = true;
          break;
        }
      }
  ...
  public Vector<IItemType> getGuiItems(AdminRecord admin) {
    /**
     * CAUTION: The implementation of this method is *NOT* complete. It is *NOT* intended to display
     * the current record properly for editing. All it does is return dummy GUI fields which are
     * used, for example, by BatchEditDialog, to figure out data content and labels for the various
     * fields.
     */
    ...
    autoDestinations.setDataAccess(getPersistence().getProject().getDestinations(false).data(), 0,
        Long.MAX_VALUE);
    AutoCompleteList autoWaters = new AutoCompleteList();
    autoWaters.setDataAccess(getPersistence().getProject().getWaters(false).data(), 0,
        Long.MAX_VALUE);

    IItemType item;
    Vector<IItemType> v = new Vector<IItemType>();

    v.add(item = new ItemTypeString(LogbookRecord.ENTRYID, "", IItemType.TYPE_PUBLIC, null,
        International.getStringWithMnemonic("Lfd. Nr.")));
    v.add(item = new ItemTypeLabel(LogbookRecord.OPEN, IItemType.TYPE_PUBLIC, null, International
        .getStringWithMnemonic("Fahrt offen (Boot unterwegs)")));
    v.add(item = new ItemTypeDate(LogbookRecord.DATE, new DataTypeDate(), IItemType.TYPE_PUBLIC,
        null, International.getStringWithMnemonic("Datum")));
    v.add(item = new ItemTypeDate(LogbookRecord.ENDDATE, new DataTypeDate(), IItemType.TYPE_PUBLIC,
        null, International.getStringWithMnemonic("Enddatum")));
    v.add(item = new ItemTypeStringAutoComplete(LogbookRecord.EXP_BOAT, "", IItemType.TYPE_PUBLIC,
        null, International.getStringWithMnemonic("Boot"), true));
    ...
    v.add(item = new ItemTypeStringAutoComplete(LogbookRecord.ALLCREWNAMES, "",
        IItemType.TYPE_INTERNAL, null,
        International.getString("Mannschaft"), true));
    ((ItemTypeStringAutoComplete) item).setAutoCompleteData(autoPersons);
    v.add(item = new ItemTypeStringAutoComplete(LogbookRecord.EXP_COX, "", IItemType.TYPE_PUBLIC,
        null, International.getStringWithMnemonic("Steuermann"), true));
    ((ItemTypeStringAutoComplete) item).setAutoCompleteData(autoPersons);
    for (int i = 1; i <= CREW_MAX; i++) {
      v.add(item = new ItemTypeStringAutoComplete(LogbookRecord.EXP_CREW + i, "",
