...
public class Persons extends StorageObject {
  ...
  public Persons(int storageType,
    ...
    super(storageType, storageLocation, storageUsername, storagePassword, storageObjectName,
        DATATYPE, International.getString("Personen"));
  ...
  public PersonRecord getPerson(UUID id, long earliestValidAt, long latestValidAt,
      long preferredValidAt) {
    try {
      return (PersonRecord) data().getValidNearest(PersonRecord.getKey(id, preferredValidAt),
  ...
  public PersonRecord getPersonWithInputShortcut(String myInputShortcut, long validAt) {
    try {
      DataKey<?, ?, ?>[] keys = data().getByFields(
          new String[] { PersonRecord.INPUTSHORTCUT },
          staticPersonRecord.getQualifiedNameValues(myInputShortcut), validAt);
      if (keys == null || keys.length < 1) {
        return null;
      }
      if (validAt < 0 && keys.length > 1) {
  ...
  public Vector<PersonRecord> getAllPersons(long validFrom, long validUntil, boolean alsoDeleted,
      boolean alsoInvisible) {
    try {
      Vector<PersonRecord> v = new Vector<PersonRecord>();
      DataKeyIterator it = data().getStaticIterator();
      ...
      while (k != null) {
        PersonRecord r = (PersonRecord) data().get(k);
        if (r != null
            && (r.isInValidityRange(validFrom, validUntil) || (r.getDeleted() && alsoDeleted))
            && (!r.getInvisible() || alsoInvisible)) {
          v.add(r);
        }
        k = it.getNext();
  ...
  public void preModifyRecordCallback(DataRecord record,
    ...
    if (delete) {
      assertNotReferenced(record, getProject().getFahrtenabzeichen(false),
          new String[] { FahrtenabzeichenRecord.PERSONID });
  ...
  class MergePersonsProgressTask extends ProgressTask {
    ...
    public void run() {
      ...
      try {
        ...
        for (UUID mergeID : mergeIDs) {
          DataRecord[] mergePerson = persons.data()
              .getValidAny(PersonRecord.getKey(mergeID, 0));
