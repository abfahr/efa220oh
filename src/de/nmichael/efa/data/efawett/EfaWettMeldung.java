...
public class EfaWettMeldung {
  ...
  public boolean drvint_fahrtErfuellt = false; // betrachte "fahrt" als erfüllt (egal, was dort
  // angegeben ist)
  public boolean drvint_wirdGewertet = false;
  public boolean drvint_wirdGewertetExplizitGesetzt = false; // wird beim Lesen der Datei auf true
  // gesetzt, wenn diese
  ...
  public EfaWettMeldung(EfaWettMeldung ewm) {
    ...
    for (int i = 0; ewm.fahrt != null && this.fahrt != null && i < this.fahrt.length
        && i < ewm.fahrt.length; i++) {
      for (int j = 0; ewm.fahrt[i] != null && this.fahrt[i] != null && j < this.fahrt[i].length
          && j < ewm.fahrt[i].length; j++) {
