...
public class ImportBoats extends ImportBase {
  ...
  public boolean runImport() {
    try {
      ...
      while (d != null) {
        ...
        if (newBoatRecord) {
          try {
            ...
            updateHashes(k, d);
            logDetail(International
                .getMessage("Importiere Eintrag: {entry}", boatRecord.toString()));
          } catch (Exception e) {
            logError(International.getMessage(
                "Import von Eintrag fehlgeschlagen: {entry} ({error})", boatRecord.toString(),
                e.toString()));
            Logger.logdebug(e);
          }
        } else {
          if (changedBoatRecord) {
            try {
              ...
              logDetail(International.getMessage("Importiere Eintrag: {entry}",
                  boatRecord.toString()));
            } catch (Exception e) {
              if (newBoatRecord) {
                logError(International.getMessage(
                ...
                logWarning(International.getMessage(
                    ...
                    boatRecord.toString(), e.toString()));
              }
              Logger.logdebug(e);
            }
          } else {
            logDetail(International.getMessage("Identischer Eintrag: {entry}",
                boatRecord.toString()));
          }
        }
      ...
      // mark all boats that have *not* been imported with this run, but still have a valid version,
      // as deleted
      ...
      while (key != null) {
        ...
        if (br != null && importedBoats.get(br.getId()) == null) {
          try {
            ...
            logError(International.getMessage(
                "Gültigkeit ändern von Eintrag fehlgeschlagen: {entry} ({error})", br.toString(),
                e.toString()));
            Logger.logdebug(e);
          }
        }
      ...
      logError(International.getMessage("Import von {list} aus {file} ist fehlgeschlagen.",
          getDescription(), boote.getFileName()));
      logError(e.toString());
      Logger.logdebug(e);
      return false;
    }
  ...
  public static boolean runPostprocessing(Hashtable<DataKey, String> boatsAllowedGroups,
      Hashtable<DataKey, String> boatsRequiredGroup,
