...
public class ImportFahrtenabzeichen extends ImportBase {
  ...
  public boolean runImport() {
    try {
      de.nmichael.efa.efa1.Fahrtenabzeichen fahrtenabzeichen1 = new de.nmichael.efa.efa1.Fahrtenabzeichen(
          efa1fname);
        ...
        if (personID != null) {
          ...
          try {
            if (d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.ANZABZEICHEN).length() > 0) {
              r.setAbzeichen(EfaUtil.string2int(
                  d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.ANZABZEICHEN), 0));
            }
            if (d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.ANZABZEICHENAB).length() > 0) {
              r.setAbzeichenAB(EfaUtil.string2int(
                  d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.ANZABZEICHENAB), 0));
            }
            if (d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.GESKM).length() > 0) {
              r.setKilometer(EfaUtil.string2int(d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.GESKM),
                  0));
            }
            if (d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.GESKMAB).length() > 0) {
              r.setKilometerAB(EfaUtil.string2int(
                  d.get(de.nmichael.efa.efa1.Fahrtenabzeichen.GESKMAB), 0));
            }
            ...
            try {
              ...
              logError(International.getMessage(
                  "Import von Eintrag fehlgeschlagen: {entry} ({error})", "DRV-Schlüssel " + key,
                  ekey.toString()));
            }
          } catch (Exception e) {
            logError(International.getMessage(
