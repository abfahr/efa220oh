...
public abstract class DataRecord implements Cloneable, Comparable {
  ...
  protected static MetaData constructMetaData(String dataType, Vector<String> fields,
    ...
    types.add(IDataAccess.DATA_LONGINT);
    // LastModified must always be the last field; this class's set(int, Object) method implicitly
    // uses this to update the timestamp!
  ...
  /*
   * public DataRecord clone() { try { return this.getClass().getConstructor(this.getClass(),
   * metaData.getClass()).newInstance(this, metaData); } catch (Exception e) { throw new
  ...
  protected void set(int fieldIdx, Object data, boolean updateTimestamp) {
    ...
    if (data != null) {
      ...
      if (data instanceof String && type != IDataAccess.DATA_STRING) {
        ...
        if (data == null) {
          throw new IllegalArgumentException(persistence.toString()
              + ": Data could not be transformed to Type " + persistence.data().getTypeName(type)
              + " for Data Field " + metaData.getFieldName(fieldIdx) + ".");
        }
      }
      switch (type) {
        case IDataAccess.DATA_STRING:
          if (!(data instanceof String)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type STRING expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_INTEGER:
          if (!(data instanceof Integer)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type INTEGER expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_LONGINT:
          if (!(data instanceof Long)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type LONGINT expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_DOUBLE:
          if (!(data instanceof Double)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type DOUBLE expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_DECIMAL:
          if (!(data instanceof DataTypeDecimal)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type DECIMAL expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_DISTANCE:
          if (!(data instanceof DataTypeDistance)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type DISTANCE expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_BOOLEAN:
          if (!(data instanceof Boolean)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type BOOLEAN expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_DATE:
          if (!(data instanceof DataTypeDate)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type DATE expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_TIME:
          if (!(data instanceof DataTypeTime)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type TIME expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_UUID:
          if (!(data instanceof UUID)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type UUID expected for Data Field " + metaData.getFieldName(fieldIdx)
                + ".");
          }
          break;
        case IDataAccess.DATA_INTSTRING:
          if (!(data instanceof DataTypeIntString)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type INTSTRING expected for Data Field "
                + metaData.getFieldName(fieldIdx) + ".");
          }
          break;
        case IDataAccess.DATA_PASSWORDH:
          if (!(data instanceof DataTypePasswordHashed)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type PASSWORDH expected for Data Field "
                + metaData.getFieldName(fieldIdx) + ".");
          }
          break;
        case IDataAccess.DATA_PASSWORDC:
          if (!(data instanceof DataTypePasswordCrypted)) {
            throw new IllegalArgumentException(persistence.toString()
                + ": Data Type PASSWORDC expected for Data Field "
                + metaData.getFieldName(fieldIdx) + ".");
          }
          break;
        case IDataAccess.DATA_VIRTUAL:
  ...
  public String addTextItemToList(String fieldName, String value) {
    try {
      ...
      DataTypeList listtmp = rtmp.getList(fieldName, getPersistence().data()
  ...
  public String encodeAsString() {
    ...
    for (int i = 0; i < getFieldCount(); i++) {
      ...
      if (o != null && getFieldType(i) != IDataAccess.DATA_VIRTUAL && !isDefaultValue(i)) {
        s.append("<" + getFieldName(i) + ">" + EfaUtil.escapeXml(o.toString()) + "</"
            + getFieldName(i) + ">");
      }
    }
    s.append("</" + ENCODING_RECORD + ">");
  ...
  public String[] getQualifiedNameFieldsTranslateVirtualToReal() {
    return getQualifiedNameFields(); // not supported - no qualified name; to be overridden in
  ...
  public void setLastModified() {
    synchronized (this.data) {
      this.data[getFieldCount() - 1] = System.currentTimeMillis(); // LastModified timestamp
    }
  }
  ...
  public boolean isInValidityRange(long validStart, long validEnd) {
    ...
    if ((rValidFrom >= validStart && rValidFrom <= validEnd) || // rValidFrom is in specified range
        (rValidUntil >= validStart && rValidUntil <= validEnd) || // rValidUntil is in specified
        // range
        (rValidFrom < validStart && rValidUntil > validEnd)) { // rValidFrom is before specified
      // range and rValidUntil is after
      // specified range
      return true;
    }
    return false;
  ...
  protected ItemTypeStringAutoComplete getGuiItemTypeStringAutoComplete(String name, UUID value,
    ...
    ItemTypeStringAutoComplete item = new ItemTypeStringAutoComplete(name, svalue, type, category,
        description, true);
