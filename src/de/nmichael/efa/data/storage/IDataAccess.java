...
public interface IDataAccess {
  ...
  /**
   * Sets the storage location (e.g. file system directory or database connect string) Examples:
   * /home/efa/data/ (with or without trailing file.separator) jdbc:mysql://localhost:1234/efa Note:
   * the combination StorageLocation/StorageObjectName.StorageObjectType must be unique!
   *
  ...
  /**
   * Returns the storage location (e.g. file system directory (always with trailing file.separator)
   * or database connect string)
   *
  ...
  /**
   * Sets the storage object name (e.g. file name (without file extension) or database table name
   * (just postfix)) Examples: 2009 (for a logbook with the name "2009") Note: the combination
   * StorageLocation/StorageObjectName.StorageObjectType must be unique!
   *
  ...
  /**
   * Returns the storage object name (e.g. file name (without file extension) or database table name
   * (just postfix))
   *
  ...
  /**
   * Sets the storage object type (e.g. logbook, members list, boat list, ...) which may be used as
   * a file extension or database table name prefix Examples: efb (for a logbook) efbm (for a
   * members list) efbb (for a boat list) Note: the combination
   * StorageLocation/StorageObjectName.StorageObjectType must be unique!
   *
  ...
  /**
   * Returns the storage object type (e.g. logbook, members list, boat list, ...) which may be used
   * as a file extension or databale table name prefix
   *
  ...
  /**
   * Sets an informal description of this storage object, e.g. "Logbook", to be displayed on GUI
   * items or in logfiles
   *
  ...
  /**
   * Returns an informal description of this storage object, e.g. "Logbook", to be displayed on GUI
   * items or in logfiles
   *
  ...
  /**
   * Tests whether a storage object already exists. For file systems, this tests whether the
   * associated file in the file system exists. In order to succeed, the storage object location,
   * object name and objecet type must have been specified before.
   *
  ...
  /**
   * Creates a new storage object (overwrites existing objects). For file systems, this method may
   * imply recursive creation of directories as well. In order to succeed, the storage object
   * location, object name and objecet type must have been specified before.
   *
  ...
  /**
   * Checks whether the storage object is currently being opened (i.e. within openStorageObject(),
  ...
  /**
   * Locks the entire storage object for exclusive write access. Locking only affects DML
   * operations. Storage Object operations (as for example closing the storage object) are still
   * permitted. Locking may time out depending on the implementation of the underlying storage
   * object.
   *
   * @return a lock ID
   * @throws Exception if the storage object is already locked or cannot be locked at the moment
   *           (e.g. because it has already been closed)
   */
  public long acquireGlobalLock() throws EfaException;

  /**
   * Locks one data record in the storage object for exclusive write access. Locking only affects
   * DML operations. Storage Object operations (as for example closing the storage object) are still
   * permitted. Locking may time out depending on the implementation of the underlying storage
   * object.
   *
   * @return a lock ID
  ...
  /**
   * Specifies the key fields for this storage object. The combination of key field values must be
   * unique in the storage object.
   *
  ...
  /**
   * Sets the meta data associated with this data access. This must be done prior to using the data
   * access.
   *
  ...
  /**
   * Adds a new data record to this storage object. This data record will be valid from timestamp t.
   * If another data record is already valid around t, its validity range will be automatically
   * adapted to end at t-1. Note: This method requires a global lock!
   *
   * @param record the data record to add
  ...
  /**
   * Adds a new data record to this storage object with a previously acquired global lock. This data
   * record will be valid from timestamp t. If another data record is already valid around t, its
   * validity range will be automatically adapted to end at t-1. Note: This method requires a global
   * lock!
   *
   * @param record the data record to add
   * @param t the ValidFrom timestamp
   ...
   * @throws Exception if any of the data record already exists or the operation fails for another
   *           reason
   */
   ...
   * @throws Exception if the data record does not exist, is locked or the operation fails for
   *           another reason
   */
  public void delete(DataKey key) throws EfaException;

  /**
   * Deletes an existing data record from this storage object with a previously acquired local or
   * global lock.
   *
   * @param key the key of the data record to delete
   * @param lockID an ID of a previously acquired local or global lock
  ...
  /**
   * Deletes an existing data record from this storage object. This method will adapt other data
   * records with adjacent validity ranges, depending on the value of merge: merge = 0: Don't merge,
   * leave a "hole" where no records are valid merge = -1: Merge left record, i.e. extend
   * InvalidFrom from previous record to the InvalidFrom of the record to be deleted. merge = 1:
   * Merge right record, i.e. lower ValidFrom from next record to the ValidFrom of the record to be
   * deleted. Note: This method requires a global lock!
   *
   * @param key the key of the data record to delete
  ...
  /**
   * Deletes an existing data record from this storage object with a previously acquired global
   * lock. This method will adapt other data records with adjacent validity ranges, depending on the
   * value of merge: merge = 0: Don't merge, leave a "hole" where no records are valid merge = -1:
   * Merge left record, i.e. extend InvalidFrom from previous record to the InvalidFrom of the
   * record to be deleted. merge = 1: Merge right record, i.e. lower ValidFrom from next record to
   * the ValidFrom of the record to be deleted. Note: This method requires a global lock!
   *
   * @param key the key of the data record to delete
   * @param merge specifies how to merge adjacent record's validity ranges
  ...
  /**
   * Deletes all versions of an existing data record from this storage object with a previously
   * acquired global lock. Depending on the parameter deleteAt, this method will if deleteAt == -1:
   * mark all versions of this record as deleted if deleteAt >= 0: - leave all versions before
   * deleteAt untouched - change the validity of the version that is currently valid at deleteAt to
   * become invalid at deleteAt - physically delete all versions which become valid after deleteAt,
   * unless this is the last version of this record (in this case, it will only be marked deleted to
   * make sure that at least one version still physically remains) Note: This method requires a
   * global lock!
   *
   * @param key the key of the data record to delete
  ...
  /**
   * Changes the validity range of an existing data record from this storage object. This method
   * will adapt other data records with adjacent validity ranges, if necessary.
   *
  ...
  /**
   * Changes the validity range of an existing data record from this storage object with a
   * previously acquired global lock. This method will adapt other data records with adjacent
   * validity ranges, if necessary. Note: This method requires a global lock!
   *
   * @param record the record to be changed
   * @param validFrom the new validFrom value
   * @param invalidFrom the new invalidFrom value
   ...
   *
   * @param key the key of the data record to retrieve (with or without the validity information)
   * @param earliestValidAt earlist timestamp which is accepted to fall into the validity range of
   *          the record
   * @param latestValidAt latest timestamp which is accepted to fall into the validity range of the
   *          record
  ...
  /**
   * Truncates all records from this storage objects and copies the data from source, thus
   * re-building this storage object from source' contents
   *
   * @param source
   * @throws EfaException
   */
  ...
  /*
   * public DataRecord getCurrent(DataKeyIterator it) throws EfaException; public DataRecord
   * getFirst(DataKeyIterator it) throws EfaException; public DataRecord getLast(DataKeyIterator it)
   * throws EfaException; public DataRecord getNext(DataKeyIterator it) throws EfaException; public
