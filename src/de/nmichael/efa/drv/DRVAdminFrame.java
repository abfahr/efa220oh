...
public class DRVAdminFrame extends JDialog implements ActionListener {
  ...
  private void jbInit() throws Exception {
    ...
    try {
      ...
      jPanel2.add(wettJahrButton, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0
          , GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(20, 20, 0, 20), 0,
          0));
      jPanel2.add(configButton, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0
          , GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 20, 0, 20), 0,
          0));
      ...
      jPanel2.add(updateButton, new GridBagConstraints(0, 4, 1, 1, 0.0, 0.0
          , GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 20, 20, 20), 0,
          0));
  ...
  void wettJahrButton_actionPerformed(ActionEvent e) {
    ...
    if (!EfaUtil.canOpenFile(mdatFA) || !EfaUtil.canOpenFile(mdatWS)) {
      ...
      try {
        ...
        if (!(new File(mdir)).isDirectory()) {
          ...
          Logger.log(Logger.INFO, "Neues Verzeichnis " + mdir + " für Wettbewerbsjahr " + j
              + " erstellt.");
          neuesDir = true;
        }
        ...
        if (!mIndex.readFile()) {
          ...
          Logger.log(Logger.INFO, "Neue Meldungen-Indexdatei " + mdatFA + " für Wettbewerbsjahr "
              + j + " (Fahrtenabzeichen) erstellt.");
        } else {
          Logger.log(Logger.INFO, "Meldungen-Indexdatei " + mdatFA + " für Wettbewerbsjahr " + j
              + " (Fahrtenabzeichen) geöffnet.");
        }
        ...
        if (!mIndex.readFile()) {
          ...
          Logger.log(Logger.INFO, "Neue Meldungen-Indexdatei " + mdatWS + " für Wettbewerbsjahr "
              + j + " (Wanderruderstatistik) erstellt.");
        } else {
          Logger.log(Logger.INFO, "Meldungen-Indexdatei " + mdatFA + " für Wettbewerbsjahr " + j
              + " (Wanderruderstatistik) geöffnet.");
        }
    ...
    Dialog.infoDialog("Wettbewerbsjahr ausgewählt",
        "Das ausgewählte Jahr für die Erfassung von Meldungen ist jetzt " + j + ".");
  }
  ...
  void datensicherungButton_actionPerformed(ActionEvent e) {
    ...
    try {
      ...
      directories.add(Daten.efaMainDirectory);
      selected.add(new Boolean(true));
      inclSubdirs.add(new Boolean(false));
      directories.add(Daten.efaDataDirectory);
      selected.add(new Boolean(true));
      inclSubdirs.add(new Boolean(false));
      if (Main.drvConfig.aktJahr != 0) {
        directories.add(Daten.efaDataDirectory + Main.drvConfig.aktJahr + Daten.fileSep);
        selected.add(new Boolean(true));
        inclSubdirs.add(new Boolean(true));
      }
      if ((new File(Daten.efaDataDirectory + "CA" + Daten.fileSep)).isDirectory()) {
        directories.add(Daten.efaDataDirectory + "CA" + Daten.fileSep);
        selected.add(new Boolean(true));
        inclSubdirs.add(new Boolean(true));
      }
      directories.add(Daten.efaCfgDirectory);
      selected.add(new Boolean(true));
      inclSubdirs.add(new Boolean(true));
