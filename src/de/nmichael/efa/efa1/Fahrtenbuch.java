...
public class Fahrtenbuch extends DatenListe {
    ...
    try {
      while ((s = freadLine()) != null && !s.startsWith("##ENDE_KONFIG")) {
        ...
        if (s.startsWith("MEHRTAGESFAHRT=")) {
          ...
          Mehrtagesfahrt m = new Mehrtagesfahrt(d.get(0), d.get(1), d.get(2), EfaUtil.string2int(
              d.get(3), 1), d.get(4), d.get(5).equals("+"));
  ...
  public synchronized boolean writeEinstellungen() {
    try {
      ...
      if (prevFb != null) {
        fwrite("VORHERIGESFB=" + prevFb + "\n");
      } else {
        fwrite("VORHERIGESFB=\n");
      }
      if (nextFb != null) {
        fwrite("NAECHSTESFB=" + nextFb + "\n");
      } else {
        fwrite("NAECHSTESFB=\n");
      }
      if (fbDaten.erstVorname) {
        fwrite("NAMENSANGABEN=VORNACH\n");
      } else {
        fwrite("NAMENSANGABEN=NACHVOR\n");
      }
      ...
      for (Object key2 : keys) {
        Mehrtagesfahrt m = (Mehrtagesfahrt) mehrtagesfahrten.get(key2);
        fwrite("MEHRTAGESFAHRT=" + m.name + "|" + m.start + "|" + m.ende + "|" + m.rudertage + "|"
            + m.gewaesser + "|" + (m.isEtappen ? "+" : "-") + "\n");
      }
  ...
  public void readZusatzdatenbanken(boolean neuErstellen) {
    ...
    if (fbDaten.bootDatei.equals("")) {
      ...
      if (!fbDaten.boote.readFile()) {
        fbDaten.bootDatei = null;
      }
    }

    // Mitglieder einlesen
    if (fbDaten.mitgliederDatei.equals("")) {
      fbDaten.mitglieder = null;
    } else {
      ...
      if (!fbDaten.mitglieder.readFile()) {
        fbDaten.mitgliederDatei = null;
      }
    }

    // Ziele einlesen
    if (fbDaten.zieleDatei.equals("")) {
      fbDaten.ziele = null;
    } else {
      ...
      if (!fbDaten.ziele.readFile()) {
        fbDaten.zieleDatei = null;
      }
    }

    // Statistiken einlesen
    if (fbDaten.statistikDatei.equals("")) {
      fbDaten.statistik = null;
    } else {
      ...
      if (!fbDaten.statistik.readFile()) {
        fbDaten.statistikDatei = null;
      }
    }
  }
  ...
  public String getNextFb(boolean absolut) {
    if (nextFb != null && nextFb.equals("")) {
      return nextFb;
    }
    if (nextFb != null
        || (openFile() && readEinstellungen() && closeFileWithoutHash() && nextFb != null)) {
      if (absolut) {
        return EfaUtil.makeFullPath(EfaUtil.getPathOfFile(dat), nextFb);
      } else {
        return nextFb;
      }
    }
  ...
  public String getPrevFb(boolean absolut) {
    if (prevFb != null && prevFb.equals("")) {
      return prevFb;
    }
    if (prevFb != null || (openFile() && readEinstellungen() && closeFile() && nextFb != null)) {
      if (absolut) {
        return EfaUtil.makeFullPath(EfaUtil.getPathOfFile(dat), prevFb);
      } else {
        return prevFb;
      }
    }
  ...
  public void addMehrtagesfahrt(String name, String start, String ende, int rudertage,
      String gewaesser, boolean isEtappen) {
  ...
  public void removeMehrtagesfahrt(String name) {
    if (mehrtagesfahrten.get(name) != null) {
      mehrtagesfahrten.remove(name);
    }
  }
  ...
  public String[] getAllMehrtagesfahrtNamen() {
    if (mehrtagesfahrten == null || mehrtagesfahrten.size() == 0) {
      return null;
    }
  ...
  private class MtourStrings implements Comparable {
    ...
    public int compareTo(Object o) throws ClassCastException {
      ...
      if (this.start == null || this.ende == null || b.start == null || b.ende == null) {
        return 0;
      }
      if (EfaUtil.secondDateIsAfterFirst(this.start, b.start)) {
        return -1;
      }
      if (!this.start.equals(b.start)) {
        return 1;
      }
      if (EfaUtil.secondDateIsAfterFirst(this.ende, b.ende)) {
        return -1;
      }
      if (!this.ende.equals(b.ende)) {
        return 1;
      }
