...
public class Mitglieder extends DatenListe {
  ...
  public void validateValues(DatenFelder d) {
    String s;
    // if ( !(s = d.get(JAHRGANG)).equals("") )
    // d.set(JAHRGANG,Integer.toString(EfaUtil.string2date(s,0,0,0).tag));
    if (!(s = d.get(JAHRGANG)).equals("")) {
      d.set(JAHRGANG, Integer.toString(EfaUtil.yy2yyyy(EfaUtil.string2date(s, 0, 0, 0).tag)));
    }
  }

  public static boolean isKmwettMelden(DatenFelder d) {
    if (d == null || d.get(KMWETT_MELDEN) == null) {
      return true;
    }
  ...
  public Vector<String> getAllNames(boolean onlyMembers) {
    ...
    while (d != null) {
      if (!onlyMembers || d.get(Mitglieder.VEREIN).length() == 0) {
        String name = getFullName(d.get(Mitglieder.VORNAME), d.get(Mitglieder.NACHNAME),
            d.get(Mitglieder.VEREIN));
        names.add(name);
      }
      d = getCompleteNext();
    }
  ...
  public static String[] zerlegeNamen(String s, boolean vorNach) {
    ...
    if (vorNach) { // erst Vorname
      if ((to = name.indexOf(" v. ")) >= 0 ||
          ((to = name.indexOf(" van ")) >= 0 && name.indexOf(" van Maren") < 0)) { // Titel
      ...
      if ((to = name.indexOf(",")) >= 0) { // normal
        ...
        if (to + 2 <= name.length()) {
          vor = name.substring(to + 2, name.length());
        } else {
          vor = "";
        }
      } else { // kein Vorname
  ...
  public static String getVorname(String s) {
    if (Fahrtenbuch.fahrtenbuch == null) {
      return "";
    }
  ...
  public static String getFullName(String vor, String nach, String ver) {
    if (Fahrtenbuch.fahrtenbuch == null) {
      return getFullName(vor, nach, ver, true);
    }
