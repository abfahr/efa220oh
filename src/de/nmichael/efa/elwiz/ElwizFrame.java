...
public class ElwizFrame extends JFrame {
  ...
  public ElwizFrame() {
    ...
    optionsHTML = XSLTReader.run(Daten.efaFormattingDirectory + "layout" + Daten.fileSep + "html"
        + Daten.fileSep + "elwiz.xml");
    if (optionsHTML == null) {
      Dialog.error("Es konnte kein XML-Parser geladen werden.");
      Daten.haltProgram(Daten.HALT_MISCONFIG);
    }
    optionsPDF = XSLTReader.run(Daten.efaFormattingDirectory + "layout" + Daten.fileSep + "pdf"
        + Daten.fileSep + "elwiz.xml");
    if (optionsPDF == null) {
      Dialog.error("Es konnte kein XML-Parser geladen werden.");
      Daten.haltProgram(Daten.HALT_MISCONFIG);
  ...
  private void jbInit() throws Exception {
    ...
    setIconImage(Toolkit.getDefaultToolkit().createImage(
        ElwizFrame.class.getResource("/de/nmichael/efa/img/efa_icon.gif")));
    contentPane = (JPanel) this.getContentPane();
    contentPane.setLayout(borderLayout1);
    this.setSize(new Dimension(600, 600));
    this.setTitle("elwiz - efa Layout Wizard");
    optionHTMLPanel.setLayout(gridBagLayout1);
    Mnemonics.setButton(this, createButton,
        International.getStringWithMnemonic("Stylesheet erstellen"));
    ...
    Mnemonics.setButton(this, jMenuItem1,
        International.getStringWithMnemonic("Stylesheet erstellen"));
          ...
          if (o.options != null) {
            for (int j = 0; j < o.options.size(); j++) {
              ...
              optionPanel.add(radio,
                  new GridBagConstraints(0, y++, 3, 1, 0.0, 0.0
                      , GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 50, 0, 0),
                      0, 0));

              butgr.add(radio);

              if (o.components == null) {
                o.components = new Vector();
              }
        ...
        case ElwizOption.O_VALUE:
          ...
          if (o.options != null) {
            for (int j = 0; j < o.options.size(); j++) {
              ...
              optionPanel.add(label3a,
                  new GridBagConstraints(0, y, 1, 1, 0.0, 0.0
                      , GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 50, 0, 0),
                      0, 0));
              optionPanel
              .add(textfield, new GridBagConstraints(1, y++, 1, 1, 0.0, 0.0
  ...
  void colorButton_actionPerformed(ActionEvent e) {
    ...
    if (color != null) {
      ...
      for (float element : rgb) {
        cstr += EfaUtil.hexByte((int) (element * 255));
      }
  ...
  void textfield_focusLost(FocusEvent e) {
    ...
    if (textfieldType.get(textfield) == null) {
      return;
    }
    ...
    if (type.equals("integer")) {
      textfield
      .setText(Integer.toString(EfaUtil.string2date(textfield.getText().trim(), 0, 0, 0).tag));
      return;
    }
    ...
    if (type.equals("color")) {
      if (colorButtons.get(textfield) == null) {
        return;
      }
  ...
  void createButton_actionPerformed(ActionEvent e) {
    ...
    if (optionHTMLPanel.isShowing()) {
      ...
      if (outfile != null) {
        XSLTWriter.run(infile, outfile, optionsHTML);
      }
    } else {
      ...
      if (outfile != null) {
        XSLTWriter.run(infile, outfile, optionsPDF);
      }
    }
  }
