...
public class EmilFrame extends JFrame {
  ...
  int currentTeilnehmerNummer = 0; // Position des Teilnehmers in der verketteten Liste, 0 ==
  // efw.meldung; 1 == efw.meldung.next usw.
  boolean geaendertEintrag = false;
  boolean geaendertDatei = false;
  boolean anStdCsvAngehaengt = false; // true, wenn aktuelle Daten bereits an Standard-CSV-Datei
  // angehängt wurden
  ...
  private void jbInit() throws Exception {
    ...
    meldegeld.setVisible(false); // @todo (P9) Emil Meldegeld (deaktiviert)
  ...
  void menuItemNeu_actionPerformed(ActionEvent e) {
    if (efw != null && !dateiAenderungenGespeichert()) {
      return;
    }

    if (efw != null) {
      this.setTitle(KURZTITEL + " - <neue Datei>");
    }
    efw = new EfaWett();
  ...
  void menuItemOeffnen_actionPerformed(ActionEvent e) {
    ...
    String dat = Dialog.dateiDialog(this, "Meldedatei öffnen", "efa Meldedatei (*.efw)", "efw",
        dir, false);
      ...
      try {
        if (!efw.readFile()) {
          Dialog.infoDialog("Fehler", "Datei '" + efw.datei + "' hat ungültiges Format!\n" +
              "Es können nur Dateien im Format " + EfaWett.EFAWETT + " gelesen werden!");
          efw = null;
        } else {
  ...
  void menuItemSpeichern_actionPerformed(ActionEvent e) {
    if (eintragAenderungenGespeichert()) {
      speichereDatei(false);
    }
  }
  ...
  void menuItemExport_actionPerformed(ActionEvent e) {
    if (efw == null || !eintragAenderungenGespeichert()) {
      return;
    }
    exportCVS();
  }
  ...
  void menuitemAddToCVS_actionPerformed(ActionEvent e) {
    ...
    try {
      ...
      if (efw.wettId != WettDefs.LRVBERLIN_BLAUERWIMPEL) {
        Dialog.infoDialog("Bestätigung", "Es wurden " + c + " von insgesamt " + getAnzTeilnehmer()
            + " Einträgen in die Datei\n'" + cfg.getStdCsv() + "'\ngeschrieben!");
      } else {
        Dialog.infoDialog("Bestätigung", "Es wurden die Daten für den Verein " + efw.verein_name
            + " in die Datei\n'" + cfg.getStdCsv() + "'\ngeschrieben!");
      }
  ...
  void wettJahr_focusLost(FocusEvent e) {
    if (wett.getSelectedIndex() == 0) {
      return;
    }
    TMJ tmj = EfaUtil.string2date(wettJahr.getText(), -1, 0, 0);
    int year = 2000;
    if (tmj.tag >= 0 && tmj.tag < 100) {
      tmj.tag += 1900;
    }
    if (tmj.tag >= 0 && tmj.tag < 1980) {
      tmj.tag += 100;
    }
    if (tmj.tag >= 0) {
      year = tmj.tag;
  ...
  void prevButton_actionPerformed(ActionEvent e) {
    ...
    int i = 0;
    while (i < currentTeilnehmerNummer && m != null) {
      i++;
      m = m.next;
    }
  ...
  void nextButton_actionPerformed(ActionEvent e) {
    ...
    if (currentTeilnehmer.next == null) {
      return;
    }
    if (!eintragAenderungenGespeichert()) {
      return;
  ...
  void newButton_actionPerformed(ActionEvent e) {
    ...
    if (m == null) {
      efw.meldung = currentTeilnehmer;
    } else {
      m.next = currentTeilnehmer;
    }
    setBlankTeilnehmer();
    geaendertDatei = true;
  }
  ...
  void deleteButton_actionPerformed(ActionEvent e) {
    ...
    if (currentTeilnehmer == null) {
      return;
    }
    if (Dialog.yesNoDialog("Wirklich löschen?",
        "Möchtest Du den aktuellen Eintrag wirklich löschen?") == Dialog.NO) {
      return;
    }

    if (currentTeilnehmerNummer == 0) {
      currentTeilnehmer = efw.meldung = efw.meldung.next;
    } else {
      // vorherigen Datensatz ausfindig machen
      EfaWettMeldung m = efw.meldung;
      int i = 0;
      while (i < currentTeilnehmerNummer - 1 && m != null && m.next != null) {
        i++;
        m = m.next;
      }
      if (m.next != null) {
  ...
  void jahrgang_focusLost(FocusEvent e) {
    if (jahrgang.getText().trim().equals("")) {
      jahrgang.setText("");
      return;
    }
  ...
  void kilometer_focusLost(FocusEvent e) {
    if (kilometer.getText().trim().equals("")) {
      kilometer.setText("");
      return;
    }
    TMJ tmj = EfaUtil.string2date(kilometer.getText(), 0, 0, 0);
    if (tmj.monat != 0) {
      kilometer.setText(tmj.tag + "," + tmj.monat);
    } else {
      kilometer.setText("" + tmj.tag);
    }
  }
  ...
  void fahrtDatumValidate(FocusEvent e) {
    ...
    TMJ tmj = EfaUtil.correctDate(feld.getText(), 0, 0,
        EfaUtil.string2date(wettJahr.getText(), 0, 0, 0).tag);
  ...
  void fahrtKmValidate(FocusEvent e) {
    ...
    if (tmj.monat != 0) {
      feld.setText(tmj.tag + "," + tmj.monat);
    } else {
      feld.setText("" + tmj.tag);
    }
  }
  ...
  void setBlankTeilnehmer() {
    ...
    adresse.setText("");
    setErfuellt(false);
    for (JTextField element : fahrtDatum) {
      if (element != null) {
        element.setText("");
      }
    }
    for (JTextField element : fahrtZiel) {
      if (element != null) {
        element.setText("");
      }
    }
    for (JTextField element : fahrtKm) {
      if (element != null) {
        element.setText("");
      }
    }
    for (JTextField element : fahrtZf) {
      if (element != null) {
        element.setText("");
      }
    }
  ...
  void setTeilnehmer(EfaWettMeldung m) {
    ...
    if (m.geschlecht != null) {
      if (m.geschlecht.equals("M")) {
        geschlecht.setSelectedIndex(1);
      } else if (m.geschlecht.equals("W")) {
      ...
      if (m.fahrt[i][0] != null) {
        for (int j = 0; j < m.fahrt[i].length; j++) {
          if (m.fahrt[i][j] != null && i < fahrtDatum.length && j < 4) {
            switch (j) {
              case 0:
                fahrtDatum[i].setText(m.fahrt[i][j]);
                break;
              case 1:
                fahrtZiel[i].setText(m.fahrt[i][j]);
                break;
              case 2:
                fahrtKm[i].setText(m.fahrt[i][j]);
                break;
              case 3:
                fahrtZf[i].setText(m.fahrt[i][j]);
                break;
            }
          }
        }
  ...
  void updateTeilnehmerMeldegeld() {
    ...
    if (efw != null && efw.wettId >= 0) {
      geld = 0; // @todo (P9) Emil: wettDefs.getWettDef(efw.wettId,year).meldegeld_grund +
      // anz*wettDefs.getWettDef(efw.wettId,year).meldegeld_teiln;
    } else {
      geld = 0;
    }
    meldegeld.setText(geld / 100 + "," + (geld % 100 < 10 ? "0" + geld % 100 : "" + geld % 100)
        + " EUR");
  ...
  void getFields() {
    ...
    int wettjahr = 0;
    try {
      wettjahr = Integer.parseInt(wettJahr.getText().trim());
    } catch (Exception e) {}
    if (wett.getSelectedIndex() == 0) {
      efw.allg_wett = null;
    } else {
      efw.allg_wett = (wettDefs.getWettDef(wett.getSelectedIndex(), wettjahr) != null ? wettDefs
          .getWettDef(wett.getSelectedIndex(), wettjahr).key : null);
    }
    if (!(s = wettJahr.getText().trim()).equals("")) {
      efw.allg_wettjahr = s;
    } else {
      efw.allg_wettjahr = null;
    }

    if (!(s = vereinsname.getText().trim()).equals("")) {
      efw.verein_name = s;
    } else {
      efw.verein_name = null;
    }
    if (!(s = benutzername.getText().trim()).equals("")) {
      efw.verein_user = s;
    } else {
      efw.verein_user = null;
    }
    if (!(s = mitglieder.getText().trim()).equals("")) {
      int i = EfaUtil.string2date(s, -1, 0, 0).tag;
      if (i != -1) {
        efw.verein_mitglieder = Integer.toString(i);
      } else {
        efw.verein_mitglieder = null;
      }
    } else {
      efw.verein_mitglieder = null;
    }

    if (!(s = meldName.getText().trim()).equals("")) {
      efw.meld_name = s;
    } else {
      efw.meld_name = null;
    }
    if (!(s = meldEmail.getText().trim()).equals("")) {
      efw.meld_email = s;
    } else {
      efw.meld_email = null;
    }

    if (!(s = versandName.getText().trim()).equals("")) {
      efw.versand_name = s;
    } else {
      efw.versand_name = null;
    }
    if (!(s = versandStrasse.getText().trim()).equals("")) {
      efw.versand_strasse = s;
    } else {
      efw.versand_strasse = null;
    }
    if (!(s = versandOrt.getText().trim()).equals("")) {
      efw.versand_ort = s;
    } else {
      efw.versand_ort = null;
    }

    if (efw.wettId == WettDefs.LRVBERLIN_BLAUERWIMPEL) {
      if (!(s = wimpelKm.getText().trim()).equals("")) {
        efw.wimpel_km = s;
      } else {
        efw.wimpel_km = null;
      }
      if (!(s = wimpelSchnitt.getText().trim()).equals("")) {
        efw.wimpel_schnitt = s;
      } else {
        efw.wimpel_schnitt = null;
      }
      if (!(s = teilnehmerAnz.getText().trim()).equals("")) {
        efw.wimpel_mitglieder = s;
      } else {
  ...
  EfaWettMeldung getTeilnehmer() {
    if (efw == null || currentTeilnehmer == null) {
      return null;
    }
  ...
  /*
   * // prüft, ob Bedingungen erfüllt; // Rückgabe: Name der Gruppe oder null, wenn nicht erfüllt
   * String isErfuellt() { if (wett.getSelectedIndex() == 0 || wettJahr.getText().trim().equals(""))
   * return null; TMJ tmj = EfaUtil.string2date(wettJahr.getText(),0,0,0); int _jahr = tmj.tag; tmj
   * = EfaUtil.string2date(jahrgang.getText(),0,0,0); int _jahrgang = tmj.tag; tmj =
   * EfaUtil.string2date(kilometer.getText(),0,0,0); if (tmj.monat<0 || tmj.monat>10) tmj.monat=0;
   * int _km = tmj.tag*10 + tmj.monat; int _geschlecht = geschlecht.getSelectedIndex()-1; if
   * (_jahrgang == 0 || _geschlecht<0) return null; int _z1 = 0; int _z2 = 0; switch
   * (wett.getSelectedIndex()) { case WettDefs.LRVBERLIN_SOMMER: _z1 = countFahrten(); break; case
   * WettDefs.LRVBERLIN_WINTER: _z1 = countFahrten(); boolean[] monate = new boolean[13]; // Monate
   * von 1 bis 12 plus Dummy _z2 = 0; for (int i=1; i<=12; i++) monate[i] = true; for (int i=0;
   * i<fahrtDatum.length; i++) { tmj = EfaUtil.string2date(fahrtDatum[i].getText(),0,0,0); if
   * (tmj.monat>0 && tmj.monat<13 && monate[tmj.monat]) { _z2++; monate[tmj.monat] = false; } }
   * break; } return
   * wettDefs.erfuellt(wett.getSelectedIndex(),_jahr,_jahrgang,_geschlecht,_km,_z1,_z2,0); }
   */
  ...
  String isErfuellt(int wettbewerb, String wettjahr, EfaWettMeldung m) {
    ...
    if (wettbewerb == 0 || wettjahr == null || wettjahr.trim().equals("")) {
      return null;
    }
    if (wettbewerb == WettDefs.LRVBERLIN_BLAUERWIMPEL) {
      if (m.nachname != null && m.vorname != null && m.kilometer != null &&
          m.nachname.length() > 0 && m.vorname.length() > 0
          && EfaUtil.string2date(m.kilometer, 0, 0, 0).tag > 0) {
        return "";
      } else {
        return null;
      }
    }
    if (m.jahrgang == null || m.kilometer == null || m.geschlecht == null || m.fahrt == null) {
      return null;
    }
    ...
    if (tmj.monat < 0 || tmj.monat > 10) {
      tmj.monat = 0;
    }
    int _km = tmj.tag * 10 + tmj.monat;
    int _geschlecht = (m.geschlecht.equals("M") ? 0 : (m.geschlecht.equals("W") ? 1 : -1));
    boolean _behinderung = false; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! (wenn DRV
    // implementiert, dann muß hierfür ein Feld eingefügt werden!)
    if (_jahrgang == 0 || _geschlecht < 0) {
      return null;
    }
    int _z1 = 0;
    int _z2 = 0;
    switch (wettbewerb) {
      ...
      case WettDefs.LRVBERLIN_WINTER:
        ...
        for (String[] element : m.fahrt) {
          if (element[0] == null) {
            continue;
          }
          tmj = EfaUtil.string2date(element[0], 0, 0, 0);
  ...
  void checkErfuellt() {
    ...
    String gr;
    setErfuellt((gr = isErfuellt(wett.getSelectedIndex(), wettJahr.getText().trim(),
        getTeilnehmer())) != null);
    gruppe.setText(gr);
  }
  ...
  int getAnzTeilnehmerErfuellt() {
    ...
    EfaWettMeldung m = efw.meldung;
    while (m != null) {
      if (isErfuellt(wett.getSelectedIndex(), wettJahr.getText().trim(), m) != null) {
        i++;
      }
      m = m.next;
    }
  ...
  /*
   * // Anzahl der wertbaren Fahrten ermitteln int countFahrten() { // Zeitraum für Wettbewerb
   * ermitteln int wjahr = EfaUtil.string2date(wettJahr.getText(),0,0,0).tag; GregorianCalendar
   * vonCal = new
   * GregorianCalendar(wettDefs.wett[wett.getSelectedIndex()].von.jahr+wjahr,wettDefs.wett
   * [wett.getSelectedIndex()].von.monat-1,wettDefs.wett[wett.getSelectedIndex()].von.tag);
   * vonCal.set
   * (wettDefs.wett[wett.getSelectedIndex()].von.jahr+wjahr,wettDefs.wett[wett.getSelectedIndex
   * ()].von.monat-1,wettDefs.wett[wett.getSelectedIndex()].von.tag); GregorianCalendar bisCal = new
   * GregorianCalendar
   * (wettDefs.wett[wett.getSelectedIndex()].bis.jahr+wjahr,wettDefs.wett[wett.getSelectedIndex
   * ()].bis.monat-1,wettDefs.wett[wett.getSelectedIndex()].bis.tag);
   * bisCal.set(wettDefs.wett[wett.getSelectedIndex
   * ()].bis.jahr+wjahr,wettDefs.wett[wett.getSelectedIndex
   * ()].bis.monat-1,wettDefs.wett[wett.getSelectedIndex()].bis.tag); String tage=" "; // zum
   * ermitteln der doppelten Tage (nur eine Fahrt pro Tag!) String zf=""; // Zielfahrten insgesamt
   * if (wett.getSelectedIndex()<1) return 0; int c=0; // Zähler für die Fahrten for (int i=0;
   * i<fahrtDatum.length; i++) { // Fahrt vollständig if (fahrtDatum[i].getText().trim().equals("")
   * || fahrtKm[i].getText().trim().equals("")) continue; if (wett.getSelectedIndex() ==
   * WettDefs.LRVBERLIN_SOMMER && fahrtZiel[i].getText().trim().equals("")) continue; if
   * (wett.getSelectedIndex() == WettDefs.LRVBERLIN_SOMMER &&
   * fahrtZf[i].getText().trim().equals("")) continue; // Fahrt im Wettbewerbs-Zeitraum? TMJ dateF =
   * EfaUtil.string2date(fahrtDatum[i].getText(),0,0,0); GregorianCalendar dateCal = new
   * GregorianCalendar(dateF.jahr,dateF.monat-1,dateF.tag);
   * dateCal.set(dateF.jahr,dateF.monat-1+dateCal.getMinimum(GregorianCalendar.MONTH),dateF.tag); if
   * (dateCal.before(vonCal) || dateCal.after(bisCal)) { continue; } // Tag doppelt? String s =
   * ""+dateF.tag+"-"+dateF.monat+"-"+dateF.jahr; if (tage.indexOf(" "+s+" ")>=0) { continue; } tage
   * = tage + s + " "; // LRVSOMMER: Zielfahrt mind. 20 Km lang? if (wett.getSelectedIndex() ==
   * WettDefs.LRVBERLIN_SOMMER && EfaUtil.string2date(fahrtKm[i].getText(),0,0,0).tag < 20) {
   * continue; } zf = Statistik.makeZf(zf,fahrtZf[i].getText().trim()); c++; } int zfc =
   * EfaUtil.countCharInString(zf,';') + 1; if (wett.getSelectedIndex() == WettDefs.LRVBERLIN_SOMMER
   * && zfc < c) c = zfc; return c; }
   */
  ...
  int countFahrten(int wettbewerb, int wettjahr, EfaWettMeldung m) {
    ...
    GregorianCalendar vonCal = new GregorianCalendar(wett.von.jahr + wettjahr, wett.von.monat - 1,
        wett.von.tag);
    vonCal.set(wett.von.jahr + wettjahr, wett.von.monat - 1, wett.von.tag);
    GregorianCalendar bisCal = new GregorianCalendar(wett.bis.jahr + wettjahr, wett.bis.monat - 1,
        wett.bis.tag);
    ...
    for (String[] element : m.fahrt) {
      ...
      // Fahrt vollständig
      if (element[0] == null || element[0].trim().equals("") ||
          element[2] == null || element[2].trim().equals("")) {
        continue;
      }
      if (wettbewerb == WettDefs.LRVBERLIN_SOMMER
          && (element[1] == null || element[1].trim().equals(""))) {
        continue;
      }
      if (wettbewerb == WettDefs.LRVBERLIN_SOMMER
          && (element[3] == null || element[3].trim().equals(""))) {
        continue;
      }

      // Fahrt im Wettbewerbs-Zeitraum?
      TMJ dateF = EfaUtil.string2date(element[0], 0, 0, 0);
      GregorianCalendar dateCal = new GregorianCalendar(dateF.jahr, dateF.monat - 1, dateF.tag);
      dateCal.set(dateF.jahr, dateF.monat - 1 + dateCal.getMinimum(Calendar.MONTH),
          dateF.tag);
      ...
      if (wettbewerb == WettDefs.LRVBERLIN_SOMMER
          && EfaUtil.string2date(element[2], 0, 0, 0).tag < 20) {
        continue;
      }

      zf.addZielfahrten(element[3]);
      c++;
    }
    zf.reduceToMinimun();
    int zfc = zf.getAnzZielfahrten();
    if (wettbewerb == WettDefs.LRVBERLIN_SOMMER && zfc < c) {
      c = zfc;
    }
    return c;
  }
  ...
  void speichereEintrag() {
    ...
    EfaWettMeldung m = efw.meldung;
    int i = 0;
    while (i < currentTeilnehmerNummer - 1 && m != null) {
      i++;
      m = m.next;
    }
  ...
  boolean speichereDatei(boolean unter) {
    ...
    if (efw.allg_wett == null && fehlendeAngabeAbbruch("Es wurde kein Wettbewerb angegeben!")) {
      return false;
    }
    if (efw.allg_wettjahr == null
        && fehlendeAngabeAbbruch("Es wurde kein Wettbewerbsjahr angegeben!")) {
      return false;
    }
    if (efw.verein_name == null && fehlendeAngabeAbbruch("Es wurde kein Vereinsname angegeben!")) {
      return false;
    }
    if (efw.verein_user == null && fehlendeAngabeAbbruch("Es wurde kein Benutzername angegeben!")) {
      return false;
    }
    ...
    if (unter || efw.datei == null) {
      ...
      String dat = Dialog.dateiDialog(this, "Meldedatei speichern unter...",
          "efa Meldedatei (*.efw)", "efw", dir, true);
      if (dat != null) {
        efw.datei = dat;
        if (!efw.datei.toLowerCase().endsWith(".efw")) {
          efw.datei = efw.datei + ".efw";
        }
        this.setTitle(KURZTITEL + " - " + efw.datei);
      } else {
        return false;
  ...
  void exportCVS() {
    ...
    String dat = Dialog.dateiDialog(this, "Meldedatei exportieren als", "CSV-Dateien (*.csv)",
        "csv", dir, true);
    ...
    if (!datei.toLowerCase().endsWith(".csv")) {
      datei += ".csv";
    }
  ...
  int writeCSVfile(BufferedWriter f) throws IOException {
    getFields();
    if (efw.wettId == WettDefs.LRVBERLIN_BLAUERWIMPEL) {
      f.write(efw.verein_name + "\t" + efw.verein_mitglieder + "\t" + efw.wimpel_km + "\t"
          + efw.wimpel_schnitt + "\t" + efw.wimpel_mitglieder + "\n");
      f.close();
      return 1;
    }
    EfaWettMeldung m = efw.meldung;
