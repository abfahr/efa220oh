...
public class AdminDialog extends BaseDialog implements IItemListener {
  ...
  public AdminDialog(EfaBoathouseFrame parent, AdminRecord admin) {
    super(parent,
        International.getStringWithMnemonic("Admin-Modus") + " [" + admin.getName() + "]",
        International.getStringWithMnemonic("Logout"));
  ...
  private void updateMessageButton(ItemTypeButton button) {
    ...
    try {
      long msg = (Daten.project != null ? Daten.project.getMessages(false).countUnreadMessages()
          : 0);
      if (msg > 0) {
  ...
  public void itemListenerAction(IItemType itemType, AWTEvent event) {
    if (event != null && event instanceof ActionEvent) {
      ...
      boolean permission = EfaMenuButton.menuAction(this, action, admin,
          efaBoathouseFrame.getLogbook());

      if (action.equals(EfaMenuButton.BUTTON_EXIT) && permission) {

        efaBoathouseFrame.cancel(null, EfaBoathouseFrame.EFA_EXIT_REASON_USER, admin,
            EfaMenuButton.getLastBooleanValue());
        return;
      }
      ...
      if ((action.equals(EfaMenuButton.BUTTON_PROJECTS) || action
          .equals(EfaMenuButton.BUTTON_LOGBOOKS)) && permission) {
      ...
      if ((action.equals(EfaMenuButton.BUTTON_PROJECTS) || action
          .equals(EfaMenuButton.BUTTON_CLUBWORKBOOK)) && permission) {
        if (Daten.project == null) {
          Dialog.error(International.getString("Kein Projekt geöffnet."));
          return;
        }
