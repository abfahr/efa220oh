...
public class EfaBaseFrame extends BaseDialog implements IItemListener {
  ...
  LogbookRecord referenceRecord; // refDatensatz = Referenz-Datensatz (zuletzt angezeigter
  // Datensatz, wenn neuer erstellt wird)
  long logbookValidFrom = 0;
  long logbookInvalidFrom = 0;
  boolean isNewRecord; // neuerDatensatz = ob akt. Datensatz ein neuer Datensatz, oder ein
  // bearbeiteter ist
  boolean isInsertedRecord; // neuerDatensatz_einf = ob der neue Datensatz eingefügt wird (dann beim
  // Hinzufügen keine Warnung wegen kleiner Lfd. Nr.!)
  int entryNoForNewEntry = -1; // lfdNrForNewEntry = LfdNr (zzgl. 1), die für den nächsten per "Neu"
  // erzeugten Datensatz verwendet werden soll; wenn <0, dann wird
  // "last+1" verwendet
  ...
  private void iniGuiMain() {
    ...
    date.setBackgroundColorWhenFocused(Daten.efaConfig.getValueEfaDirekt_colorizeInputField() ? Color.yellow
        : null);
    ...
    boat.setBackgroundColorWhenFocused(Daten.efaConfig.getValueEfaDirekt_colorizeInputField() ? Color.yellow
        : null);
    ...
    cox = new ItemTypeStringAutoComplete(LogbookRecord.COXNAME, "", IItemType.TYPE_PUBLIC, null,
        International.getStringWithMnemonic("Steuermann"), true);
    ...
    cox.setBackgroundColorWhenFocused(Daten.efaConfig.getValueEfaDirekt_colorizeInputField() ? Color.yellow
        : null);
  ...
  PersonRecord findPerson(ItemTypeString item, long validAt) {
    ...
    try {
      ...
      if (s.length() > 0) {
        ...
        if (validAt > 0 && r == null) {
          PersonRecord r2 = Daten.project.getPersons(false).getPerson(s, this.logbookValidFrom,
  ...
  BoatRecord findBoat(long validAt) {
    try {
      ...
      if (s.length() > 0) {
        ...
        // That means, even though the user entered "A", we will save the ID, and display "B".
        if (validAt > 0 && r == null) {
          BoatRecord r2 = Daten.project.getBoats(false).getBoat(s, this.logbookValidFrom,
              logbookInvalidFrom - 1, validAt);
          if (r2 != null) {
  ...
  boolean saveEntryInLogbook() {
    ...
    try {
      ...
      if (mode == MODE_BOATHOUSE_START || mode == MODE_BOATHOUSE_START_CORRECT) {
        ...
        currentRecord.setSessionIsOpen(false); // all other updates to an open entry (incl. Admin
  ...
  boolean promptSaveChangesOk() {
    ...
    if (isEntryChanged()) {
      ...
      if (isNewRecord) {
        txt = International
            .getString("Der aktuelle Eintrag wurde verändert und noch nicht zum Fahrtenbuch hinzugefügt.")
            + "\n" +
      ...
      if (groupIdList != null && groupIdList.length() > 0) {
        ...
        if (Daten.efaConfig.getValueCheckAllowedPersonsInBoat() &&
          ...
          for (int j = 0; j < groupIdList.length(); j++) {
            ...
            erlaubteGruppen = (erlaubteGruppen == null ? name : erlaubteGruppen
  ...
  public void itemListenerAction(IItemType item, AWTEvent event) {
    ...
    if (id == FocusEvent.FOCUS_GAINED) {
      ...
      if (item == destination) {
        lastDestination = DestinationRecord.tryGetNameAndVariant(destination.getValueFromField()
            .trim())[0];
        ;
      }
      if (item == distance) {
      ...
      if (item == comments) {
        ...
        if (k != null && k.length > 0) {
          for (String element : k) {
            if ((element.equals("F6") && e.getKeyCode() == KeyEvent.VK_F6 && hash.get(element) != null)
                ||
                (element.equals("F7") && e.getKeyCode() == KeyEvent.VK_F7 && hash.get(element) != null)
                ||
                (element.equals("F8") && e.getKeyCode() == KeyEvent.VK_F8 && hash.get(element) != null)
                ||
  ...
  void setDefaultCrew(UUID crewId) {
    ...
    if (r != null) {
      ...
      if ((r.getBoatCaptainPosition() == 0 && currentBoatTypeCoxing != null && !currentBoatTypeCoxing
          .equals(EfaTypes.TYPE_COXING_COXLESS))
          ||
