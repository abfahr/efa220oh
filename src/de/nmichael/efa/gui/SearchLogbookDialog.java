...
public class SearchLogbookDialog extends BaseTabbedDialog implements IItemListener {
  ...
  private SearchLogbookDialog(EfaBaseFrame parent) {
    ...
    items.add(eUnknownPersonIgnoreGuest = new ItemTypeBoolean("ESEARCH_UNKNOWNPERSONIGNOREGUEST",
        ...
        IItemType.TYPE_PUBLIC, CAT_SPECIAL, International.getMessage(
            "Unbekannte Einträge mit '{guest}' ignorieren",
  ...
  public static void showSearchDialog(EfaBaseFrame parent, Logbook logbook,
      DataKeyIterator iterator,
      String searchString) {
    ...
    if (CAT_NORMAL.equals(searchLogbookDialog
        .getSelectedPanel((JTabbedPane) searchLogbookDialog.topLevelPane))) {
      ...
      while (true) {
        ...
        if (mode == SearchMode.special) {
          ...
          if (searchLogbookDialog != null && searchLogbookDialog.eIncomplete.getValue()) {
            ...
            if (r.getDestinationAndVariantName() == null
                || r.getDestinationAndVariantName().length() == 0) {
              foundMatch(r, efaBaseFrame.destination, jumpToField);
              return true;
            }
          ...
          if (searchLogbookDialog != null && searchLogbookDialog.eUnknownPerson.getValue()) {
            for (int i = 0; i <= LogbookRecord.CREW_MAX; i++) {
              if (r.getCrewId(i) == null && r.getCrewName(i) != null
                  && r.getCrewName(i).length() > 0) {
          ...
          if (searchLogbookDialog != null && searchLogbookDialog.eInvalidDestination.getValue()) {
            ...
            if (r.getDestinationId() == null ||
              ...
              DestinationRecord br = destinations.getDestination(r.getDestinationName(), 0,
                  Long.MAX_VALUE, validAt);
              if (br != null && !br.isValidAt(validAt)) {
                invalid = true;
              }
            }
          ...
          if (searchLogbookDialog != null && searchLogbookDialog.eOpenEntry.getValue()) {
            if (r.getDistance() != null
                && (!r.getDistance().isSet() || r.getDistance().getValueInMeters() == 0)) {
              foundMatch(r, efaBaseFrame.distance, jumpToField);
              return true;
            }
          }
