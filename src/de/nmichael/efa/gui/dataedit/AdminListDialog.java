...
public class AdminListDialog extends DataListDialog {
  ...
  public static final int ACTION_EFALIVE_ADMIN = 901; // negative actions will not be shown as popup
  // actions
    ...
    switch (actionId) {
      case ACTION_EFALIVE_ADMIN:
        if (Daten.admins.createOrFixEfaLiveAdmin()) {
          Dialog
          .infoDialog((efaLiveRepair ?
              International.getMessage("Admin '{name}' erfolgreich repariert",
                  Admins.EFALIVEADMIN) :
                    International.getMessage("Admin '{name}' erfolgreich erstellt",
                        Admins.EFALIVEADMIN)));
        } else {
