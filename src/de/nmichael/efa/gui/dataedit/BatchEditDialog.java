...
public class BatchEditDialog extends BaseTabbedDialog implements IItemFactory, IItemListener {
  ...
  public void setPersistence(StorageObject persistence, long validAt) {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if (!fields[i].equals(DataRecord.LASTMODIFIED) &&
        ...
        showDescriptionsCondition.add(fields[i]
        ...
        try {
          if (persistence.data().getFieldType(fields[i]) != IDataAccess.DATA_VIRTUAL) {
            ...
            showDescriptionsEdit.add(fields[i]
                + (item != null ? " (" + item.getDescription() + ")" : ""));
          }
  ...
  protected void iniDialog() throws Exception {
    ...
    editConditions = new ItemTypeItemList(GUIITEM_EDITCONDITION, itemListConditions, this,
        IItemType.TYPE_PUBLIC, makeCategory(null), "... "
            + International.getString("die folgende Bedingungen erfüllen"));
    ...
    editFields = new ItemTypeItemList(GUIITEM_EDITFIELDS, itemListFields, this,
        IItemType.TYPE_PUBLIC, makeCategory(null), "... "
            + International.getString("und ändere folgende Felder"));
    ...
    if (isVersionized) {
      versionAction = new ItemTypeStringList(
          GUIITEM_VERSIONACTION,
          VA_NEWVERSION,
  ...
  public void closeButton_actionPerformed(ActionEvent e) {
    ...
    for (int i = 0; i < editConditions.size(); i++) {
      ...
      rules.editConditions.add(new EditCondition(items[0].toString(), items[1].toString(), items[2]
          .toString()));
    }
    for (int i = 0; i < this.editFields.size(); i++) {
      IItemType[] items = editFields.getItems(i);
      rules.editValues.add(new EditValue(items[0].toString(), items[1].toString(), items[2]
          .toString()));
    }
    if (isVersionized) {
        ...
        if (r != null) {
          ...
          for (EditValue editValue : rules.editValues) {
            ...
            if (newValueText != null) {
              ...
              throw new Exception("New value '" + editValue.value + "' for field '"
                  + editValue.field + "' cannot be applied!");
            }
          }
        }
      }
  ...
  private boolean showEditChanges(Vector<EditChange> editChanges, EditRules editRules) {
    ...
    try {
      ...
      HtmlFactory.writeHeader(f, International.getStringWithMnemonic("Vorschau der Änderungen"),
          true);
      ...
      if (isVersionized) {
        f.write("<p align=\"center\">"
            ...
            International.getMessage("basierend auf der Version gültig am {date}",
                DataTypeDate
                .getDateTimeString(new DataTypeDate(validAt), new DataTypeTime(validAt)))
                +
                ":<br>"
                +
                (editRules.versionAction.equals(VA_EDITCURRENT) ?
                    International.getString("aktualisiere diese Version") :
                      International.getMessage("erstelle eine neue Version gültig ab {date}",
                          DataTypeDate.getDateTimeString(new DataTypeDate(editRules.versionValidFrom),
                              new DataTypeTime(editRules.versionValidFrom)))) +
            "</p>\n");
      }
  ...
  class ChangeTask extends ProgressTask {
    ...
    public void run() {
          ...
          try {
            ...
            if (r != null) {
              ...
              if (editRules.versionAction == null ||
                ...
                Logger.log(
                    Logger.INFO,
                    Logger.MSG_DATAADM_BE_RECORDUPDATED,
                ...
                Logger.log(
                    Logger.INFO,
                    Logger.MSG_DATAADM_BE_RECORDADDEDVER,
                    r.getPersistence().getDescription()
                    + ": "
                    + International.getMessage(
              ...
              if (!change.newValue.equals(newVal)) {
                logInfo("WARNING: " +
                    "Value '" + change.newValue + "' for Field '" + change.field
                    + "' corrected to '" + newVal + "'\n");
              } else {
                successCount++;
              }
      ...
      logInfo(International
          .getMessage("{count} Änderungen erfolgreich durchgeführt.", successCount) + "\n");
      setDone();
    }
