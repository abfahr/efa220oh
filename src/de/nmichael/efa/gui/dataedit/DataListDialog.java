...
IItemListenerDataRecordTable {
  ...
  public static final int ACTION_IMPORT = -100; // negative actions will not be shown as popup
  // actions
  public static final int ACTION_EXPORT = -101; // negative actions will not be shown as popup
  // actions
  public static final int ACTION_PRINTLIST = -102; // negative actions will not be shown as popup
  // actions
  public static final int ACTION_EDITASSISTENT = -103; // negative actions will not be shown as
  // popup actions
  ...
  protected void iniControlPanel() {
    if (persistence != null && persistence.data().getMetaData().isVersionized()) {
      ...
      showAll = new ItemTypeBoolean("SHOW_ALL",
          ...
          International.getString("auch derzeit ungültige Datensätze zeigen"));
  ...
  public void itemListenerActionTable(int actionId, DataRecord[] records) {
    ...
    switch (actionId) {
          ...
          if (records.length == 1) {
            res = Dialog.yesNoDialog(International.getString("Wirklich verstecken?"),
                International.getMessage(
                    "Möchtest Du den Datensatz '{record}' wirklich verstecken?",
                    records[0].getQualifiedName()));
          } else {
            res = Dialog.yesNoDialog(International.getString("Wirklich verstecken?"),
                International.getMessage(
                    "Möchtest Du {count} ausgewählte Datensätze wirklich verstecken?",
                    records.length));
          }
        } else {
          if (records.length == 1) {
            res = Dialog.yesNoDialog(International.getString("Wirklich sichtbar machen?"),
                International.getMessage(
                    "Möchtest Du den Datensatz '{record}' wirklich sichtbar machen?",
                    records[0].getQualifiedName()));
          } else {
            res = Dialog.yesNoDialog(International.getString("Wirklich sichtbar machen?"),
                International.getMessage(
                    "Möchtest Du {count} ausgewählte Datensätze wirklich sichtbar machen?",
                    records.length));
          }
        }
        if (res != Dialog.YES) {
      ...
      case ACTION_MERGE:
        ...
        ItemTypeHtmlList list = new ItemTypeHtmlList(
            "LIST",
            keys,
            items,
            keys[0],
