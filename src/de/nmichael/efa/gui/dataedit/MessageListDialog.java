...
public class MessageListDialog extends DataListDialog {
  ...
  // actions
  public static final int ACTION_FORWARD = 902; // negative actions will not be shown as popup
  // actions
        ...
        try {
          for (int i = 0; records != null && i < records.length; i++) {
            ...
            if (r != null
              ...
              /*
               * ((Messages)persistence).createAndSaveMessageRecord(r.getFrom(), (origToAdmin ?
               * MessageRecord.TO_BOATMAINTENANCE : MessageRecord.TO_ADMIN), (String)null,
               * r.getSubject(), International.getMessage("Weitergeleitet von {name}",
               * admin.getName()) + ":\n\n" + r.getText());
               */
  ...
  public DataEditDialog createNewDataEditDialog(JDialog parent, StorageObject persistence,
      DataRecord record, boolean newRecord) {
    if (record == null) {
      record = Daten.project.getMessages(false).createMessageRecord();
    }
    return new MessageEditDialog(parent, (MessageRecord) record, newRecord, admin);
