...
public class ProjectEditDialog extends UnversionizedDataEditDialog implements IItemListener {
  ...
  public ProjectEditDialog(Frame parent, Project p, Type type, String projectRecordName,
      int subtype, AdminRecord admin) {
    super(parent, International.getString("Projekt"), null, false, admin);
    iniItems(p, projectRecordName, type, subtype);
  }

  public ProjectEditDialog(JDialog parent, Project p, Type type, String projectRecordName,
      int subtype, AdminRecord admin) {
    super(parent, International.getString("Projekt"), null, false, admin);
    iniItems(p, projectRecordName, type, subtype);
  }

  public ProjectEditDialog(Frame parent, Project p, ProjectRecord projectRecord, int subtype,
      AdminRecord admin) {
    super(parent, getInternationalProjectTypeString(projectRecord), null, false, admin);
    iniItems(p, (projectRecord != null ? projectRecord.getName() : null),
        typeMapping(projectRecord), subtype);
  }

  public ProjectEditDialog(JDialog parent, Project p, ProjectRecord projectRecord, int subtype,
      AdminRecord admin) {
    super(parent, getInternationalProjectTypeString(projectRecord), null, false, admin);
    iniItems(p, (projectRecord != null ? projectRecord.getName() : null),
        typeMapping(projectRecord), subtype);
  ...
  protected boolean saveRecord() throws InvalidValueException {
    for (IItemType item : getItems()) {
      if (!item.isValidInput() && item.isVisible()) {
        ...
        // @todo (P4) make sure that if dates of logbook is changed, that all sessions are still
        // within the range!
      }
    }
    try {
