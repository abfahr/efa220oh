...
public class VersionizedDataEditDialog extends UnversionizedDataEditDialog implements IItemListener {
  ...
  public VersionizedDataEditDialog(Frame parent, String title, DataRecord dataRecord,
      boolean newRecord, AdminRecord admin) {
  ...
  protected void iniDialog() throws Exception {
    ...
    // versionList = new ItemTypeHtmlList("VERSION_LIST", null, null, null, IItemType.TYPE_PUBLIC,
    // null, International.getString("Versionen"));
    ...
    if (dataRecord != null) {
      try {
        ...
        for (int i = 0; i < recs.length; i++) {
          ...
          // String boldStart = (r.getKey().equals(dataRecord.getKey()) ?
          // "<b style=\"font-color:blue\">" : "");
          // String boldEnd = (r.getKey().equals(dataRecord.getKey()) ? "</b>" : "");
          // String bgcol = (r.getKey().equals(dataRecord.getKey()) ?
          ...
          // content.append("<td width=\"20%\"" + bgcol + " cellpadding=\"0\">" + boldStart +
          // International.getString("Version") + " " + (i+1) + boldEnd + "</td>");
          // content.append("<td width=\"38%\"" + bgcol + " cellpadding=\"0\">" + boldStart +
          // r.getValidFromTimeString() + boldEnd + "</td>");
          // content.append("<td width=\"4%\"" + bgcol + " cellpadding=\"0\">" + boldStart + "-" +
          // boldEnd + "</td>");
          // content.append("<td width=\"38%\"" + bgcol + " cellpadding=\"0\">" + boldStart +
          // r.getValidUntilTimeString() + boldEnd + "</td>");
  ...
  void setSelectedVersionLabel() {
    if (versionList != null) {
      ...
      if (v >= 0) {
        ...
        if (r != null) {
          selectedVersionLabel.setText(International.getString("Version") + " " + (v + 1) + ": "
              + r.getValidRangeString());
        }
      }
    }
  }
  ...
  void newButton_actionPerformed(ActionEvent e, boolean fromPopupMenu) {
    ...
    if (v >= 0) {
      DataRecord r = versions.get(v);
      if (r != null) {
        VersionizedDataCreateVersionDialog dlg = new VersionizedDataCreateVersionDialog(this, r,
            v + 1);
        dlg.showDialog();
    ...
    if (v >= 0) {
      ...
      if (r != null) {
        switch (Dialog.yesNoDialog(International.getString("Version löschen"),
            International.getMessage(
                "Möchtest Du Version {version} dieses Datensatzes wirklich löschen?", (v + 1)))) {
                  case Dialog.YES:
                    ...
                    if (prevVersion >= 0 && nextVersion >= 0) {
                      switch (Dialog
                          .auswahlDialog(
                              International.getString("Version auswählen"),
                              International
                              .getString("Welche Version dieses Datensatzes soll den Gültigkeitszeitraum der gelöschten Version übernehmen?"),
  ...
  void versionValidityChangeButton_actionPerformed(ActionEvent e) {
    ...
    if (dataRecord != null) {
      VersionizedDataChangeValidityDialog dlg = new VersionizedDataChangeValidityDialog(this,
          thisVersion, dataRecord, rPrev, rNext);
      dlg.showDialog();
      long newValidFrom = dlg.getValidFromResult();
      ...
      if (newValidFrom != dataRecord.getValidFrom()
          || newInvalidFrom != dataRecord.getInvalidFrom()) {
        try {
          ...
          .log(
              Logger.INFO,
              Logger.MSG_DATAADM_RECORDVALIDCHANGED,
    ...
    if (newRecord) {
      ...
      if (promptToEnterValidity) {
        if (SimpleInputDialog.showInputDialog(this,
            International.getString("Gültigkeitsbeginn festlegen"), validFrom)) {
      ...
      if (super.saveRecord()) {
        if (validFromTs > 0) {
          try {
            dataRecord.getPersistence().data()
            .changeValidity(dataRecord, validFromTs, Long.MAX_VALUE);
          } catch (EfaException ex) {
            ex.log();
            Dialog.error("Der Datensatz konnte nicht geändert werden." + "\n" + ex.toString());
            return false;
