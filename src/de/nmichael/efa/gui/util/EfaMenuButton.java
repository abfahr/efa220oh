...
public class EfaMenuButton {
  ...
  public EfaMenuButton(String menuName, String buttonName, String menuText, String buttonText,
      ImageIcon icon) {
  ...
  public static synchronized Vector<EfaMenuButton> getAllMenuButtons(AdminRecord admin,
    ...
    if (v.size() > 0 && v.get(v.size() - 1).getMenuName().equals(MENU_FILE)
    ...
    if (admin == null || (admin.isAllowedEditLogbook() && adminMode)) { // we have the same menu
      // again at the end for
      // non-admin mode...
    ...
    if (admin == null || admin.isAllowedEditLogbook() && adminMode) { // we have the same menu again
      // at the end for non-admin
      // mode...
    ...
    if (v.size() > 0 && v.get(v.size() - 1).getMenuName().equals(MENU_ADMINISTRATION)
        && !v.get(v.size() - 1).isSeparator()) {
    ...
    if (v.size() > 0 && v.get(v.size() - 1).getMenuName().equals(MENU_MANAGEMENT)
        && !v.get(v.size() - 1).isSeparator()) {
  ...
  public static boolean menuAction(BaseFrame parent, String action, AdminRecord admin,
      Logbook logbook) {
    return menuAction(parent, null, action, admin, logbook);
  }

  public static boolean menuAction(BaseDialog parent, String action, AdminRecord admin,
      Logbook logbook) {
    return menuAction(null, parent, action, admin, logbook);
  }

  private static boolean menuAction(BaseFrame parentFrame, BaseDialog parentDialog, String action,
      AdminRecord admin, Logbook logbook) {
    if (action == null) {
      return false;
    }
    ...
    if (action.equals(BUTTON_BACKUP)) {
      ...
      BackupDialog dlg = (parentFrame != null ? new BackupDialog(parentFrame, admin)
    ...
    if (action.equals(BUTTON_OSCOMMAND)) {
      ...
      if ((parentFrame != null ? SimpleInputDialog.showInputDialog(parentFrame,
    ...
    if (action.equals(BUTTON_BOATS)) {
      ...
      BoatListDialog dlg = (parentFrame != null ? new BoatListDialog(parentFrame, -1, admin)
    ...
    if (action.equals(BUTTON_BOATSTATUS)) {
      ...
      BoatStatusListDialog dlg = (parentFrame != null ? new BoatStatusListDialog(parentFrame, admin)
    ...
    if (action.equals(BUTTON_PERSONS)) {
      ...
      PersonListDialog dlg = (parentFrame != null ? new PersonListDialog(parentFrame, -1, admin)
    ...
    if (action.equals(BUTTON_STATUS)) {
      ...
      StatusListDialog dlg = (parentFrame != null ? new StatusListDialog(parentFrame, admin)
    ...
    if (action.equals(BUTTON_WATERS)) {
      ...
      WatersListDialog dlg = (parentFrame != null ? new WatersListDialog(parentFrame, admin)
    ...
    if (action.equals(BUTTON_CONFIGURATION)) {
      ...
      EfaConfigDialog dlg = (parentFrame != null ? new EfaConfigDialog(parentFrame, myEfaConfig)
          : new EfaConfigDialog(parentDialog, myEfaConfig));
      dlg.showDialog();
    }
    ...
    if (action.equals(BUTTON_ADMINS)) {
      ...
      AdminListDialog dlg = (parentFrame != null ? new AdminListDialog(parentFrame, myAdmins, admin)
    ...
    if (action.equals(BUTTON_PASSWORD)) {
      ...
      AdminPasswordChangeDialog dlg = (parentFrame != null ? new AdminPasswordChangeDialog(
          parentFrame, admin) : new AdminPasswordChangeDialog(parentDialog, admin));
      dlg.showDialog();
    }
    ...
    if (action.equals(BUTTON_STATISTICS)) {
      ...
      StatisticsListDialog dlg = (parentFrame != null ? new StatisticsListDialog(parentFrame, admin)
          : new StatisticsListDialog(parentDialog, admin));
      dlg.showDialog();
    }
    ...
    if (action.equals(BUTTON_HELP)) {
      Help.showHelp((parentFrame != null ? parentFrame.getHelpTopics()
          : parentDialog
              .getHelpTopics()));
    }
    ...
    if (action.equals(BUTTON_MESSAGES)) {
      ...
      if (admin == null
          || (!admin.isAllowedMsgReadAdmin() && !admin.isAllowedMsgReadBoatMaintenance())) {
        insufficientRights(admin, action);
        return false;
      }
      MessageListDialog dlg = (parentFrame != null ? new MessageListDialog(parentFrame, admin)
          : new MessageListDialog(parentDialog, admin));
      dlg.showDialog();
    }
    ...
    if (action.equals(BUTTON_LOGFILE)) {
      ...
      LogViewDialog dlg = (parentFrame != null ? new LogViewDialog(parentFrame)
          : new LogViewDialog(parentDialog));
      dlg.showDialog();
    }
    ...
    if (action.equals(BUTTON_ABOUT)) {
      EfaAboutDialog dlg = (parentFrame != null ? new EfaAboutDialog(parentFrame)
          : new EfaAboutDialog(parentDialog));
      dlg.showDialog();
    }
