...
public class Table extends JTable {
  ...
  /**
   * This is certainly a very messy method, and it's not very elegant. But it works. DON'T CHANGE
   * IT!!
   */
  @Override
  ...
  private void setIntelligentColumnWidth() {
    ...
    int absoluteWidth = 0;
    for (TableItemHeader element : header) {
      absoluteWidth += element.getMaxColumnWidth();
    }

    int[] widths = new int[header.length];
    for (int i = 0; i < widths.length; i++) {
      widths[i] = (int) Math
          .floor((((float) header[i].getMaxColumnWidth()) / ((float) absoluteWidth))
              * (width));
  ...
  public static Table createTable(BaseDialog dlg, TableCellRenderer renderer,
      TableItemHeader[] header, TableItem[][] data) {
  ...
  public static Table createTable(BaseDialog dlg, TableCellRenderer renderer,
    ...
    for (TableItem[] element : data) {
      for (int j = 0; j < element.length; j++) {
        header[j].updateColumnWidth(element[j].toString());
      }
    }
