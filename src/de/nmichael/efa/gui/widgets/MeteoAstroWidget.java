...
public class MeteoAstroWidget extends Widget {
  ...
  public void runWidgetWarnings(int mode, boolean actionBegin, LogbookRecord r) {
    try {
      if ((mode == EfaBaseFrame.MODE_BOATHOUSE_START ||
        ...
        } else if (now >= sunset - getWarnTimeBeforeSunset()
            && now < sunset + getWarnTimeAfterSunset()) {
  ...
  class HTMLUpdater extends Thread {
    ...
    public void run() {
      ...
      while (keepRunning) {
          ...
          try {
            ...
            if (getLayout().equals(LAYOUT_HORIZONTAL)) {
              ...
              if (isShowCurrentWeatherTemperature()) {
                data.add(new Cell(column++, row, "<big><b>"
                    + (getTemperatureScale().equals(TEMP_CELSIUS) ? response.currentTempC + " °C"
                        : response.currentTempF + " °F") + "</b></big>", false));
              }
              if (isShowForecastWeatherText()) {
                data.add(new Cell(column++, row, International.getMessage("Vorhersage für {day}",
                    checkIfToday(response.forecastDay)) + ":<br>"
                    + response.forecastCondifion, false));
              }
              if (isShowForecastWeatherImage()) {
                data.add(new Cell(column++, row, "<img src=\"" + response.forecastIcon + "\">",
                    false));
              }
              if (isShowForecastWeatherTemperature()) {
                data.add(new Cell(column++, row, International.getString("Min") + ": "
                    + response.forecastLow + " °" + "<br>"
            ...
            if (getLayout().equals(LAYOUT_VERTICAL)) {
              ...
              if (isShowCurrentWeatherTemperature()) {
                data.add(new Cell(column, row++, "<big><b>"
                    + (getTemperatureScale().equals(TEMP_CELSIUS) ? response.currentTempC + " °C"
                        : response.currentTempF + " °F") + "</b></big>", true));
              }
              if (isShowForecastWeatherText()) {
                data.add(new Cell(column, row++, International.getMessage("Vorhersage für {day}",
                    checkIfToday(response.forecastDay)) + ":<br>"
          ...
          try {
            ...
            if (!sunriseError) {
              ...
              .log(
                  ...
                  + ": "
                  + Plugins.PLUGIN_JSUNTIMES
                  + " - "
                  + International
                  .getString("Die Sonnenaufgangs- und Untergangszeiten können nicht angezeigt werden.")
                  + " "
        ...
        try {
          ...
          htmlDoc.append("<table align=\"center\" cellspacing=\""
              + (getLayout().equals(LAYOUT_COMPACT) ? 0 : 8) + "\">\n");
          ...
          for (int i = 0; i < data.size(); i++) {
            ...
            htmlDoc.append("<td colspan=\"" + colspan +
                "\" align=\""
                + (cell.alignCenter ? "center" : (cell.alignRight ? "right" : "left")) + "\">" +
                cell.content + "</td>");
          }
          htmlDoc.append("</tr>\n");
    ...
    private String saveImage(String image, String format) {
      ...
      if (!EfaUtil.canOpenFile(fname)) {
        try {
          BufferedImage img = javax.imageio.ImageIO.read(MeteoAstroWidget.class
              .getResource(Daten.IMAGEPATH + image));
  ...
  class GoogleXmlResponse extends WeatherXmlResponse {
    ...
    public void startElement(String uri, String localName, String qname, Attributes atts) {
      ...
      if (parentElement != null && parentElement.equals("forecast_conditions")
          && forecastConditions == 1) {
        ...
        try {
          ...
          for (int angle = 22; angle <= 22 + 360; angle += 45) {
            if (dir < angle) {
              switch (angle) {
                case 22:
                  direction = International.getString("N", "direction");
                  break;
                case 67:
                  direction = International.getString("NO", "direction");
                  break;
                case 112:
                  direction = International.getString("O", "direction");
                  break;
                case 157:
                  direction = International.getString("SO", "direction");
                  break;
                case 202:
                  direction = International.getString("S", "direction");
                  break;
                case 247:
                  direction = International.getString("SW", "direction");
                  break;
                case 292:
                  direction = International.getString("W", "direction");
                  break;
                case 337:
                  direction = International.getString("NW", "direction");
                  break;
                case 382:
                  direction = International.getString("N", "direction");
                  break;
