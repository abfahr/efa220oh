...
public class CompetitionDRVFahrtenabzeichen extends Competition {
    ...
    for (int j = 0; sd.sessionHistory != null && j < sd.sessionHistory.size(); j++) {
      ...
      if (fahrt == null) {
        ...
        if (r.getEndDate() != null
            && (fahrt.dateEnd == null || r.getEndDate().isAfter(fahrt.dateEnd))) {
  ...
  public static long getWanderfahrtenMeter(StatisticsData sd) {
    ...
    for (Object key : keys) {
      DRVFahrt drvFahrt = wanderfahrten.get(key);
  ...
  public void calculate(StatisticsRecord sr, StatisticsData[] sd) {
    ...
    for (int g = 0; g < gruppen.length; g++) {
      ...
      sr.pCompGroupNames[g][1] = "Jahrgänge "
          + makeJahrgang(sr.sCompYear - gruppen[g].hoechstalter)
      ...
      for (int i = 0; i < sd.length; i++) {
        ...
        if (!sd[i].sYearOfBirth.equals("")
          ...
          String[][] wafa = new String[7][6]; // 7 Einträge mit jeweils
          // LfdNr/Abfahrt/Ankunft/Ziel/Km/Bemerk
          Object[] keys = wanderfahrten.keySet().toArray(); // Keys ermitteln
          long totalWafaMeters = 0; // Wafa-Meter insgesamt
          for (Object key : keys) {
            DRVFahrt drvFahrt = wanderfahrten.get(key);
          ...
          String totalWafaKm = DataTypeDistance.getDistanceFromMeters(totalWafaMeters)
              .getStringValueInKilometers(false, 0, 1);
          boolean[] ausg = new boolean[keys.length]; // merken, welche Fahrt schon zur Ausgabe
          // markiert wurde
          ...
          for (int nr = 0; nr < wafa.length + 1; nr++) { // max. für 7 auszufüllende Felder Fahrten
            // suchen (plus 1 weitere, die aber nicht
            // gemerkt wird)
            hoechst = 0; // höchste verbleibende KmZahl oder Tagezahl

            // nächste geeignete Fahrt heraussuchen (meiste Km (Gruppe<3) oder längste Tour (Gruppe
            // 3))
            for (int k = 0; k < ausg.length; k++) {
              drvel = wanderfahrten.get(keys[k]);
            ...
            if (hoechst > 0 && // was gefunden?
                (nr < 5 || // weniger als 5 Einträge, oder ...
                    (wafaMeters / 10 < gruppen[g].zusatz && gruppen[g].gruppe != 3) || // noch Km
                    // nötig
              ...
              if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett && !bestEl.jum
                  && wafa[nr][5].equals(EfaWettMeldung.JUM)) {
                wafa[nr][5] = "";
              }
          ...
          if (erfuellt
            ...
            if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett) {
              ...
              if (erfuellt && sd[i].personRecord != null) {
                ...
                FahrtenabzeichenRecord fahrtenheft = fahrtenabzeichen
                    .getFahrtenabzeichen((UUID) sd[i].key);
                if (fahrtenheft != null) {
                  if (fahrtenheft.getFahrtenheft() != null
                      && fahrtenheft.getFahrtenheft().length() > 0) {
                ...
                // Äquatorpreis
                int aeqKm = gesKm; // - gesKmAB; (seit 2007 zählen auch die Kilometer AB zum
                // Äquatorpreis)
                int anzAeqBefore = aeqKm / WettDefs.DRV_AEQUATOR_KM;
                int anzAeqJetzt = (aeqKm + EfaUtil.string2int(ewm.kilometer, 0))
                    / WettDefs.DRV_AEQUATOR_KM;
                if (anzAeqJetzt > anzAeqBefore) {
                  ewm.drv_aequatorpreis = Integer.toString(anzAeqJetzt);
                }

                // Abzeichen
                ewm.abzeichen = WettDefs.getDRVAbzeichen(gruppen[g].gruppe != 3, anzAbzeichen,
                    anzAbzeichenAB, sr.sCompYear);
              ...
              participant.sDistance = DataTypeDistance.getDistance(sd[i].distance)
              ...
              if (!sr.sIsOutputCompWithoutDetails && erfuellt) {
                participant.sDetailsArray = new String[wafaLength + (mehrFahrten ? 1 : 0)][6]; // ein
                // zusätzliches
                // Arrayelement
                // (nicht
                // gefüllt),
                ...
                if (gruppen[g].gruppe < 3) {
                  ...
                  if (sr.sIsOutputCompShort) {
                    if (sr.sIsOutputCompAdditionalWithRequirements) {
                      participant.sAdditional = wafaAnzMTour + "/" + gruppen[g].zusatz
                          + " Wafa-Tage";
                    } else {
    ...
    if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett
        && anzahlGemeldeteTeilnehmer > 0 && ungueltigeFahrtenhefte.size() > 0) {
    ...
    if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett
        && anzahlGemeldeteTeilnehmer > 0 && zumErstenMalGemeldet.size() > 0) {
      String info;
    ...
    if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett
        && anzahlGemeldeteTeilnehmer > 0 && letzteElektronischeMeldung + 1 != sr.sCompYear) {
      if (letzteElektronischeMeldung == 0) {
        ...
        if (Dialog
            .yesNoDialog(
                "Letzte elektronische Meldung?",
                "Die ausgewerteten Teilnehmer haben zum letzten Mal im Jahr "
                    + letzteElektronischeMeldung
                    + " ein\n"
        ...
        Dialog.infoDialog(
            ...
            "Es liegen für einige Teilnehmer bereits vom DRV bestätigte Fahrtenhefte aus dem Jahr "
                + sr.sCompYear
                + " vor.\n"
                + "Du kannst daher für das Jahr "
                + sr.sCompYear
                + " nicht erneut melden.\n\n"
                + "Falls Deine Meldung nachträglich vom DRV zurückgewiesen wurde und Du nun eine korrigierte Meldung\n"
                + "einsenden möchtest, mußt Du zuvor alle elektronischen Fahrtenhefte des Jahres "
                + sr.sCompYear
                + " in efa\n"
                + "löschen und die elektronischen Fahrtenhefte bis "
                + (sr.sCompYear - 1)
                + " erneut einspielen. Gehe dazu in\n"
                + "die Übersicht der DRV-Fahrtenabzeichen unter 'Administration - DRV-Fahrtenabzeichen', lösche dort alle\n"
                + "Fahrtenabzeichen des Jahres " + sr.sCompYear
                + " und spiele anschließend alle Bestätiungsdateien bis zum\n"
    ...
    if (sr.getOutputTypeEnum() != StatisticsRecord.OutputTypes.efawett) {
      ...
      sr.pAdditionalTable1[1][1] = DataTypeDistance.getDistance(totalDistanceInDefaultUnit)
          .getStringValueInKilometers(true, 0, 0);
    }
  }
}
