...
public class CompetitionDRVWanderruderstatistik extends Competition {
  ...
  public int calculateAggregation(Hashtable<Object, StatisticsData> data,
    ...
    if (jahrgang == 0) {
      ...
      String wtext = "Das Alter des Teilnehmers '" + person.getQualifiedName()
          + "' konnte nicht ermittelt werden, da sein/ihr Jahrgang "
    ...
    meters = sd.compData.etappen.get(etappenName);
    if (meters == null) {
      ...
      if (meters.longValue() != distanceInMeters) {
        ...
        // immer den größeren Km-Wert verwenden (falls eine Etappe wegen Landdienstwechsel
        // aufgeteilt wurde)
  ...
  public void calculate(StatisticsRecord sr, StatisticsData[] sd) {
    ...
    if (sr.sIsOutputCompRules) {
      sr.pCompRules = createAusgabeBedingungen(sr, wett.key, sr.pCompRulesBold,
          sr.pCompRulesItalics);
    }

    if (!checkWettZeitraum(sr.sCompYear, sr.sStartDate, sr.sEndDate,
        WettDefs.DRV_WANDERRUDERSTATISTIK)) {
    ...
    sr.pOutputLinesAbove.addLine("Name des Vereins:|" + Daten.project.getClubName(), 2,
        StatOutputLines.FONT_BOLD);
    sr.pOutputLinesAbove.addLine("Ort:|" + Daten.project.getClubAddressCity(), 2,
        StatOutputLines.FONT_BOLD);
    sr.pOutputLinesAbove.addLine(
        "DRV Mitgliedsnummer:|" + Daten.project.getClubGlobalAssociationMemberNo(), 2,
        StatOutputLines.FONT_BOLD);
    sr.pOutputLinesAbove.addLine("LRV:|" + Daten.project.getClubRegionalAssociationName(), 2,
        StatOutputLines.FONT_BOLD);
    String mitgl = "";
    ...
    sr.pOutputLinesBelow.addLine("Anschrift des Wanderruderwartes bzw. des Ausfüllers: ", 1,
        StatOutputLines.FONT_BOLD);
    sr.pOutputLinesBelow.addLine(Daten.project.getClubName(), 1, StatOutputLines.FONT_NORMAL);
    sr.pOutputLinesBelow.addLine("c/o " + Daten.project.getAdminName(), 1,
    ...
    for (StatisticsData element : sd) {
      ...
      long tmp;
      sr.pAdditionalTable1[pos][0] = (element.entryNo != null ? element.entryNo.toString() : "");
      sr.pAdditionalTable1[pos][1] = element.sName;
      sr.pAdditionalTable1[pos][2] = (element.sAdditional != null ? element.sAdditional : "");

      if (element.compData != null) {
        ...
        sr.pAdditionalTable1[pos][3] = printWatersHash(element.compData.waters);
        _gesWaters.putAll(element.compData.waters);

        long tmpTotalMeters = getSumOfAllHashEntries(element.compData.etappen);
        _gesMeters += tmpTotalMeters;
        sr.pAdditionalTable1[pos][4] = DataTypeDistance.getDistanceFromMeters(tmpTotalMeters)
            .getStringValueInKilometers();

        _gesTage += element.compData.activeDays;
        sr.pAdditionalTable1[pos][5] = Integer.toString(element.compData.activeDays);

        int tmpAnzTeiln = element.compData.teilnFbis18.size()
            + element.compData.teilnFueber18.size()
            + element.compData.teilnMbis18.size() + element.compData.teilnMueber18.size();
        sr.pAdditionalTable1[pos][6] = Integer.toString(tmpAnzTeiln);

        _gesCrewMeters += element.compData.totalDistanceInMeters;
        sr.pAdditionalTable1[pos][7] = DataTypeDistance.getDistanceFromMeters(
            element.compData.totalDistanceInMeters).getStringValueInKilometers();
        ...
        _gesTeilnMueber18 += element.compData.teilnMueber18.size();
        sr.pAdditionalTable1[pos][8] = Integer.toString(element.compData.teilnMueber18.size());

        tmp = getSumOfAllHashEntries(element.compData.teilnMueber18);
        _gesMeterTeilnMueber18 += tmp;
        sr.pAdditionalTable1[pos][9] = DataTypeDistance.getDistanceFromMeters(tmp)
            .getStringValueInKilometers();

        _gesTeilnMbis18 += element.compData.teilnMbis18.size();
        sr.pAdditionalTable1[pos][10] = Integer.toString(element.compData.teilnMbis18.size());

        tmp = getSumOfAllHashEntries(element.compData.teilnMbis18);
        _gesMeterTeilnMbis18 += tmp;
        sr.pAdditionalTable1[pos][11] = DataTypeDistance.getDistanceFromMeters(tmp)
            .getStringValueInKilometers();

        _gesTeilnFueber18 += element.compData.teilnFueber18.size();
        sr.pAdditionalTable1[pos][12] = Integer.toString(element.compData.teilnFueber18.size());

        tmp = getSumOfAllHashEntries(element.compData.teilnFueber18);
        _gesMeterTeilnFueber18 += tmp;
        sr.pAdditionalTable1[pos][13] = DataTypeDistance.getDistanceFromMeters(tmp)
            .getStringValueInKilometers();

        _gesTeilnFbis18 += element.compData.teilnFbis18.size();
        sr.pAdditionalTable1[pos][14] = Integer.toString(element.compData.teilnFbis18.size());

        tmp = getSumOfAllHashEntries(element.compData.teilnFbis18);
        _gesMeterTeilnFbis18 += tmp;
        sr.pAdditionalTable1[pos][15] = DataTypeDistance.getDistanceFromMeters(tmp)
            .getStringValueInKilometers();
    ...
    sr.pAdditionalTable1[anzMtours][4] = DataTypeDistance.getDistanceFromMeters(_gesMeters)
        .getStringValueInKilometers();
    sr.pAdditionalTable1[anzMtours][5] = Integer.toString(_gesTage);
    sr.pAdditionalTable1[anzMtours][6] = Integer.toString(_gesTeilnMueber18 + _gesTeilnMbis18
        + _gesTeilnFueber18 + _gesTeilnFbis18);
    sr.pAdditionalTable1[anzMtours][7] = DataTypeDistance.getDistanceFromMeters(_gesCrewMeters)
        .getStringValueInKilometers();
    sr.pAdditionalTable1[anzMtours][8] = Integer.toString(_gesTeilnMueber18);
    sr.pAdditionalTable1[anzMtours][9] = DataTypeDistance.getDistanceFromMeters(
        _gesMeterTeilnMueber18).getStringValueInKilometers();
    sr.pAdditionalTable1[anzMtours][10] = Integer.toString(_gesTeilnMbis18);
    sr.pAdditionalTable1[anzMtours][11] = DataTypeDistance.getDistanceFromMeters(
        _gesMeterTeilnMbis18).getStringValueInKilometers();
    sr.pAdditionalTable1[anzMtours][12] = Integer.toString(_gesTeilnFueber18);
    sr.pAdditionalTable1[anzMtours][13] = DataTypeDistance.getDistanceFromMeters(
        _gesMeterTeilnFueber18).getStringValueInKilometers();
    sr.pAdditionalTable1[anzMtours][14] = Integer.toString(_gesTeilnFbis18);
    sr.pAdditionalTable1[anzMtours][15] = DataTypeDistance.getDistanceFromMeters(
        _gesMeterTeilnFbis18).getStringValueInKilometers();
    ...
    if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett && alleAktive != null) {
      ...
      for (Object element : aktive) {
        String s = alleAktive.get(element);
        if (s != null) {
