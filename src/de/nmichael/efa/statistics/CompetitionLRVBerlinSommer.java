...
public class CompetitionLRVBerlinSommer extends Competition {
  ...
  protected int countZf(Zielfahrt[] z, ZielfahrtFolge zf) {
    ...
    if (z == null) {
      if (zf == null) {
        ...
        return (zfAnz < 4 ? zfAnz : 3); // wenn z==null, dann maximal 3 zurückgeben (weil: zf könnte
        // mehrere Fahrten an einem Tag enthalten)
      }
    }
  ...
  protected String zfAusgabeString(String szf, int izf, boolean bzf, boolean kurz,
      boolean mitAnford, int geforderteFahrten) {
    if (szf.length() == 0) {
      izf = 0;
    }
    if (!bzf) {
  ...
  protected boolean zfErfuellt(Zielfahrt[] zf) {
    if (zf == null || zf.length < 4 || zf[0] == null || zf[1] == null || zf[2] == null
        || zf[3] == null) {
      return false;
    }
  ...
  public void calculate(StatisticsRecord sr, StatisticsData[] sd) {
    ...
    for (int i = 0; i < sd.length; i++) {
      ...
      sd[i].additionalDestinationAreas = getAdditionalZf(sd[i].destinationAreaVector,
          sd[i].bestDestinationAreas);
    }
          ...
          if (erfuellt
            ...
            if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett) {
              ...
              if (erfuellt && sd[i].personRecord != null) {
                ...
                if (sd[i].bestDestinationAreas != null && sd[i].additionalDestinationAreas != null
                  ...
                  for (int j = 0; j < sd[i].additionalDestinationAreas.length; j++) {
                    if (sd[i].additionalDestinationAreas[j] != null
                        && j + lengthBefore < ewm.fahrt.length) {
                      for (int jj = 0; jj < 4; jj++) {
                        switch (jj) {
                          case 0:
                            ewm.fahrt[j + lengthBefore][jj] = sd[i].additionalDestinationAreas[j]
                                .getDatum();
                            break;
                          case 1:
                            ewm.fahrt[j + lengthBefore][jj] = sd[i].additionalDestinationAreas[j]
              ...
              if (!sr.sIsOutputCompWithoutDetails && erfuellt) {
                ...
                for (int j = 0; sr.sIsOutputCompAllDestinationAreas
                    && sd[i].additionalDestinationAreas != null
