          ...
          if (erfuellt
            ...
            if (!sr.sIsOutputCompWithoutDetails && erfuellt) {
              ...
              if (!sr.sIsOutputCompShort && warnung) {
                participant.sAdditional += " (davon nur " + winterAnz
                    + " wertbare Fahrten, da mehrere Fahrten am selben Tag)";
              }
            }
            ...
            if (erfuellt && !sr.sIsOutputCompWithoutDetails) {
              ...
              for (String[][] element : winterfahrten) {
                for (int j = 0; j < element.length; j++) {
                  if (j == 0 && element[j][0] != null) {
                    gefm++;
                  }
                  if (c < gruppen[g].zusatz && gruppen[g].zusatz - c > anzMonate - gefm
                      && element[j][0] != null) {
                    if (sr.getOutputTypeEnum() == StatisticsRecord.OutputTypes.efawett) {
                      ewm.fahrt[c] = element[j];
                    } else {
                      participant.sDetailsArray[c] = element[j];
                    }
                    c++;
                    teilkm += EfaUtil.zehntelString2Int(element[j][2]);
                  }
                }
              }
            }
