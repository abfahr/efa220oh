...
public class StatisticHTMLWriter extends StatisticWriter {
  ...
  public boolean write() {
    ...
    try {
      ...
      if (sr.sOutputHtmlUpdateTable) {
        ...
        fo = new BufferedReader(new InputStreamReader(new FileInputStream(tmpFile),
            sr.getOutputEncoding()));
      }

      // Datei erstellen und Kopf schreiben
      f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(sr.sOutputFile),
          sr.getOutputEncoding()));
      if (sr.sOutputHtmlUpdateTable) {
        String zz;
      ...
      f.write("<tr><td class=\"header_name\">"
          ...
          + ":</td><td class=\"header_value\"><a href=\"" + sr.pStatCreatedByUrl + "\">"
          + sr.pStatCreatedByName + "</a></td></tr>\n");
      f.write("<tr><td class=\"header_name\">"
      ...
      if (sr.pStatFilter != null) {
        f.write("<tr><td class=\"header_name\">"
            ...
            + ":</td><td class=\"header_value\">"
            + EfaUtil.replace(sr.pStatFilter, "\n", "<br>", true) + "</td></tr>\n");
      }
                  ...
                  for (String[] element : participant.sDetailsArray) {
                    ...
                    if (element != null
                      ...
                      for (int k = 0; k < element.length; k++) {
                        f.write("<td class=\"comp_entry_details\">"
                            + element[k] + "</td>");
                      }
                    } else {
                      f.write("<td class=\"comp_entry_details\" colspan=\"" + colspan
                          + "\">und weitere Fahrten</td>");
                    }
                    f.write("</tr>\n");
                  }
                }
  ...
  void outHTML(BufferedWriter f, String s, long value, long maximum, String cssclass, long barSize)
      throws IOException {
    if (s != null) {
      f.write("<td"
          + (cssclass == null || barSize == 0 ? " class=\"entry_value\"" : " class=\"entry_bar\"")
          + ">");
  ...
  /*
   * void outHTML(BufferedWriter f, String s, long value, long maximum, String colorBar, long
   * barSize) throws IOException { if (s != null) { f.write("<td" + (colorBar == null || barSize ==
   * 0 ? " align=\"right\"" : "") + ">"); if (colorBar != null && barSize != 0 && value != 0 &&
   * maximum > 0) { long width = value*barSize / maximum; if (width <= 0) { width = 1; }
   * f.write("<img src=\"" + EfaUtil.saveImage("color_" + colorBar + ".gif", "gif", sr.sOutputDir,
   * true, false, false) + "\" width=\"" + width + "\" height=\"20\" alt=\"\">&nbsp;"); } if
   * (s.length() > 0) { f.write(EfaUtil.escapeHtml(s)); } f.write("</td>\n"); } }
   */
