...
public class StatisticTask extends ProgressTask {
    ...
    if (splitSessionIntoDays) {
      ...
      if (entryDestinationAreas != null && entryDestinationAreas.getAnzZielfahrten() > 0) {
        ...
        if (entryDestinationAreas.getAnzZielfahrten() > 1) {
          sr.cWarnings.put("Fahrt #" + entryNo.toString()
              + " hat zu viele Zielfahrten; überzählige Zielbereiche werden ignoriert.",
              "foo");
        }
      }
  ...
  private int calculateAggregations(ClubworkRecord r, Object key, double hours,
    ...
    // aggregate
    if (sr.sIsAggrClubwork || sr.sIsAggrClubworkRelativeToTarget
        || sr.sIsAggrClubworkOverUnderCarryOver) {
      sd.clubwork += hours;
      sd.clubworkTargetHours = targetHours;
    }
    if (sr.sIsAggrClubworkCredit
        && r.getDescription().startsWith(International.getString("Gutschrift"))) {
  ...
  private int calculateAggregationsForMatrix(LogbookRecord r, Object key, long distance, int seat) {
    ...
    if (sr.sStatistikKey == StatisticsRecord.StatisticKey.name) {
      for (int i = 0; i <= LogbookRecord.CREW_MAX; i++) {
        ...
        PersonRecord pRecord = (pId != null ? persons.getPerson(pId, r.getValidAtTimestamp())
            : null);
        boolean pGuest = (pRecord != null ? !pRecord.isStatusMember() : false);
        boolean pOther = (pRecord != null ? statusOther.getId().equals(pRecord.getStatusId())
            : true);
  ...
  private Object getAggregationKey_boatTypeDetail(String boatType, String boatSeats,
      String boatCoxing) {
  ...
  private Object getAggregationKey_month(DataTypeDate date) {
    if (date != null && date.isSet()) {
      return date.getMonthAsStringWithIntMarking(StatisticsData.SORTING_PREFIX,
          StatisticsData.SORTING_POSTFIX);
    }
    return null;
  }
  ...
  private Object getAggregationKey_weekday(DataTypeDate date) {
    if (date != null && date.isSet()) {
      return date.getWeekdayAsStringWithIntMarking(StatisticsData.SORTING_PREFIX,
          StatisticsData.SORTING_POSTFIX);
    }
    return null;
  }
  ...
  private String getAggregationKey_timeOfDay(int hour) {
    return StatisticsData.SORTING_PREFIX + EfaUtil.int2String(hour, 2, true)
        + StatisticsData.SORTING_POSTFIX +
  ...
  private Object getAggregationKeyForList(LogbookRecord r) {
    ...
    switch (sr.sStatistikKey) {
      ...
      case timeOfDay:
        ...
        long totalDistance = (r.getDistance() != null ? r.getDistance().getValueInDefaultUnit()
            : 0);
        if (totalDistance == 0) {
          return null;
        }
        ...
        for (String k : keys) {
          ...
          long distance = (long) ((((double) minutes) / ((double) elapsed)) * (totalDistance));
  ...
  private int calculateEntryForMatrix(LogbookRecord r) {
    ...
    if (sr.sStatisticTypeEnum == StatisticsRecord.StatisticType.persons) {
      for (int i = 0; i <= LogbookRecord.CREW_MAX; i++) {
        ...
        if (isInPersonFilter() && isInGroupFilter()) {
          Object aggregationKey = getAggregationKey_persons(entryPersonRecord, entryPersonId,
              entryPersonName,
              entryPersonIsGuest, entryPersonIsOther);
          if (aggregationKey != null) {
    ...
    if (sr.sStatisticTypeEnum == StatisticsRecord.StatisticType.boats) {
      if (isAtLeastOneInPersonOrGroupFilter(r)) {
        Object aggregationKey = getAggregationKey_boats(entryBoatRecord, entryBoatId,
            entryBoatName);
        if (aggregationKey != null) {
          cnt += calculateAggregationsForMatrix(r, aggregationKey,
  ...
  private int calculateEntryForLogbook(LogbookRecord r) {
    ...
    if (sr.sIsLFieldsEndDate) {
      sd.logbookFields[col++] = (entryEndDate != null && entryEndDate.isSet() ? entryEndDate
          .toString() : "");
    }
    if (sr.sIsLFieldsBoat) {
      sd.logbookFields[col++] = (entryBoatRecord != null ? entryBoatRecord.getQualifiedName()
          : entryBoatName);
    }
    String coxName = "";
    String crewNames = "";
    for (int i = 0; i <= LogbookRecord.CREW_MAX; i++) {
      getEntryPerson(r, i);
      String name = (entryPersonRecord != null ? entryPersonRecord.getQualifiedName()
          : entryPersonName);
      if (name == null) {
        name = "";
      }
      if (sr.sPublicStatistic && entryPersonExcludeFromPublic) {
        name = "<" + International.getString("anonym") + ">"; // < and > will be correctly escaped
        // in output!
      }
    ...
    if (sr.sIsLFieldsDistance) {
      ...
      sd.distance = (r.getDistance() != null
          ? r.getDistance().getValueInDefaultUnit()
          : 0);
    }
        ...
        if (aggregationKey != null) {
          if (!sr.sStatisticType.equals(WettDefs.STR_DRV_WANDERRUDERSTATISTIK)) {
            ...
            if (sr.cCompetition != null) {
              cnt += ((CompetitionDRVWanderruderstatistik) sr.cCompetition).calculateAggregation(
                  data,
  ...
  private void calculateEntry(LogbookRecord r) {
    ...
    if (entryDate != null && entryDate.isSet()) {
      ...
      if (entryEndDate != null && entryEndDate.isSet()
          && (sr.cEntryDateLast == null || entryEndDate.isAfter(sr.cEntryDateLast))) {
  ...
  private void calculateEntry(Object key, BoatRecord r, DataTypeDate date,
    ...
    this.entryDate = date;
    this.entryValidAt = (date != null && date.isSet() ? date.getTimestamp(null)
        : System
            .currentTimeMillis());
  ...
  private void getEntryPerson(PersonRecord entryPersonRecord) {
    ...
    entryPersonIsGuest = (entryPersonStatusId != null && entryPersonStatusId
  ...
  private boolean isInRange(LogbookRecord r) {
    ...
    if (entryEndDate == null || !entryEndDate.isSet()) {
      ...
      return entryDate.isInRange(sr.sStartDate, sr.sEndDate) && // both start *and* end date must be
      // in range!
    ...
    if (sr.sPublicStatistic && entryBoatExclude &&
      ...
      if (isAtLeastOneInPersonOrGroupFilter(r)) {
        sr.pStatIgnored.put((entryBoatRecord != null
            ? entryBoatRecord.getQualifiedName()
            : entryBoatName), "foo");
      }
      return false;
    }
  ...
  private StatisticsData[] runPostprocessing() {
    ...
    for (Object key : keys) {
      ...
      if ((sr.sStatisticCategory == StatisticsRecord.StatisticCategory.competition
        ...
        if (sr.sIsFieldsName) {
          sd.sName = (pr != null ? pr.getQualifiedName()
              : (isUUID ? "*** " + International.getString("ungültiger Eintrag") + " ***"
                  : sd.key
                      .toString()));
        }
      ...
      if ((sr.sStatisticTypeEnum == StatisticsRecord.StatisticType.boats &&
        ...
        if (sr.sIsFieldsBoatType) {
          sd.sBoatType = (br != null ? br.getDetailedBoatType(0)
              : Daten.efaTypes.getValue(
                  EfaTypes.CATEGORY_BOAT, EfaTypes.TYPE_BOAT_OTHER));
        }
    ...
    if (sr.sStatisticCategory == StatisticsRecord.StatisticCategory.logbook) {
      ...
      if (sr.sLFieldDistancePos >= 0 && sr.sLFieldDistancePos < sdSummary.logbookFields.length) {
        sdSummary.logbookFields[sr.sLFieldDistancePos] = DataTypeDistance
            .getDistance(sdSummary.distance).getStringValueInDefaultUnit(
                sr.sDistanceWithUnit, 0,
                (sr.sTruncateDistanceToFullValue ? 0 : 1));
      }
    }

    // Create Array and sort
    boolean summax = (sr.sStatisticCategory != StatisticsRecord.StatisticCategory.competition ? true
        : false);
    ...
    for (int i = 0; i < sdArray.length; i++) {
      ...
      sdArray[i].createStringOutputValues(sr, i,
          positionSet ? sdArray[i].sPosition : sdArray[i].getPositionString(sdArray, i) + ".");
      ...
      case matrix:
        statDescrShort = sr.getStatisticCategoryDescription() + " "
            + (sr.sStatistikKey == StatisticsRecord.StatisticKey.name
                ...
                : sr.getStatisticKeyDescriptionPlural());
    ...
    if (isTemporaryStatistic) {
      ...
      if (sr.cEntryDateFirst != null && sr.cEntryDateFirst.isSet()) {
        ...
        if (sr.cEntryDateLast != null && sr.cEntryDateLast.isSet()
            && sr.cEntryDateFirst.getYear() != sr.cEntryDateLast.getYear()) {
    ...
    if (sr.cNumberOfEntries > 0 && sr.cEntryNoFirst != null && sr.cEntryNoLast != null) {
      sr.pStatConsideredEntries += ": #" + sr.cEntryNoFirst.toString() + " - #"
          + sr.cEntryNoLast.toString();
      if (sr.cEntryDateFirst != null && sr.cEntryDateFirst.isSet() && sr.cEntryDateLast != null
          && sr.cEntryDateLast.isSet()) {
  ...
  private boolean createStatisticLogbook(StatisticsRecord sr, int statisticsNumber) {
    ...
    if (logbooks.size() == 0) {
      Dialog.error(International.getMessage(
          "Keine {items} im Zeitraum {fromdate} bis {todate} gefunden.",
          ...
          if (r != null) {
            if (Logger.isTraceOn(Logger.TT_STATISTICS, 9)) {
              Logger.log(Logger.DEBUG, Logger.MSG_STAT_VISITEDENTRIES,
                  "visited: "
                      + (r.getEntryId() != null ? r.getEntryId().toString() : "EntryId = <null>"));
            }
            calculateEntry(r);
          }
          this.setCurrentWorkDone(((++pos * WORK_PER_LOGBOOK) / size) + (i * WORK_PER_LOGBOOK)
              + (statisticsNumber * WORK_PER_STATISTIC));
          k = it.getNext();
        }
      } catch (Exception e) {
        Logger.logdebug(e);
      ...
      switch (sr.sStatisticTypeEnum) {
        case boatstatus:
          ...
          while (k != null) {
            ...
            if (r != null) {
              ...
              if (br != null) {
                ...
                if (sr.sIsOFieldsBaseStatus) {
                  fields.add(BoatStatusRecord.getStatusDescription(r.getBaseStatus()));
                }
                if (sr.sIsOFieldsCurrentStatus) {
                  fields.add(BoatStatusRecord.getStatusDescription(r.getCurrentStatus()));
                }
            ...
            this.setCurrentWorkDone(((++count * WORK_PER_STATISTIC) / size)
                + (statisticsNumber * WORK_PER_STATISTIC));
            k = it.getNext();
          }
          return true;
            ...
            if (r != null) {
              ...
              if (br != null) {
                ...
                if (sr.sIsOFieldsReservedFrom) {
                  fields.add(r.getDateTimeFromDescription(BoatReservationRecord.KEEP_NUM_DATE));
                }
                if (sr.sIsOFieldsReservedTo) {
                  fields.add(r.getDateTimeToDescription(BoatReservationRecord.KEEP_NUM_DATE));
                }
                ...
                calculateEntry(br.getQualifiedName() + StatisticsData.KEY_MODIFIER + count, br,
                    r.getDateFrom(), fields, 0);
              }
            }
            this.setCurrentWorkDone(((++count * WORK_PER_STATISTIC) / size)
                + (statisticsNumber * WORK_PER_STATISTIC));
        ...
        case boatdamagestat:
          ...
          while (k != null) {
            ...
            if (r != null && (!r.getFixed() || !sr.sFilterOnlyOpenDamages)) {
              BoatRecord br = r.getBoatRecord();
              if (br != null) {
                calculateEntry(br.getQualifiedName(), br, r.getReportDate(), null,
                    r.getRepairDays());
              }
            }
            this.setCurrentWorkDone(((++count * WORK_PER_STATISTIC) / size)
  ...
  private boolean createStatisticClubwork(StatisticsRecord sr, int statisticsNumber) {
    ...
    for (int i = 0; names != null && i < names.length; i++) {
      ...
      if (pr != null) {
        ...
        if (clubwork != null
            && DataTypeDate.isRangeOverlap(sr.sStartDate, sr.sEndDate, lbStart, lbEnd)) {
    ...
    Vector<PersonRecord> personRecords = persons.getAllPersons(sr.sStartDate.getTimestamp(null),
        sr.sEndDate.getTimestamp(null), false, false);
          ...
          if (thisYear != lastYearWS) {
            if (Dialog.yesNoDialog(International.onlyFor("DRV-Wanderruderstatistik", "de"),
                International.onlyFor(
                    "Möchtest du als nächstes auch die DRV-Wanderruderstatistik melden?",
                    "de")) == Dialog.YES) {
  ...
  public int getAbsoluteWork() {
    return (statisticsRecords != null ? statisticsRecords.length : 0) * WORK_PER_STATISTIC
        + WORK_POSTPROCESSING;
  }
  ...
  public static void createStatisticsTask(BaseFrame parentFrame, BaseDialog parentDialog,
      StatisticsRecord[] sr, AdminRecord admin) {
    ...
    ProgressDialog progressDialog = (parentFrame != null
        ? new ProgressDialog(parentFrame, International.getString("Statistik erstellen"),
            statisticTask, true, true)
        : (parentDialog != null
            ? new ProgressDialog(parentDialog, International.getString("Statistik erstellen"),
                statisticTask, true, true)
            : null));
