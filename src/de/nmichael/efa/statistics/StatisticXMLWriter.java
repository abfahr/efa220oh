...
public class StatisticXMLWriter extends StatisticWriter {
  ...
  private void writeTable(BufferedWriter f, String tableName, String[] header, String[][] data)
      throws IOException {
    ...
    for (String[] element : data) {
      ...
      for (int j = 0; j < element.length; j++) {
        if (header != null) {
          write(f, indent, xmltag(FIELD_TABLE_CELL, element[j], header[j]));
        } else {
          write(f, indent, xmltag(FIELD_TABLE_CELL, element[j]));
        }
      }
  ...
  public boolean write(String filename, boolean printColumnHeaders) {
    ...
    try {
      // Create File
      f = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filename),
          Daten.ENCODING_UTF));
      ...
      if (sr.pStatIgnored != null && sr.pStatIgnored.size() > 0) {
        write(
            f,
            indent,
            xmltag(FIELD_HEADER_IGNORED, Integer.toString(sr.pStatIgnored.size()),
                ATTR_DESCRIPTION,
      ...
      if (sr.pCompGroupNames != null && sr.pCompParticipants != null) {
        ...
        if (sr.pCompRules != null) {
          ...
          for (String pCompRule : sr.pCompRules) {
            compRules.append(pCompRule + " ");
          }
        ...
        for (int i = 0; i < sr.pCompGroupNames.length; i++) {
          ...
          for (StatisticsData participant = sr.pCompParticipants[i]; participant != null; participant = participant.next) {
            ...
            write(f, indent,
                xmltag(FIELD_COMPETITION_GROUP_PARTICIPANT_YEAROFBIRTH, participant.sYearOfBirth));
            write(
                f,
                indent,
                xmltag(FIELD_COMPETITION_GROUP_PARTICIPANT_FULFILLED,
                    Boolean.toString(participant.compFulfilled)));
            write(f, indent,
                xmltag(FIELD_COMPETITION_GROUP_PARTICIPANT_DISTANCE, participant.sDistance));
            write(f, indent,
                xmltag(FIELD_COMPETITION_GROUP_PARTICIPANT_ADDITIONAL, participant.sAdditional));
            // ausführliche Ausgabe
            if (participant.sDetailsArray != null && participant.sDetailsArray.length > 0) {
              writeTable(f, FIELD_COMPETITION_GROUP_PARTICIPANT_DETAILS, null,
                  participant.sDetailsArray);
            }
      ...
      if (sr.pTableColumns != null && sr.pTableColumns.size() > 0) {
        ...
        for (int i = 0; i < sd.length; i++) {
          ...
          if (sr.sStatisticCategory == StatisticsRecord.StatisticCategory.list ||
            ...
            write(f, indent,
                xmltag(FIELD_ITEM_CLUBWORKCARRYOVER, sd[i].sClubworkOverUnderCarryOver));
