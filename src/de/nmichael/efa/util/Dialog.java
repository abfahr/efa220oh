...
public class Dialog {
  ...
  public static Dimension getMaxSize(Dimension d) {
    ...
    if (d.getWidth() > screenSize.getWidth() || d.getHeight() > screenSize.getHeight()) {
      newd.setSize((int) Math.min(d.getWidth(), screenSize.getWidth()),
          (int) Math.min(d.getHeight(), screenSize.getHeight()));
    }
    return newd;
  }
  ...
  public static String chopDialogString(String s) {
    ...
    try {
      ...
      for (int i = 0; i < s.length(); i++) {
        if (s.charAt(i) != '\n') {
          if (++chars > MAX_DIALOG_WIDTH) {
            int splitAt = -1;
            for (int j = i; j > i - (MAX_DIALOG_WIDTH / 2); j--) {
              if (s.charAt(j) == ' ') {
                splitAt = j;
                break;
              }
      ...
      if (lines > MAX_DIALOG_HEIGHT) {
        ...
        s = "";
        for (int i = 0; i < MAX_DIALOG_HEIGHT / 2; i++) {
          s += (String) _s.get(i) + "\n";
        }
        s += "...\n";
        int remaining = MAX_DIALOG_HEIGHT - (MAX_DIALOG_HEIGHT / 2 + 1);
  ...
  public static void frameOpened(Window w) {
    ...
    if (Logger.isDebugLogging() && Logger.isTraceOn(Logger.TT_GUI)) {
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_GUI_WINDOWS, "Dialog.frameOpened("
  ...
  public static void frameClosed(Window w) {
    ...
    if (Logger.isDebugLogging() && Logger.isTraceOn(Logger.TT_GUI)) {
      Logger.log(Logger.DEBUG, Logger.MSG_DEBUG_GUI_WINDOWS, "Dialog.frameClosed("
          + w.getClass().getCanonicalName() + ")");
    }
  ...
  public static String dateiDialog(Window frame, String titel, String typen, String extension,
      String startdir, String selectedfile, String buttontxt, boolean save, boolean dirsOnly) {
    JFileChooser dlg;
    ...
    try {
      ...
      if (typen != null && extension != null) {
        ...
        if ((wo = extension.indexOf("|")) >= 0) {
          ...
          dlg.setFileFilter(new EfaFileFilter(typen, extension));
        }
  ...
  public static void setFontSize(UIDefaults uid, String font, int size, int style) {
    ...
    if (orgFont == null) {
      Logger.log(Logger.WARNING, Logger.MSG_WARN_FONTDOESNOTEXIST,
          International.getMessage(
              "Schriftart {font} exisitert nicht; ihre Größe kann nicht geändert werden!", font));
      return;
    }
    ...
    if (style == -1) {
      ...
      newFont = orgFont.deriveFont(style, size);
    }
