...
/**
 * Adapted from
 * http://www.devdaily.com/java/jwarehouse/jazzy/src/com/swabunga/spell/engine/EditDistance
 * .java.shtml
 */
public class EditDistance {
  ...
  public static final int getDistance(String word, String similar) {
    ...
    {
      matrix[i][0] = matrix[i - 1][0] + costOfInsertingSourceCharacter; // initialize the first
      // column
    }
    ...
    for (int i = 1; i != a_size; ++i) {
      ...
      for (int j = 1; j != b_size; ++j) {
        ...
        if (sourceChar == otherChar) {
          matrix[i][j] = matrix[i - 1][j - 1]; // no change required, so just carry the current cost
          // up
          continue;
        }
        ...
        boolean isSwap = (i != 1) && (j != 1) && sourceChar == similar.charAt(j - 1)
            && word.charAt(i - 1) == otherChar;
        if (isSwap) {
