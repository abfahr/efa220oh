...
public class EfaUtil {
  ...
  public static TMJ string2date(String s, int tt, int mm, int yy) {
    ...
    for (int i = 0; i < s.length(); i++) {
      ...
      if (s.charAt(i) == '-' && t.length() == 0 && i + 1 < s.length()
  ...
  // Aus einem String s ein korrektes (gültiges) Datum machen, ggf. aktuelles Datum als
  // Referenzdatum verwenden
  public static String correctDate(String s) {
    Calendar cal = new GregorianCalendar();
    TMJ tmj = correctDate(s, cal.get(Calendar.DAY_OF_MONTH), cal.get(Calendar.MONTH) + 1,
  ...
  public static String correctTime(String s, int hdef, int mdef, int sdef, boolean withSeconds,
    ...
    if (useZeroAsMinuteAndSecondIfHourWasGivenInS) {
      hhmmss = EfaUtil.string2date(s, -1, -1, -1); // TMJ mißbraucht für die Auswertung von
      // Uhrzeiten
      if (hhmmss.tag != -1) {
        mdef = 0;
        sdef = 0;
    ...
    hhmmss = EfaUtil.string2date(s, hdef, mdef, sdef); // TMJ mißbraucht für die Auswertung von
    // Uhrzeiten
    int hh = hhmmss.tag;
    int mm = hhmmss.monat;
    int ss = hhmmss.jahr;
  ...
  public static String zehntelInt2String(int i) {
    ...
    if (s.endsWith("0")) {
      ...
      s = s.substring(0, s.length() - 1) + International.getDecimalSeparator()
  ...
  /*
   * // Eine Komma-Liste in ein Array of String umwandeln public static String[]
   * kommaList2Arr(String s, char sep, boolean addAndere) { if (s == null) return null; Vector v; if
   * (s.length()>0) v = split(s,sep); else v = new Vector(); String[] aa = new String[v.size()+
   * (addAndere ? 1 : 0) ]; for (int ii=0; ii<v.size(); ii++) aa[ii] = (String)v.get(ii); if
   * (addAndere) aa[aa.length-1] = "andere"; // nur für diese Listen soll ein "andere" angef return
   * aa; } public static String[] statusList2Arr(String s) { return kommaList2Arr(s,',',true); }
   */
  ...
  // Differenz (Anzahl der Tage) zwischen zwei TMJ-Datumsangaben berechnen (von und bis
  // mitgerechnet)
  ...
  // Differenz (Anzahl der Tage) zwischen zwei String-Datumsangaben berechnen (von und bis
  // mitgerechnet)
  ...
  // Differenz (Anzahl der Tage) zwischen zwei TMJ-Datumsangaben berechnen (von und bis *nicht*
  // mitgerechnet),
  ...
  public static int getRealDateDiff(TMJ d1, TMJ d2) {
    ...
    return Math
        .round((((float) c2.getTime().getTime() - (float) c1.getTime().getTime()) / 86400000));
  }
  ...
  public static TMJ incDate(TMJ tmj, int diff) {
    ...
    return new TMJ(cal.get(Calendar.DAY_OF_MONTH), cal.get(Calendar.MONTH) + 1,
        cal.get(Calendar.YEAR));
  }
  ...
  // Dateinamen "fileName" ggf. um Pfadangabe "basePath" vervollständigen, falls "fileName" kein
  // absoluter Pfad
  ...
  // Dateinamen "fileName" relativ zum Namen "efbName" machen, d.h. falls gleiches Verzeichnis,
  // dieses weglassen
  ...
  // zu einem gegebenen (Synonym-)Namen s aus einer Datenliste l den passenden Originalnamen
  // heraussuchen
  ...
  // Auf File f (nur max. erste 2 MB) den SHA-Algorithmus anwenden und Ergebnis Hex-Codiert
  // zurückliefern
  ...
  // prüft, ob im String s das Zeichen pos ein "+" ist. Falls der String zu kurz ist, wird false
  // geliefert
  ...
  public static GregorianCalendar dateTime2Cal(String dateTTMMJJ, String timeHHMMSS) {
    ...
    return new GregorianCalendar(datum.jahr, datum.monat - 1, datum.tag, zeit.tag, zeit.monat,
        zeit.jahr);
  }
  ...
  public static String getColor(Color c) {
    ...
    for (float element : rgb) {
      s += EfaUtil.hexByte((int) (element * 255));
    }
    return s;
  }
    ...
    try {
      ...
      while (file != null) {
        ...
        if (file.isDirectory()) {
            ...
            if (!(new File(destDir + filename)).mkdirs()) {
              result = (result == null ? "" : result + "\n")
                  + LogString.operationFailed(
                      ...
                      LogString.directoryCreationFailed(destDir + filename,
          ...
          // normale Datei
          try {
            // enthälter Dateiname einen Verzeichnisnamen eines Verzeichnisses, das möglicherweise
            ...
            if (dir != null && !(new File(destDir + dir)).isDirectory()) {
              if (!(new File(destDir + dir)).mkdirs()) {
                result = (result == null ? "" : result + "\n")
                    + LogString.operationFailed(
                        International.getString("Entpacken"),
                        LogString.directoryCreationFailed(destDir + dir,
                            International.getString("Verzeichnis"))
                            + "\n"
                            + LogString.operationAborted(International.getString("Entpacken")));
              }
            ...
            BufferedOutputStream f = new BufferedOutputStream(new FileOutputStream(destDir
                + filename));
  ...
  public static String createZipArchive(Vector<String> sourceDirs, Vector<Boolean> inclSubdirs,
    ...
    try {
      ...
      for (int j = 0; j < sourceDirs.size(); j++) {
        ...
        if (reldir.length() > 2 && reldir.charAt(1) == ':'
            && reldir.charAt(2) == Daten.fileSep.charAt(0)) {
          reldir = reldir.substring(3, reldir.length());
        }
        if (!Daten.fileSep.equals("/")) {
          reldir = EfaUtil.replace(reldir, Daten.fileSep, "/", true); // Bugfix: in <=160 konnten
          // die unter Windows
          // erstellten ZIP-Archive
          // unter Linux nicht richtig
          // gelesen werden
        }
        for (String file : files) {
          if ((new File(dir + file)).isDirectory()) {
            ...
            try {
              ...
              ZipEntry entry = new ZipEntry(reldir + file);
  ...
  /*
   * public static String getCertInfos(String keyStore, String alias, char[] password) { PrintStream
   * stdOut = System.out; CaptureOutputPrintStream cops = new CaptureOutputPrintStream(new
   * PipedOutputStream()); System.setOut(cops); try {
   * sun.security.tools.KeyTool.main(EfaUtil.kommaList2Arr("-list -v -alias "+alias+
   * " -storepass "+password+ " -keystore "+keyStore,' ',false)); } catch(Throwable e) { return "";
   * } System.setOut(stdOut); String s = ""; Vector v = cops.getLines(); for (int i=0; i<v.size();
   * i++) s += v.get(i).toString()+"\n"; return s; }
   */
  ...
  // Setzt für die Komponente c und alle ihre Subkomponenten die MinimumSize so, daß sie der
  // PreferredSize entspricht
  ...
  public static String saveImage(String image, String format, String dir,
    ...
    if (forceOverwrite || !EfaUtil.canOpenFile(fname)) {
      try {
        BufferedImage img = javax.imageio.ImageIO.read(EfaUtil.class.getResource(Daten.IMAGEPATH
            + image));
