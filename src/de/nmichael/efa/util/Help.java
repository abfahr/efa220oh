...
public class Help {
  ...
  public static HelpSet getHelpSet() {
    if (helpSet == null) {
      try {
        ...
        Logger.log(Logger.ERROR, Logger.MSG_HELP_ERRORHELPSET,
            "Cannot create HelpSet: " + e.toString());
      }
    }
    return helpSet;
  }
  ...
  public static HelpBroker getHelpBroker() {
    ...
    if (helpBroker == null && helpSet != null) {
      try {
        ...
        Logger.log(Logger.ERROR, Logger.MSG_HELP_ERRORHELPBROKER,
            "Cannot create HelpBroker: " + e.toString());
      }
    }
    return helpBroker;
  }
  ...
  public static void showHelp(String[] topics) {
    ...
    try {
      ((javax.help.DefaultHelpBroker) Help.getHelpBroker()).setActivationWindow(Dialog
          .frameCurrent());
      for (int i = 0; i < topics.length; i++) {
        try {
          if (Logger.isTraceOn(Logger.TT_HELP, 2)) {
            Logger.log(Logger.DEBUG, Logger.MSG_HELP_DEBUGHELPTOPICTRYHELP,
                "Help needed! Trying Help Topic: " + topics[i]);
          }
          Help.getHelpBroker().setCurrentID(topics[i]);
          break;
        } catch (Exception e) {
          if (Logger.isTraceOn(Logger.TT_HELP, 2)) {
            Logger.log(Logger.DEBUG, Logger.MSG_HELP_DEBUGHELPTOPICTNOTFOUND, "  -> Help Topic "
                + topics[i] + " not found.");
          }
          if (i + 1 == topics.length) {
            if (Logger.isTraceOn(Logger.TT_HELP, 2)) {
              Logger.log(Logger.DEBUG, Logger.MSG_HELP_DEBUGHELPTOPICFALLBACK,
                  "Fallback to default Help Topic: " + DEFAULT_TOPIC);
            }
      ...
      if (Logger.isTraceOn(Logger.TT_HELP, 1)) {
        Logger.log(Logger.DEBUG, Logger.MSG_HELP_DEBUGHELPTOPICSHOWHELP, "  -> Showing Help: "
            + Help.getHelpBroker().getCurrentID().getIDString());
      }
