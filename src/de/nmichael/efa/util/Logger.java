...
public class Logger {
  ...
  private static final int LOGGING_THRESHOLD = 1000; // max LOGGING_THRESHOLD logging messages per
  // second
  private static final int LOGGING_THRESHOLD_ERR = 100; // max LOGGING_THRESHOLD logging messages
  // per second
  ...
  private static boolean debugLoggingActivatedByCommandLine = false; // if set by Command Line, this
  // overwrites any configuration
  // in EfaConfig
  private static boolean globalTraceTopicSetByCommandLine = false; // if set by Command Line, this
  // overwrites any configuration
  // in EfaConfig
  private static boolean globalTraceLevelSetByCommandLine = false; // if set by Command Line, this
  // overwrites any configuration
  // in EfaConfig
      ...
      try {
        ...
        try {
          ...
          if (log.exists() && log.length() > MAX_LOG_FILE_SIZE) {
            baklog = EfaUtil.moveAndEmptyFile(Daten.efaLogfile,
                Daten.efaBaseConfig.efaUserDirectory + "backup" + Daten.fileSep);
          }
        } catch (Exception e) {
        ...
        efaErrorPrintStream = new EfaErrorPrintStream(
            new FileOutputStream(Daten.efaLogfile, append));
  ...
  /**
   * Log a message. Use this method for loggin!
   *
  ...
  public static String log(String type, String key, String txt, boolean msgToAdmin) {
    ...
    try {
      ...
      if (txt.length() < MAX_LOGMSG_SIZE) {
        t = "[" + EfaUtil.getCurrentTimeStamp() + "] - " + EfaUtil.getString(Daten.applName, 7)
            + " - " + Daten.applPID + " - " + EfaUtil.getString(type, 7) + " - " + key + " - "
            + txt;
      } else {
        t = "[" + EfaUtil.getCurrentTimeStamp() + "] - " + EfaUtil.getString(Daten.applName, 7)
            + " - " + Daten.applPID + " - " + EfaUtil.getString(type, 7) + " - " + key + " - "
            + txt.substring(0, MAX_LOGMSG_SIZE);
      }
      if (type != null && !type.equals(INPUT) && !type.equals(OUTPUT)) {
        ...
        EfaErrorPrintStream.ignoreExceptions = true; // Damit Exception-Ausschriften nicht
        // versehentlich als echte Exceptions gemeldet
        // werden
    ...
    } catch (Exception eignore) {} finally {
      inLogging = false;
    }
    return t;
  }
      ...
      if (globalTraceTopic != newTraceTopic) {
        ...
        if (debugLogging) {
          Logger.log(Logger.INFO, Logger.MSG_LOGGER_TRACETOPIC,
              "Trace Topic set to 0x" + Long.toString(globalTraceTopic, 16) + "."); // do not
          // internationalize!
        }
      }
  ...
  public static boolean setTraceLevel(int level, boolean setFromCommandLine) {
    ...
    if (globalTraceLevel != level) {
      ...
      if (debugLogging && globalTraceLevel > 0) {
        Logger.log(Logger.INFO, Logger.MSG_LOGGER_TRACETOPIC,
            "Trace Level set to " + Integer.toString(globalTraceLevel) + "."); // do not
        // internationalize!
      }
    }
    return true;
  ...
  private static void mailError(String key, String msg, String to) {
    ...
    try {
      ...
      txt.append(International
          .getString("Dies ist eine automatisch erstellte Fehlermeldung von efa.") + "\n\n"
